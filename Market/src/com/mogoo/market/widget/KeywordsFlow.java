...
public class KeywordsFlow extends FrameLayout implements OnGlobalLayoutListener {
	...
	private static final int[] ColorArray= {0xff12872F,0xff2E6BD9,0xffF3B93E,0xffA80000,0xffA330D5 ,0xff12872F,0xff2E6BD9,0xffF3B93E,0xffA80000,0xffA330D5,0xff12872F,0xff2E6BD9};
	...
	private static android.view.animation.Interpolator interpolator;
    ...
    private void disapper()
    {
    	...
    	for(int i=size-1;i>=0;i--)
    	{
    		...
    		int[] xy = new int[]{
    				layParams.leftMargin, layParams.topMargin, txt.getWidth() 
    		};
...
//    	int in;
//    	int out;
//        for (out = 1; out < endIdx; out++) {  
//        	int tmp = ((int[]) listTxt.get(out).getTag())[IDX_DIS_Y];
//        	in = out;
//        	while(in>0 && ((int[]) listTxt.get(out-1).getTag())[IDX_DIS_Y]>=tmp)
//              {
//        		((int[]) listTxt.get(in).getTag())[IDX_DIS_Y] = ((int[]) listTxt.get(in-1).getTag())[IDX_DIS_Y];
//        		--in;
//              TextView iTmp = listTxt.get(in);  
//              listTxt.set(in, iTmp);  
//              }
//    		TextView kTmp = listTxt.get(out);
//    		listTxt.set(out, kTmp);
//        }  
