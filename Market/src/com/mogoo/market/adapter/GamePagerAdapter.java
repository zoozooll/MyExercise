...
public class GamePagerAdapter extends PagerAdapter {
	...
	private PaginateListView_1<Game> mSubClassListView;
	...
	public GamePagerAdapter(Activity context) {
		mContext = context;
		mUrls = new String[] {
				HttpUrls.createBaseUrlWithPairs(HttpUrls.URL_GAME_LIST),
				HttpUrls.createBaseUrlWithPairs(HttpUrls.URL_TOP_GAME_LIST),
				HttpUrls.createBaseUrlWithPairs(HttpUrls.URL_NEWEST_GAME_LIST) };
	...
	private void initListViewAdapter() {
		IBeanDao<Game> subClassDao = DaoFactory.getGameCateDao(mContext);
		IBeanDao<HotApp> qualityDao = DaoFactory.getQualityGameDao(mContext);
		IBeanDao<HotApp> newestDao = DaoFactory.getNewestGameDao(mContext);
		
		mSubClassAdapter = new AppsAdapter<Game> (mContext, subClassDao.getAllBean(), mImageLoader);
		mSubClassListView = new PaginateListView_1<Game>(mContext, mSubClassAdapter, subClassDao, mUrls[INDEX_SUBCLASS]);
		...
		initPaginateListView(INDEX_SUBCLASS, mSubClassListView, mSubClassAdapter, new Game.GameCallback());
