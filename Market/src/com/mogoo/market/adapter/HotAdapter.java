...
import com.mogoo.market.adapter.HotCursorAdapter.DownClickListener;
 ...
 */
public class HotAdapter<T> extends ArrayAdapter<T> {
	...
	public HotAdapter(Context context, List<T> list) {
	...
	public View getView(int position, View convertView, ViewGroup parent) {
		...
		final HotApp appInfo = (HotApp) item;
		...
		mHandler.post(new Runnable() {
			...
			@Override
			public void run() {
				mImageManager.download(appInfo.getIconUrl(), tempHolder.app_iv,
...
//System.out.println(progress);
	...
	private long onDownApp(HotApp appInfo) {
		...
		// 判断sdcard,不存在提示用户，并返回－1
		if(!AppUtils.isSDCardReady()){
			String text = mContext.getResources().getString(R.string.no_sdcard_toast);
