...
public class ScrollingTabsView extends HorizontalScrollView implements OnPageChangeListener {
	...
	private static final String TAG = "com.mogoo.market.viewpager.extensions";
	...
	public ScrollingTabsView(Context context, AttributeSet attrs, int defStyle) {
		...
		DisplayMetrics aDisplayMetrics = new DisplayMetrics();
		((Activity) mContext).getWindowManager().getDefaultDisplay().getMetrics(aDisplayMetrics);
		mChildViewWidth = aDisplayMetrics.widthPixels / 3;
	...
	private void initTabs() {
		...
		for (int i = 0; i < mPager.getAdapter().getCount(); i++) {
			...
			mTabs.add(tab);
			// 将子视图平均等分为屏幕的1/3.
			LinearLayout.LayoutParams parms = (LinearLayout.LayoutParams) tab.getLayoutParams();
			parms.width = mChildViewWidth;
			tab.setLayoutParams(parms);
	...
	private void selectTab(int position) {
		...
		// 如果按钮之间添加了分割线视图，那么
		// 循环条件为for (int i = 0, pos = 0; i < mContainer.getChildCount(); i += 2, pos++).
		for (int i = 0, pos = 0; i < mContainer.getChildCount(); i += 1, pos++) {
			View tab = mContainer.getChildAt(i);
			tab.setSelected(pos == position);
		}
		
		// 如果按钮之间添加了分割线视图，那么
		// 当前选中视图View selectedTab = mContainer.getChildAt(position * 2).
