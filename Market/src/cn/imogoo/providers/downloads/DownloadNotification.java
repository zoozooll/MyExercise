...
package cn.imogoo.providers.downloads;
...
class DownloadNotification {
    ...
    private void updateActiveNotification(Collection<DownloadInfo> downloads) {
        ...
        for (DownloadInfo download : downloads) {
            ...
            if (title == null || title.length() == 0) {
            	// add by fdl
            	int titleid = Helpers.getResourseIdByName(mContext.getPackageName(), "string", "download_unknown_title");
            ...
            if (download.mStatus == Downloads.Impl.STATUS_QUEUED_FOR_WIFI
            	...
            	int strId = Helpers.getResourseIdByName(mContext.getPackageName(), "string", "notification_need_wifi_for_size");
        ...
        for (NotificationItem item : mNotifications.values()) {
            ...
            int status_bar_ongoing_event_progress_bar 
            	= Helpers.getResourseIdByName(mContext.getPackageName(), "layout", "status_bar_ongoing_event_progress_bar");
            // Build the RemoteView object
            //RemoteViews expandedView = new RemoteViews(
            //        "cn.imogoo.providers.downloads",
            ...
            if (item.mTitleCount > 1) {
            	int notification_filename_separator 
            		= Helpers.getResourseIdByName(mContext.getPackageName(), "string", "notification_filename_separator");
                //title.append(mContext.getString(R.string.notification_filename_separator));
                title.append(mContext.getString(notification_filename_separator));
                title.append(item.mTitles[1]);
                n.number = item.mTitleCount;
                if (item.mTitleCount > 2) {
                	int notification_filename_extras 
            			= Helpers.getResourseIdByName(mContext.getPackageName(), "string", "notification_filename_extras");
                    //title.append(mContext.getString(R.string.notification_filename_extras,
                    //        new Object[] { Integer.valueOf(item.mTitleCount - 2) }));
                	title.append(mContext.getString(notification_filename_extras,
                            new Object[] { Integer.valueOf(item.mTitleCount - 2) }));
                }
            } else {
            	int description 
    				= Helpers.getResourseIdByName(mContext.getPackageName(), "id", "description");
                //expandedView.setTextViewText(R.id.description,
                //        item.mDescription);
            	expandedView.setTextViewText(description,
                        item.mDescription);
            }
            int titleId 
				= Helpers.getResourseIdByName(mContext.getPackageName(), "id", "title");
            //expandedView.setTextViewText(R.id.title, title);
            expandedView.setTextViewText(titleId, title);

			int progress_bar = Helpers.getResourseIdByName(
					mContext.getPackageName(), "id", "progress_bar");
			int paused_text = Helpers.getResourseIdByName(
					mContext.getPackageName(), "id", "paused_text");
            if (hasPausedText) {
                //expandedView.setViewVisibility(R.id.progress_bar, View.GONE);
                //expandedView.setTextViewText(R.id.paused_text, item.mPausedText);
            	expandedView.setViewVisibility(progress_bar, View.GONE);
                expandedView.setTextViewText(paused_text, item.mPausedText);
                ...
                //        item.mTotalTotal == -1);
                expandedView.setViewVisibility(paused_text, View.GONE);
                expandedView.setProgressBar(progress_bar,
                        (int) item.mTotalTotal,
                        (int) item.mTotalCurrent,
                        item.mTotalTotal == -1);
            }
            int progress_text = Helpers.getResourseIdByName(
					mContext.getPackageName(), "id", "progress_text");
            int appIcon = Helpers.getResourseIdByName(
					mContext.getPackageName(), "id", "appIcon");
            ...
            expandedView.setTextViewText(progress_text,
                    getDownloadingText(item.mTotalTotal, item.mTotalCurrent));
            expandedView.setImageViewResource(appIcon, iconResource);
            
            n.contentView = expandedView;

            Intent intent = new Intent(Constants.ACTION_LIST);
            intent.setClassName("cn.imogoo.providers.downloads",
    ...
    private void updateCompletedNotification(Collection<DownloadInfo> downloads) {
        for (DownloadInfo download : downloads) {
            ...
            String title = download.mTitle;
            if (title == null || title.length() == 0) {
            	int download_unknown_title = Helpers.getResourseIdByName(
    					mContext.getPackageName(), "string", "download_unknown_title");
                //title = mContext.getResources().getString(
                //        R.string.download_unknown_title);
            	title = mContext.getResources().getString(
            ...
            if (Downloads.Impl.isStatusError(download.mStatus)) {
            	int notification_download_failed = Helpers.getResourseIdByName(
    					mContext.getPackageName(), "string", "notification_download_failed");
                //caption = mContext.getResources()
                //        .getString(R.string.notification_download_failed);
            	caption = mContext.getResources()
                        .getString(notification_download_failed);
                intent = new Intent(Constants.ACTION_LIST);
            } else {
            	int notification_download_complete = Helpers.getResourseIdByName(
    					mContext.getPackageName(), "string", "notification_download_complete");
                //caption = mContext.getResources()
                //        .getString(R.string.notification_download_complete);
            	caption = mContext.getResources()
                        .getString(notification_download_complete);
