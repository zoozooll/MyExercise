...
/**
 * @Description: ManagerList 自定义tab页卡实现
...
public class PagerSlidingTabStrip extends HorizontalScrollView {
	...
	private int indicatorColor = 0xFF5CBA0F;
	...
	private int tabTextColor = 0xFF83868A;
	private int tabTextColorSelected = Color.WHITE;
	private Typeface tabTypeface = null;
	private int tabTypefaceStyle = Typeface.BOLD;

	private int lastScrollX = 0;

	private int tabBackgroundResId = R.drawable.selector_managerlist_tab;
	...
	private class PageListener implements OnPageChangeListener {
		...
		public void onPageSelected(int position) {
			for (int i = 0; i < tabCount; i++) {
				...
				if (v instanceof TextView) {
					if (i == position) {
						((TextView) v).setTextColor(tabTextColorSelected);// setTextColor(tabTextColorSelected);
					} else {
						((TextView) v).setTextColor(tabTextColor);// setTextColor(tabTextColorSelected);
					}
				}
			}
			invalidate();
