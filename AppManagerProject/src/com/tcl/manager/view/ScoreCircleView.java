...
public class ScoreCircleView extends View {
	...
	private void setDefaultValue() {
		...
		rimWidth = (int) r.getDimension(R.dimen.default_main_rim_width);
		descTextSize = (int) r.getDimension(R.dimen.default_main_desc_textsize);
		scoreTextSize = (int) r
				.getDimension(R.dimen.default_main_score_textsize);
		textColor = r.getColor(R.color.main_good);
	...
	protected void onDraw(Canvas canvas) {
		...
		if (isSpinning) {
			Shader s = new SweepGradient(
					circleBounds.width() / 2 + paddingLeft,
					circleBounds.height() / 2 + paddingTop, mRectColors, null);
			Matrix matrix = new Matrix();
			matrix.setRotate(-90 + progress, circleBounds.width() / 2
					+ paddingLeft, circleBounds.height() / 2 + paddingTop);
			s.setLocalMatrix(matrix);
			barPaint.setShader(s);
		...
		float scoreOffset = scoreTextPaint.measureText(String
				.valueOf((int)currentScore)) / 2;
		canvas.drawText(String.valueOf((int)currentScore), this.getWidth() / 2
				- scoreOffset, this.getHeight() / 2+scoreTextSize/4, scoreTextPaint);
		// 画描述文本
	...
	public boolean onTouchEvent(MotionEvent event) {
		...
		case MotionEvent.ACTION_DOWN:
			if (isInCircle(event.getX(), event.getY()) && this.isClickable()) {
				isInCircleDown = true;
				updateCircleColor(bgColorPress);
				return true;
			}
			break;
...
//						lastClickTime = currTime;
			...
			updateCircleColor(bgColorNormal);
			break;
		default:
			break;
		}
	...
	public void setScore(int score) {
		...
		setTextColor(ScoreLevel.resolveToColor(score));
		invalidate();
	}
	
	public void addScore(float score){
		currentScore+=score;
		setDescText(ScoreLevel.resolveToString((int)currentScore));
		setTextColor(ScoreLevel.resolveToColor((int)currentScore));
		invalidate();
	}
