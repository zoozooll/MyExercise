...
public class TCLThreadPool {
    ...
    public TCLThreadPool() {
        this("thread-pool", CORE_POOL_SIZE, MAX_POOL_SIZE);
    }

    
    public TCLThreadPool(String name, int coreSize, int maxSize) {
        if (coreSize <= 0)
            coreSize = 1;
        if (maxSize <= coreSize)
            maxSize = coreSize;
    ...
    public TCLThreadPool(String name, int coreSize, int maxSize, BlockingQueue<Runnable> queue) {
        if (coreSize <= 0)
            coreSize = 1;
        if (maxSize <= coreSize)
            maxSize = coreSize;
    ...
    public <T> TCLFuture<T> submit(Job<T> job, FutureListener<T> listener, Priority priority) {
    ...
    public <T> TCLFuture<T> submit(Job<T> job, FutureListener<T> listener) {
        return submit(job, listener, Priority.NORMAL);
    }

    
    public <T> TCLFuture<T> submit(Job<T> job, Priority priority) {
        return submit(job, null, priority);
    }

    
    public <T> TCLFuture<T> submit(Job<T> job) {
    ...
    private class Worker<T> implements Runnable, TCLFuture<T>, JobContext {
        ...
        public void run() {
            ...
            if (setMode(MODE_CPU)) {
                try {
                	...
                	NLog.w(TAG, "Exception in running a job", ex);
                }
            }
        ...
        public synchronized T get() {
            while (!mIsDone) {
                try {
                	...
                	NLog.w(TAG, "ignore exception", ex);
                    // ignore.
                }
            }
            return mResult;
    ...
    public static TCLThreadPool getInstance() {
        return InstanceHolder.INSTANCE;
    }

    private static class InstanceHolder {
        public static final TCLThreadPool INSTANCE = new TCLThreadPool();
    }

    
    public static void runOnNonUIThread(final Runnable runnable) {
        if (Thread.currentThread() == Looper.getMainLooper().getThread()) {
            TCLThreadPool.getInstance().submit(new TCLThreadPool.Job<Object>() {
                @Override
                public Object run(TCLThreadPool.JobContext jc) {
                    runnable.run();
                    return null;
                }
            });
