 ...
 * @hide
 */
public class NetworkStats implements Cloneable {
    ...
    public static class Entry {
		...
		public int hashCode() {
			...
			result = prime * result + (int) (operations ^ (operations >>> 32));
    ...
    public int findIndex(String iface, int uid, int set, int tag) {
        for (int i = 0; i < size; i++) {
            if (uid == this.uid[i] && set == this.set[i] && tag == this.tag[i]
                    && (iface != null && iface.equals(this.iface[i]))) {
                return i;
            }
        }
        return -1;
