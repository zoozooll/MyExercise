...
class Shows extends Controller{
	...
	function add_view(){
		...
		$this->load->view('backend/shows/shows_add');
	}
	function edit_view(){
		...
		$data['data'] = $this->shows->shows_data($shows_vo);
		$data['type'] = type();
		$this->load->view('backend/header');
		$this->load->view('backend/shows/shows_edit', $data);		
	}
	function all_view(){
		$this->load->model('backend/shows_model', 'shows', TRUE);
		$config['base_url'] = base_url() . './index.php/backend/shows/all_view';
		...
		$data['href'] = site_url('backend/shows/all_view');
		$data['page'] = $this->pagination->create_links();
		$this->load->view('backend/header');
		$this->load->view('backend/shows/shows_all', $data);
	}
	function add(){
		$img = array();
		$this->load->model('backend/shows_model', 'shows', TRUE);
		...
		$data['content'] = $this->shows->add($shows_vo) ? 'Upload Success' : 'Faild';
		$data['href'] = site_url('backend/shows/all_view');
	...
	function del(){
		...
		$this->shows->delete_data($shows_vo);
		href($_SERVER['HTTP_REFERER']);
	}
	function edit(){
		$this->load->model('backend/shows_model', 'shows', TRUE);
		$id = $this->input->post('id');
		$data = $this->shows->shows_data(array('id' => $id));
		$config['upload_path'] = './system/images/backend/uploads';
		$config['allowed_types'] = 'gif|jpg|png';
		$config['max_size'] = '2048';
		$config['max_width'] = '1024';
		...
		$data['content'] = $this->shows->edit($shows_vo) ? 'Update Success' : 'Update Faild';
		$data['href'] = site_url('backend/shows/all_view');
		$this->load->view('backend/success', $data);
	}
