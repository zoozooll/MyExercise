...
class SmtpMail {
	...
	public function smtp($relay_host = "", $smtp_port = 25, $auth = false, $user, $pass) {
		...
		$this->log_file = __ROOT_LOGS_PATH."#sendmail.log";
		$this->sock = FALSE;
	}
	...
	public function sendmail($to, $sendfrom, $subject = "", $body = "", $mailtype, $from, $additional_headers = "gb2312", $cc = "", $bcc = "") {
	...
	#
		...
		if (!$this->smtp_putcmd("RCPT", "TO:<".$to.">")) {
			return $this->smtp_error("sendingRCPT TO command");
		}
	...
	public static function sendSimpleMail($tomail, $frommail, $content, $subject, $charcode = 'iso-8859-1'){
		if(empty($tomail) || empty($frommail)) return false;
		// To send HTML mail, the Content-type header must be set
		$headers  = 'MIME-Version: 1.0' . "\r\n";
		$headers .= 'Content-type: text/html; charset='.$charcode."\r\n";
		...
		$sent = true;
		// Mail it
		$arrTomail = explode(';', $tomail);
		$num = count($arrTomail);
		for($i = 0; $i<$num; $i++) {
			if(empty($arrTomail[$i])) continue;
			$sent = mail($arrTomail[$i], $subject, $content, $headers);
		}
		return $sent;
	}
