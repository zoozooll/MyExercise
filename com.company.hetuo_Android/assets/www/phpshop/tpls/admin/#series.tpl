...
function MP_jumpMenu(val) {
	location.href = "admincp.php?mid={$mid}&lid={$lid}&bid="+val;
}
</script>
</head>
<body>
...
<div id="BodyContent">
	<div id="divMainBodyContent">
	    ...
	    <div id="hiddenselect">   <select name="bid"  class="txInput" id="bid" onChange="MP_jumpMenu(this.value)">
				  ...
				  {$loop brand}
				  <option value="{$brand.loop.id}"{if brand.loop.id==bid} selected{/if}>
				  {$brand.loop.name}</option>
				  {/loop}
				</select>
		</div>
		<div class="cls"></div>
	...
	<div id="divMainBodyContent">
	    ...
	    <div> 中文：<input name="name[]" type="text" value="{$value.loop.name}"> 英文：<input name="enname[]" type="text" value="{$value.loop.enname}"> 
				   ...
				   <input name="id[]" type="checkbox" value="{$value.loop.id}"> 修改 <a onClick="return delYesOrNo()" title="删除" href="admincp.php?mid={$mid}&lid={$lid}&delsid={$value.loop.id}&act=del&bid={$bid}" target="_self">删除</a>
		</div>
		<div class="cls"></div>
    </div>
	{/loop}
