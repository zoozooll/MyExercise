...
if (empty($action)) {
	...
	InitGP(array('step'));
	InitGP(array('groups','groupid','author','keyword','postdate1','postdate2','hits','replies','tcounts','counts','orderby','sc','perpage','direct','page'));
	...
	if (empty($step)) {
		...
		while (@extract($db->fetch_array($query))) {
			$num++;
			$htm_tr = $num % 4 == 0 ? '' : '';
			$creategroup .= "<li><input type=\"checkbox\" name=\"groups[]\" value=\"$gid\">$grouptitle</li>$htm_tr";
		}
		
		$num++;
		$htm_tr = $num % 4 == 0 ? '' : '';
		$creategroup .= "<li><input type=\"checkbox\" name=\"groups[]\" value=\"-1\">普通会员组</li>$htm_tr";
		$creategroup && $creategroup = "<ul class=\"list_A list_120 cc\">$creategroup</ul>";
		require_once PrintApp('diary');
