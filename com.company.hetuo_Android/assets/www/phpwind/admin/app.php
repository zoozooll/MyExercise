...
if ($admintype == 'appset') {
	...
	require_once(D_P.'api/class_Other.php');
	$api = new Other();
	$personal = $api->appver;
	/*Personal version check*/
	...
	foreach($files as $value){
		...
		if (strpos($value,'./pw_api.php') !== false || strpos($value,'api/') !== false || strpos($value,'mode/o/m_app.php') !== false || strpos($value,'mode/o/m_myapp.php') !== false || strpos($value,'mode/o/template/m_app.htm') !== false || strpos($value,'mode/o/template/m_myapp.htm') !== false || strpos($value,'./apps.php') !== false || strpos($value,'template/wind/apps.htm') !== false) {
	...
	if (empty($db_appifopen)) {
		$bbs_checked = $db_appbbs ? 'checked' : '';
		$o_checked = $db_appo ? 'checked' : '';
	}
...
} elseif ($admintype == 'open_bbs') {

	setConfig('db_appbbs', 1);
	updatecache_c();

	adminmsg('operate_success',"$basename&admintype=onlineapp");

} elseif ($admintype == 'close_bbs') {
