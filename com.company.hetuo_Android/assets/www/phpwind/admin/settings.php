...
//		!is_array($db_bbstitle) && $db_bbstitle = array('index' => $db_bbstitle,'other' => '');
	...
	if ($admintype == 'safe' || $settingdb['safe']) {
		...
		$default_stopicdir = $config['htmdir'].'/stopic';
		if (!$config['stopicdir']) {
			if (!is_dir($default_stopicdir)) @mkdir(R_P.$default_stopicdir);
			if (is_dir($default_stopicdir)) {
				$config['stopicdir'] = $default_stopicdir;
			} else {
				adminmsg('settings_stopicdir_dfnotfind');
			}
		} elseif (!is_dir(R_P.$config['stopicdir']) && $db_stopicdir!=$config['stopicdir']) {
			!preg_match('/^[a-zA-Z0-9\_]+$/i',$config['stopicdir']) && adminmsg('settings_stopicdir_error');
			if (!$db_stopicdir || !is_dir(R_P.$db_stopicdir)) {
				if (is_dir($default_stopicdir)) {
					$db_stopicdir = $default_stopicdir;
				} else {
					adminmsg('settings_stopicdir_dfnotfind');
				}
			}
			if (!rename(R_P.$db_stopicdir,R_P.$config['stopicdir'])) {
				$config['stopicdir'] = $db_stopicdir;
			}
		}
	...
	if ($admintype=='credit' || $settingdb['credit']) {
		require_once(R_P.'require/credit.php');
		InitGP(array('creditpay','creditset','credit_name','credit_unit','credit_desc','cdiy_name','cdiy_unit','cdiy_desc','cname1','cname2'),'P');
