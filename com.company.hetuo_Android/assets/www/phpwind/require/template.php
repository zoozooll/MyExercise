	...
	if ($read['groupid'] != '' && $anonymous == 0) {
		...
		if ($read['ifsign'] == 1 || $read['ifsign'] == 3) {
			...
			if (!$sign[$read['author']]) {
				...
				if (strpos($db_signgroup,",$read[groupid],") !== false && $db_signmoney && (!getstatus($read['userstatus'],10) ||  (!$read['starttime'] || $read['currency'] < ($tdtime-$read['starttime'])/86400*$db_signmoney))) {
					$read['signature'] = '';
				} else {
