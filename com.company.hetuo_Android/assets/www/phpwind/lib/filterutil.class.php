...
class PW_FilterUtil {
	...
	function PW_FilterUtil($file = array()) {
		if ($file) {
			...
			$this->dict_bin_path = $this->dict_dir . 'dict_all.php';
	...
	* @desc 返回唯一实例,本例为单件
    ...
    function convertCode($content, $fcode = 'CHST', $tcode = 'CHSS', $dict_dir = null) {
        ...
        $ch = new Chinese($fcode, $tcode, $dict_dir, true);
...
class Trie {
    ...
    function getTree($words) {
        ...
        foreach ($words as $word) {
            ...
            $this->nodes[$cur][2] = trim($weight[1]); //将权重放在叶子节点
        }
        return $this->nodes;
    }
    ...
    function match($s) {
        global $db_charset;
        $utf8_charset = array('utf8', 'UTF8', 'utf-8', 'UTF-8');
        $ret = array();
        $cur = 0; //当前节点，初始为根节点
        $i = 0; //字符串当前偏移
        $p = 0; //字符串回溯位置
        ...
        while($i < $len) {
            ...
            if (isset($this->nodes[$cur][1][$c])) { //如果存在
                ...
                if (!in_array($db_charset, $utf8_charset)) {
                    $j = $i+1;
                    $d =ord($s[$j]);
                    if ($c > 127 && $d > 127) {
                        $cur = 0; //重置当前节点为根节点
