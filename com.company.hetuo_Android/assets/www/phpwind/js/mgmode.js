...
MgMode.prototype = {
	...
	forder : function() {
		...
		this.init = function() {
			...
			read.menu.innerHTML = '<div id="mgmode"><div class="menu-post"><div class="menu-b"><table width="280" cellspacing="0" cellpadding="0"><tr><th class="h">版块顺序设置</th></tr><tr class="tr3"><td>分类顺序可通过分类右边的↑↓箭头进行调整<br/>版块可以直接拖拽进行操作</td></tr><tr class="f_one"><th class="pdD tac"><input onclick="mf.submit();" type="button" class="btn" value="保存" /> <input onclick="mf.cancle(true);" type="button" class="bt" value="返回" /></th></tr></table></div></div></div>';
		}
		...
		this.setpz = function(o) {
			if (o.id.match(/^t_\d+$/ig)) {
				var th = o.getElementsByTagName('th')[0];
		...
		this.removeicon = function(o) {
			if (o.id.match(/^t_\d+$/ig)) {
				var s = o.getElementsByTagName('th')[0].childNodes;
				for (var i = s.length - 1; i >= 0 ; i--) {
					if (s[i].nodeType == 1 && s[i].tagName.toLowerCase() == 'a' && s[i].className == 'fr extra') {
		...
		this.submit = function() {
			...
			for (var i = 0; i < tbodys.length; i++) {
				if (tbodys[i].id.match(/^cate_\d+$/ig)) {
					var fid = '';
					var gtag = tbodys[i].className == 'across' ? 'td' : 'tr';
	...
	fsetname : function() {
		...
		this.init = function() {
			...
			read.menu.innerHTML = '<div id="mgmode"><div class="menu-post"><div class="menu-b"><table width="290" cellspacing="0" cellpadding="0"><tr><th class="h">版块名称修改</th></tr><tr class="tr3"><td>点击版块标题即可修改</td></tr><tr class="f_one"><th class="pdD tac"><input onclick="mf.submit();" type="button" class="btn" value="保存" /> <input onclick="mf.cancle(true);" type="button" class="bt" value="返回" /></th></tr></table></div></div></div>';
		}
