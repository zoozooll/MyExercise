...
import com.oregonscientific.meep.BuildConfig;
 ...
 ******************************************************************************
 * 
 * This class holds bitmap caches (memory and disk).
 */
public class ImageCache {
	...
	/**
	 * Retrieve in-sample size of the bitmap from the given requirements
	 * 
	 ...
	 */
	private int getSampleSizeFromDiskCache(String data, int maxWidth, int maxHeight) {
		final String key = hashKeyForDisk(data);
		synchronized (mDiskCacheLock) {
				...
				try {
					...
					if (snapshot != null) {
						...
						if (inputStream != null) {
						    ...
						    final int sampleSize = calculateInSampleSize(options, maxWidth, maxHeight);
						    return sampleSize;
							} catch (Exception ex) {
								return 1;
							}
					...
					Log.e(TAG, "getSampleSizeFromDiskCache - " + e);
				} finally {
					try {
						if (inputStream != null) {
							inputStream.close();
	 ...
	 * @return The bitmap if found in cache, null otherwise
	 */
	public Bitmap getBitmapFromDiskCache(String data, int maxWidth, int maxHeight) {
		final String key = data == null ? null : hashKeyForDisk(data);
		synchronized (mDiskCacheLock) {
			while (mDiskCacheStarting) {
				try {
					mDiskCacheLock.wait();
			...
			if (mDiskLruCache != null && key != null) {
				...
				try {
					...
					if (snapshot != null) {
						...
						if (inputStream != null) {
							try {
								// Obtain in-sample size
								final BitmapFactory.Options options = new BitmapFactory.Options();
								options.inSampleSize = getSampleSizeFromDiskCache(data, maxWidth, maxHeight);
	...
	public static File getDiskCacheDir(Context context, String uniqueName) {
		...
		final String cachePath = (Environment.MEDIA_MOUNTED.equals(Environment.getExternalStorageState()) 
				|| !isExternalStorageRemovable()) && hasPermissionToWriteToExternalStorage(context) && !isSystemProcess()
	...
	/**
	 * Determines whether or not the running process is running in the system process space
	 * @return {@code true} if the current process is running in the system process space, {@code false} otherwise
	 */
	...
	/**
	 * Determines whether the calling process of an IPC or you have permission
	 * to write to external storage
	 * 
	 * @return {@code true} if the user is granted {@link android.Manifest.permission#WRITE_EXTERNAL_STORAGE}, {@code false} otherwise
	 */
	public static boolean hasPermissionToWriteToExternalStorage(Context context) {
