...
public class DetectorFactory {
    ...
    /**
     * Load profiles from the path(s) specified
     * 
     * @param am The bridge to access an application's raw asset files on Android
     * @param profiles an array of profile file paths
     ...
     */
    public static void loadProfiles(AssetManager am, String path) throws LangDetectException {
    	if (am == null || path == null) {
    		throw new LangDetectException(ErrorCode.NeedLoadProfileError, "No profile found: " + path);
    	}
    	
    	try {
    		String[] files = am.list(path);
    		int langSize = files.length;
    		
    		for (int i = 0; i < langSize; i++) {
    			File file = new File(path, files[i]);
    			InputStream is = am.open(file.getPath());
    			LangProfile profile = JSON.decode(is, LangProfile.class);
    			addProfile(profile, i, langSize);
    		}
    	} catch (Exception ex) {
    		throw new LangDetectException(ErrorCode.FileLoadError, "Failed to load profiles at " + path);
    	}
    }
