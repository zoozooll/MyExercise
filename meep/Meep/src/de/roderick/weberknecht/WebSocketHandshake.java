...
{	
	...
	{
		if (headers == null) {
			throw new WebSocketException("connection failed: missing header");
		}
		
		String upgrade = headers.get("Upgrade");
		upgrade = upgrade == null ? headers.get("upgrade") : upgrade;
		if (upgrade == null || !upgrade.toLowerCase(Locale.ENGLISH).equalsIgnoreCase("websocket")) {
			throw new WebSocketException("connection failed: missing header field in server handshake: Upgrade");
		}
		
		String connection = headers.get("Connection");
		connection = connection == null ? headers.get("connection") : connection;
		if (connection == null || !connection.toLowerCase(Locale.ENGLISH).equalsIgnoreCase("upgrade")) {
			throw new WebSocketException("connection failed: missing header field in server handshake: Connection");
		}
		
		String origin = headers.get("Sec-WebSocket-Origin");
		origin = origin == null ? headers.get("sec-websocket-origin") : origin;
		if (origin == null || !origin.toLowerCase(Locale.ENGLISH).equalsIgnoreCase(origin)) {
