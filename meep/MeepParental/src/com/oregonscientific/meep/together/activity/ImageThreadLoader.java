			...
			synchronized(this) {
				while(Queue.size() > 0) {
					...
					if( Cache.containsKey(item.url.toString()) && Cache.get(item.url.toString()) != null) {
						Utils.printLogcatDebugMessage( "read image from cache");
...
//									}
						...
						Utils.printLogcatDebugMessage(TAG, "icon download");
						final Bitmap bmp = readBitmapFromNetwork(item.url);
						if( bmp != null ) {
	...
	public Bitmap loadImage( final String uri, final ImageLoadedListener listener) throws MalformedURLException {
		...
		if( Cache.containsKey(uri)) {
			...
			{
				Utils.printLogcatDebugMessage(TAG, "icon ref image is null");
			}
		}
		else
		{
			Utils.printLogcatDebugMessage(TAG, "icon uri:" + uri.toString());
		}
		...
		if( thread.getState() == State.NEW) {
			Utils.printLogcatDebugMessage( "request image thread state new");
			thread.start();
		} else if( thread.getState() == State.TERMINATED) {
			Utils.printLogcatDebugMessage( "request image thread state TERMINATED");
			thread = new Thread(runner);
			thread.start();
		}
		else
		{
			Utils.printLogcatDebugMessage( "request image thread state not handle : " + thread.getState());
		}
		return null;
	}
