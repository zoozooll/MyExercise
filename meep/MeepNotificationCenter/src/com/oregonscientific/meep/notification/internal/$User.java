...
class $User extends Model<$User, Long> implements Identity<Long> {
	...
	@Omit
	private ForeignCollection<$News> notifications;
	...
	public ForeignCollection<$News> getNotifications() {
		return notifications;
	}
	
	/**
	 * Returns the {@link $News} identified by {@code id}  
	 * 
	 * @param id the identifier of the {@link $News}
	 * @return the {@link $News} if found, {@code null} otherwise
	 */
	public $News getNews(String id) {
		$News result = null;
		
		// Since all foreign collections are lazy, no need to refresh
		CloseableIterator<$News> iterator = notifications.closeableIterator();
		try {
			while (iterator.hasNext()) {
				$News news = iterator.next();
				if (news.getIdentifier().equals(id)) {
					result = news;
					break;
				}
			}
