			...
			synchronized(this) {
				while(Queue.size() > 0) {
					...
					if( Cache.containsKey(item.url.toString()) && Cache.get(item.url.toString()) != null) {
						Log.d("youtube testing", "youtube read image from cache");
...
//									}
						...
						Log.d("youtube testing", "youtube read image from network");
						final Bitmap bmp = readBitmapFromNetwork(item.url);
						if( bmp != null ) {
							...
							handler.post(new Runnable() {
								public void run() {
									...
									{
										Log.d("youtube testing", "youtube cannot get image from network");
	...
	public Bitmap loadImage( final String uri, final ImageLoadedListener listener) throws MalformedURLException {
		...
		if( Cache.containsKey(uri)) {
			...
			{
				Log.d("youtube testing", "youtube ref image is null");
			}
		}
		else
		{
			Log.d("youtube testing", "youtube uri:" + uri.toString());
		}
		...
		if( thread.getState() == State.NEW) {
			Log.d("youtube testing", "youtube request image thread state new");
			thread.start();
		} else if( thread.getState() == State.TERMINATED) {
			Log.d("youtube testing", "youtube request image thread state TERMINATED");
			thread = new Thread(runner);
			thread.start();
		}
		else
		{
			Log.d("youtube testing", "youtube request image thread state not handle : " + thread.getState());
		}
		return null;
	}
