...
//		cacheParams.memoryCacheEnabled = false;	// Disable memory cache
	...
	public void download(String url, int maxWidth, int maxHeight, ImageView imageView) {
		...
		if (bitmap == null) {
			forceDownload(url, maxWidth, maxHeight, imageView);
		} else {
	 ...
	 */
	private void forceDownload(String url, int maxWidth, int maxHeight, ImageView imageView) {
	...
	class BitmapDownloaderTask extends AsyncTask<String, Void, Bitmap> {
		...
		protected Bitmap doInBackground(String... params) {
			...
			if (params.length == 3) {
				url = params[0];
				maxWidth = Integer.parseInt(params[1]);
				maxHeight = Integer.parseInt(params[2]);
