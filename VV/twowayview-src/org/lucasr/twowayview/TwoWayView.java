 ...
 */
/**
 * A view that shows items in a vertical or horizontal scrolling list. The items come from the
 * {@link ListAdapter} associated with this view.
 */
public class TwoWayView extends AdapterView<ListAdapter> implements
	 ...
	 * @param l The recycler listener to be notified of views set aside in the recycler.
	 * @see TwoWayView.RecycleBin
	 ...
	 * mode has been set to {@link ChoiceMode#SINGLE} or {@link ChoiceMode#MULTIPLE}.
	 ...
	 * not been set to {@link ChoiceMode#NONE}.
	 ...
	 * been set to {@link ChoiceMode#NONE} and the adapter has stable IDs. (
	 ...
	 * been set to {@link ChoiceMode#SINGLE} or {@link ChoiceMode#MULTIPLE}.
	...
	public void setAdapter(ListAdapter adapter) {
		...
		if (mAdapter != null) {
			...
			if (mChoiceMode != ChoiceMode.NONE && mHasStableIds
	...
	protected float getTopFadingEdgeStrength() {
		...
		if (childCount == 0) {
			...
			final float length = getVerticalFadingEdgeLength();
			return (top < paddingTop ? -(top - paddingTop) / length
					: fadingEdge);
		}
	}
	@Override
	...
	protected float getLeftFadingEdgeStrength() {
		...
		if (childCount == 0) {
			...
			final float length = getHorizontalFadingEdgeLength();
			return (left < paddingLeft ? -(left - paddingLeft) / length
					: fadingEdge);
		}
	}
	@Override
				...
				switch (mTouchMode) {
					...
					case TOUCH_MODE_DRAGGING:
						...
						if (Math.abs(velocity) >= mFlingVelocity) {
							...
							mScroller.fling(0, 0, (int) (mIsVertical ? 0
									: velocity), (int) (mIsVertical ? velocity
									: 0),
	 ...
	 *            {@link View#FOCUS_LEFT} or {@link View#FOCUS_RIGHT} depending on the current view
	 *            orientation.
	...
	/**
	 * To avoid horizontal/vertical focus searches changing the selected item, we manually focus
	 * search within the selected item (as applicable), and prevent focus from jumping to something
	 * within another item.
	 * @param direction either {@link View#FOCUS_UP} or {@link View#FOCUS_DOWN} or
	 *            {@link View#FOCUS_LEFT} or {@link View#FOCUS_RIGHT} depending on the current view
	 *            orientation.
	...
	/**
	 * When selection changes, it is possible that the previously selected or the next selected item
	 * will change its size. If so, we need to offset some folks, and re-layout the items as
	 * appropriate.
	 * @param selectedView The currently selected view (before changing selection). should be
	 *            <code>null</code> if there was no previous selection.
	 * @param direction either {@link View#FOCUS_UP} or {@link View#FOCUS_DOWN} or
	 *            {@link View#FOCUS_LEFT} or {@link View#FOCUS_RIGHT} depending on the current view
	 *            orientation.
	 * @param newSelectedPosition The position of the next selection.
	 * @param newFocusAssigned whether new focus was assigned. This matters because when something
	 *            has focus, we don't want to show selection (ugh).
	 */
	...
	/**
	 * Re-measure a child, and if its height changes, lay it out preserving its top, and adjust the
	 * children below it appropriately.
	...
	/**
	 * Do an arrow scroll based on focus searching. If a new view is given focus, return the
	 * selection delta and amount to scroll via an {@link ArrowScrollFocusResult}, otherwise, return
	 * null.
	 * @param direction either {@link View#FOCUS_UP} or {@link View#FOCUS_DOWN} or
	 *            {@link View#FOCUS_LEFT} or {@link View#FOCUS_RIGHT} depending on the current view
	 *            orientation.
		...
		if (selectedView != null && selectedView.hasFocus()) {
			...
			if (direction == View.FOCUS_DOWN || direction == View.FOCUS_RIGHT) {
				...
				final int start = getStartEdge()
						+ (fadingEdgeShowing ? getArrowScrollPreviewLength()
								: 0);
				...
				final boolean fadingEdgeShowing = (mFirstPosition
	...
	private int getArrowScrollPreviewLength() {
		return mItemMargin
				+ Math.max(MIN_SCROLL_PREVIEW_PIXELS, getFadingEdgeLength());
	}
	/**
	...
	/**
	 * Determine how much we need to scroll in order to get the next selected view visible. The
	 * amount is capped at {@link #getMaxScrollAmount()}.
	 * @param direction either {@link View#FOCUS_UP} or {@link View#FOCUS_DOWN} or
	 *            {@link View#FOCUS_LEFT} or {@link View#FOCUS_RIGHT} depending on the current view
	 *            orientation.
	 * @param nextSelectedPosition The position of the next selection, or {@link #INVALID_POSITION}
	 *            if there is no next selectable position
	 * @return The amount to scroll. Note: this is always positive! Direction needs to be taken into
	 *         account when actually scrolling.
	 */
		...
		if (action != KeyEvent.ACTION_UP) {
			switch (keyCode) {
				...
				case KeyEvent.KEYCODE_DPAD_RIGHT:
					if (!mIsVertical) {
						handled = handleKeyScroll(event, count,
								View.FOCUS_RIGHT);
					} else if (KeyEventCompat.hasNoModifiers(event)) {
			...
			case KeyEvent.ACTION_UP:
				...
				if (isClickable() && isPressed() && mSelectedPosition >= 0
					...
					final View child = getChildAt(mSelectedPosition
							- mFirstPosition);
					if (child != null) {
						performItemClick(child, mSelectedPosition,
								mSelectedRowId);
						child.setPressed(false);
					}
					setPressed(false);
	...
	private int getChildMeasuredSize(View child) {
		return (mIsVertical ? child.getMeasuredHeight() : child
				.getMeasuredWidth());
	}
	private int getFadingEdgeLength() {
		return (mIsVertical ? getVerticalFadingEdgeLength()
				: getHorizontalFadingEdgeLength());
	}
	private int getMinSelectionPixel(int start, int fadingEdgeLength,
	...
	private boolean contentFits() {
		...
		return (getChildStartEdge(first) >= getStartEdge() && getChildEndEdge(last) <= getEndEdge());
	}
	 ...
	 *            {@link View#FOCUS_LEFT} or {@link View#FOCUS_RIGHT} depending on the current view
	 *            orientation.
	 * @return The position of the next selectable position of the views that are currently visible,
	 *         taking into account the fact that there might be no selection. Returns
	 *         {@link #INVALID_POSITION} if there is no selectable view on screen in the given
	 *         direction.
	 */
	...
	/**
	 * Smoothly scroll to the specified adapter position. The view will scroll such that the
	 * indicated position is displayed <code>offset</code> pixels from the top/left edge of the
	 * view, according to the orientation. If this is impossible, (e.g. the offset would scroll the
	 * first or last item beyond the boundaries of the list) it will get as close as possible. The
	 * scroll will take <code>duration</code> milliseconds to complete.
	 * @param position Position to scroll to
	 * @param offset Desired distance in pixels of <code>position</code> from the top/left of the
	...
	public void smoothScrollBy(int distance, int duration) {
		...
		if (distance == 0
				...
				|| childCount == 0
				|| (firstPosition == 0
						&& getChildStartEdge(getChildAt(0)) == start && distance < 0)
				|| (lastPosition == mItemCount
						&& getChildEndEdge(getChildAt(childCount - 1)) == end && distance > 0)) {
			finishSmoothScrolling();
		} else {
			mScroller.startScroll(0, 0, mIsVertical ? 0 : -distance,
					mIsVertical ? -distance : 0, duration);
			mLastTouchPos = 0;
			mTouchMode = TOUCH_MODE_FLINGING;
			reportScrollStateChange(OnScrollListener.SCROLL_STATE_FLING);
			ViewCompat.postInvalidateOnAnimation(this);
			...
			switch (mLayoutMode) {
				case LAYOUT_SET_SELECTION:
					if (newSelected != null) {
						...
						selected = fillFromSelection(newSelectedStart, start,
								end);
					} else {
	...
	private View moveSelection(View oldSelected, View newSelected, int delta,
		...
		final int minStart = getMinSelectionPixel(start, fadingEdgeLength,
				selectedPosition);
		final int maxEnd = getMaxSelectionPixel(end, fadingEdgeLength,
				selectedPosition);
		View selected = null;
		if (delta > 0) {
			/*
	...
	private void handleDataChanged() {
		if (mChoiceMode != ChoiceMode.NONE && mAdapter != null
				&& mAdapter.hasStableIds()) {
					...
					case SYNC_SELECTED_POSITION:
						if (isInTouchMode()) {
							...
							mSyncPosition = Math.min(
									Math.max(0, mSyncPosition), itemCount - 1);
							return;
						} else {
	...
	boolean resurrectSelection() {
		...
		if (toPosition >= firstPosition
			...
			if (selectedStart < start) {
				...
				selectedStart = end - getChildMeasuredSize(selected)
						- getFadingEdgeLength();
			}
	...
	/**
	 * Measures the height of the given range of children (inclusive) and returns the height with
	 * this TwoWayView's padding and item margin heights included. If maxHeight is provided, the
	 * measuring will stop when the current height reaches maxHeight.
	 * @param widthMeasureSpec The width measure spec to be given to a child's
	 *            {@link View#measure(int, int)}.
	 * @param startPosition The position of the first child to be shown.
	 * @param endPosition The (inclusive) position of the last child to be shown. Specify
	 *            {@link #NO_POSITION} if the last child should be the last available child from the
	 *            adapter.
	 * @param maxHeight The maximum height that will be returned (if all the children don't fit in
	 *            this value, this value will be returned).
	 * @param disallowPartialChildPosition In general, whether the returned height should only
	 *            contain entire children. This is more powerful--it is the first inclusive position
	 *            at which partial children will not be allowed. Example: it looks nice to have at
	 *            least 3 completely visible children, and in portrait this will most likely fit;
	 *            but in landscape there could be times when even 2 children can not be completely
	 *            shown, so a value of 2 (remember, inclusive) would be good (assuming startPosition
	 *            is 0).
	...
	/**
	 * Measures the width of the given range of children (inclusive) and returns the width with this
	 * TwoWayView's padding and item margin widths included. If maxWidth is provided, the measuring
	 * will stop when the current width reaches maxWidth.
	 ...
	 *            {@link #NO_POSITION} if the last child should be the last available child from the
	 *            adapter.
	 * @param maxWidth The maximum width that will be returned (if all the children don't fit in
	 *            this value, this value will be returned).
	 * @param disallowPartialChildPosition In general, whether the returned width should only
	 *            contain entire children. This is more powerful--it is the first inclusive position
	 *            at which partial children will not be allowed. Example: it looks nice to have at
	 *            least 3 completely visible children, and in portrait this will most likely fit;
	 *            but in landscape there could be times when even 2 children can not be completely
	...
	private View fillFromSelection(int selectedTop, int start, int end) {
		...
		final int maxEnd = getMaxSelectionPixel(end, fadingEdgeLength,
				selectedPosition);
		View selected = makeAndAddView(selectedPosition, selectedTop, true,
				true);
		final int selectedStart = getChildStartEdge(selected);
		final int selectedEnd = getChildEndEdge(selected);
		// Some of the newly selected item extends below the bottom of the list
	...
	public static class LayoutParams extends ViewGroup.LayoutParams {
		...
		/**
		 * When a TwoWayView is measured with an AT_MOST measure spec, it needs to obtain children
		...
		public boolean performAccessibilityAction(View host, int action,
			...
			switch (action) {
				...
				case AccessibilityNodeInfoCompat.ACTION_CLICK:
					return isClickable()
							&& performItemClick(host, position, id);
				case AccessibilityNodeInfoCompat.ACTION_LONG_CLICK:
					return isLongClickable()
							&& performLongPress(host, position, id);
			}
			return false;
		}
	}
	...
	private class PositionScroller implements Runnable {
		...
		void start(final int position) {
			...
			final int clampedPosition = Math.max(0,
		...
		void start(final int position, final int boundPosition) {
			...
			if (clampedPosition < firstPosition) {
				...
				final int boundPositionFromFirst = boundPosition
						- firstPosition;
		...
		void startWithOffset(final int position, int offset, final int duration) {
			...
			if (mTargetPosition < firstPosition) {
				...
				final View targetView = getChildAt(mTargetPosition
						- firstPosition);
		...
		void scrollToVisible(int targetPosition, int boundPosition, int duration) {
			...
			if (targetPosition < firstPosition || targetPosition > lastPosition) {
				Log.w(LOGTAG, "scrollToVisible called with targetPosition "
						+ targetPosition + " not visible [" + firstPosition
						+ ", " + lastPosition + "]");
			}
			...
			if (boundPosition >= 0) {
				final View boundChild = getChildAt(boundPosition
						- firstPosition);
		...
		public void run() {
			final int size = getAvailableSize();
			final int firstPosition = mFirstPosition;
			final int startPadding = (mIsVertical ? getPaddingTop()
					: getPaddingLeft());
			final int endPadding = (mIsVertical ? getPaddingBottom()
					: getPaddingRight());
			switch (mMode) {
				case MOVE_AFTER_POS: {
					...
					final int extraScroll = lastPosition < mItemCount - 1 ? Math
							.max(endPadding, mExtraScroll) : endPadding;
					final int scrollBy = lastViewSize - lastViewPixelsShowing
							+ extraScroll;
				...
				case MOVE_BEFORE_POS: {
					...
					final int extraScroll = firstPosition > 0 ? Math.max(
							mExtraScroll, startPadding) : startPadding;
				...
				case MOVE_BEFORE_BOUND: {
					...
					if (lastPosition > mBoundPosition) {
						...
						if (end > lastViewEnd) {
							smoothScrollBy(-(end - lastViewEnd),
									mScrollDuration);
						}
					}
					break;
				...
				case MOVE_OFFSET: {
					...
					final float screenTravelCount = (float) viewTravelCount
							/ childCount;
					final float modifier = Math.min(
							Math.abs(screenTravelCount), 1.f);
					if (position < firstPosition) {
