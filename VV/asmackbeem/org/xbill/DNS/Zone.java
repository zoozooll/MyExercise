...
public class Zone implements Serializable {
	...
	private synchronized SetResponse lookup(Name name, int type) {
		...
		for (tlabels = olabels; tlabels <= labels; tlabels++) {
			...
			/*
			 * If this is the name, look for the actual type or a CNAME. Otherwise, look for a
			 * DNAME.
			 */
			if (isExact) {
	...
	/**
	 * Returns an Iterator over the RRsets in the zone that can be used to construct an AXFR
	 * response. This is identical to {@link #iterator} except that the SOA is returned at the end
