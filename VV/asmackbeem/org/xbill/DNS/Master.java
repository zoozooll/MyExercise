...
public class Master {
		...
		while (true) {
			...
			if (token.type == Tokenizer.WHITESPACE) {
				...
				if (s.equalsIgnoreCase("$ORIGIN")) {
					...
					/*
					 * If we continued, we wouldn't be looking in the new file. Recursing works
					 * better.
					 */
					return nextRecord();
	 ...
	 * specifications for generated records are available by calling {@link #generators}. This must
	 * be called before a $GENERATE statement is seen during iteration to have an effect.
	 */
