...
public final class Lookup {
	 ...
	 * will use the default cache, resolver, and search path, and look for records that are
	 * reasonably credible.
	...
	/**
	 * Sets ndots to use when performing this lookup, overriding the default value. Specifically,
	 * this refers to the number of "dots" which, if present in a name, indicate that a lookup for
	 * the absolute name should be attempted before appending any search path elements.
	...
	/**
	 * Returns all known aliases for this name. Whenever a CNAME/DNAME is followed, an alias is
	 * added to this array. The last element in this array will be the owner name for records in the
	 ...
	 * @return The result code, which can be SUCCESSFUL, UNRECOVERABLE, TRY_AGAIN, HOST_NOT_FOUND,
	 *         or TYPE_NOT_FOUND.
	 * @throws IllegalStateException The lookup has not completed.
	 */
