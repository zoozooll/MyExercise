...
public class ExtendedResolver implements Resolver {
	private static class Resolution implements ResolverListener {
		...
		public Resolution(ExtendedResolver eres, Message query) {
			...
			if (eres.loadBalance) {
				...
				/*
				 * Note: this is not synchronized, since the worst thing that can happen is a random
		...
		public Message start() throws IOException {
			try {
				/*
				 * First, try sending synchronously. If this works, we're done. Otherwise, we'll get
				 * an exception and continue. It would be easier to call send(0), but this avoids a
				 * thread creation. If and when SimpleResolver.sendAsync() can be made to not create
				...
				/*
				 * This will either cause more queries to be sent asynchronously or will set the
				 * 'done' flag.
				 */
		...
		/*
		 * Receive a response. If the resolution hasn't been completed, either wake up the blocking
		 * thread or call the callback.
		 */
		@Override
		...
		/*
		 * Receive an exception. If the resolution has been completed, do nothing. Otherwise make
		 * progress.
		 */
		@Override
		public void handleException(Object id, Exception e) {
			...
			synchronized (this) {
				...
				/*
				 * If this is the first response from server n, we should start sending queries to
				 * server n + 1.
				 */
	 ...
	 * multiple times until either a successful response is received, or it is clear that there is
	 ...
	 * performed in parallel. Since the callback may be invoked before the function returns,
	 ...
	 * @param flag If true, servers will be tried in round-robin order. If false, servers will
	 *            always be queried in the same order.
	 */
