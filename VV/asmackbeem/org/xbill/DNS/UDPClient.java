...
final class UDPClient extends Client {
	...
	/*
	 * On some platforms (Windows), the SecureRandom module initialization involves a call to
	 * InetAddress.getLocalHost(), which can end up here if using a dnsjava name service provider.
	 * This can cause problems in multiple ways. - If the SecureRandom seed generation process calls
	 * into here, and this module attempts to seed the local SecureRandom object, the thread hangs.
	 * - If something else calls InetAddress.getLocalHost(), and that causes this module to seed the
	 * local SecureRandom object, the thread hangs. To avoid both of these, check at initialization
	 * time to see if InetAddress is in the call chain. If so, initialize the SecureRandom object in
	 * a new thread, and disable port randomization until it completes.
	 */
	static {
