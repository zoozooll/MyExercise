...
public class Cache {
	...
	private static class CacheMap extends LinkedHashMap {
		...
		void setMaxSize(int maxsize) {
			/*
			 * Note that this doesn't shrink the size of the map if the maximum size is lowered, but
			 * it should shrink as entries expire.
			 */
			this.maxsize = maxsize;
		}
		@Override
	...
	protected synchronized SetResponse lookup(Name name, int type, int minCred) {
		...
		for (tlabels = labels; tlabels >= 1; tlabels--) {
			...
			if (types == null)
				continue;
			/*
			 * If this is the name, look for the actual type or a CNAME (unless it's an ANY query,
			 * where we return everything). Otherwise, look for a DNAME.
			 */
	...
	/**
	 * Looks up credible Records in the Cache (a wrapper around lookupRecords). Unlike
	...
	/**
	 * Gets the current number of entries in the Cache, where an entry consists of all records with
	 * a specific Name.
	 */
	public int getSize() {
		return data.size();
	...
	/**
	 * Gets the maximum number of entries in the Cache, where an entry consists of all records with
	 * a specific Name. A negative value is treated as an infinite limit.
	 */
	public int getMaxEntries() {
		return data.getMaxSize();
	}
	/**
	 * Sets the maximum number of entries in the Cache, where an entry consists of all records with
	 * a specific Name. A negative value is treated as an infinite limit. Note that setting this to
	 * a value lower than the current number of entries will not cause the Cache to shrink
