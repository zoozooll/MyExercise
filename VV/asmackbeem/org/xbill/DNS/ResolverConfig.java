 ...
 * names. The following are attempted, in order, until one succeeds.
 * <UL>
 * <LI>The properties 'dns.server' and 'dns.search' (comma delimited lists) are checked. The servers
 * can either be IP addresses or hostnames (which are resolved using Java's built in DNS support).
 * <LI>The sun.net.dns.ResolverConfiguration class is queried.
 * <LI>On Unix, /etc/resolv.conf is parsed.
 * <LI>On Windows, ipconfig/winipcfg is called and its output parsed. This may fail for non-English
 * versions on Windows.
...
public class ResolverConfig {
	...
	/**
	 * Uses the undocumented Sun DNS implementation to determine the configuration. This doesn't
	 * work or even compile with all JVMs (gcj, for example).
	 */
	...
	/**
	 * Returns the located ndots value, or the default (1) if not configured. Note that ndots can
	 * only be configured in a resolv.conf file, and will only take effect if ResolverConfig uses
