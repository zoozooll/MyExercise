...
public class SimpleResolver implements Resolver {
	 ...
	 * that the response is associated with the query.
	...
	public Message send(Message query) throws IOException {
		...
		do {
			...
			/*
			 * Check that the response ID matches the query ID. We want to check this before
			 * actually parsing the message, so that if there's a malformed response that's not
			 * ours, it doesn't confuse us.
			 */
	...
	/**
	 * Asynchronously sends a message to a single server, registering a listener to receive a
	 * callback on success or exception. Multiple asynchronous lookups can be performed in parallel.
	 * Since the callback may be invoked before the function returns, external synchronization is
	 * necessary.
	 * @param query The query to send
