...
public class TSIG {
	 ...
	 * Creates a new TSIG object, which can be used to sign or verify a message.
	 * @param name The name of the shared key.
	 * @param algorithm The algorithm of the shared key. The legal values are "hmac-md5",
	 ...
	 *            alg/name/secret. If an algorithm is specified, it must be "hmac-md5", "hmac-sha1",
	 *            or "hmac-sha256".
	...
	/**
	 * Verifies a TSIG record on an incoming message. Since this is only called in the context where
	 * a TSIG is expected to be present, it is an error if one is not present. After calling this
	 ...
	 *            the message in wire format, and we can't recreate the exact wire format (with the
	 *            same name compression).
	 ...
	 *            format, and we can't recreate the exact wire format (with the same name
	 *            compression).
	 * @param old If this message is a response, the TSIG from the request
	 * @return The result of the verification (as an Rcode)
	 * @see Rcode
	...
	public static class StreamVerifier {
		...
		/**
		 * Verifies a TSIG record on an incoming message that is part of a multiple message
		 * response. TSIG records must be present on the first and last messages, and at least every
		 * 100 records in between. After calling this routine, Message.isVerified() may be called on
		 * this message.
		 * @param m The message
