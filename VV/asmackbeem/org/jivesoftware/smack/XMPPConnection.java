...
import org.jivesoftware.smack.readerwriter.VVInputstream;
import org.jivesoftware.smack.readerwriter.VVOutputstream;
...
/**
 * Creates a socket connection to a XMPP server. This is the default connection to a Jabber server
 * and is specified in the XMPP Core (RFC 3920).
 * @see Connection
 * @author Matt Tucker
 */
public class XMPPConnection extends Connection {
	...
	/**
	 * Creates a new connection to the specified XMPP server. A DNS SRV lookup will be performed to
	 * determine the IP address and port corresponding to the service name; if that lookup fails,
	 * it's assumed that server resides at <tt>serviceName</tt> with the default port of 5222.
	 * Encrypted connections (TLS) will be used if available, stream compression is disabled, and
	 * standard SASL mechanisms will be used for authentication.
	 * <p>
	 * <p/>
	 * This is the simplest constructor for connecting to an XMPP server. Alternatively, you can get
	 * fine-grained control over connection settings using the
	 * {@link #XMPPConnection(ConnectionConfiguration)} constructor.
	 * <p>
	 * <p/>
	 * Note that XMPPConnection constructors do not establish a connection to the server and you
	 * must call {@link #connect()}.
	 * <p>
	 * <p/>
	 * The CallbackHandler will only be used if the connection requires the client provide an SSL
	 * certificate to the server. The CallbackHandler must handle the PasswordCallback to prompt for
	 * a password to unlock the keystore containing the SSL certificate.
	...
	/**
	 * Creates a new XMPP connection in the same way {@link #XMPPConnection(String,CallbackHandler)}
	 * does, but with no callback handler for password prompting of the keystore. This will work in
	 * most cases, provided the client is not required to provide a certificate to the server.
	 * @param serviceName the name of the XMPP server to connect to; e.g. <tt>example.com</tt>.
	 */
	 ...
	 * {@link #XMPPConnection(ConnectionConfiguration,CallbackHandler)} does, but with no callback
	 * handler for password prompting of the keystore. This will work in most cases, provided the
	 * client is not required to provide a certificate to the server.
	 ...
	 * <p>
	 * <p/>
	 * Manually specifying connection configuration information is suitable for advanced users of
	...
	// add by baoyuedong 2015-07-13:注入一个packet到PacketReader中,VV的团队新闻packet就是这样实现的
	public void injectPacketToReader(Packet packet) {
		packetReader.injectPacket(packet);
	}
	@Override
	 ...
	 * server. The shutdown logic will be used during a planned disconnection or when dealing with
	 * an unexpected disconnection. Unlike {@link #disconnect()} the connection's packet reader,
	 * packet writer, and {@link Roster} will not be removed; thus connection's state is kept.
	 * @param unavailablePresence the presence packet to send during shutdown.
	 */
	protected void shutdown(Presence unavailablePresence) {
		// Set presence to offline.
		if (BeemApplication.isNetworkOk() && packetWriter != null) {
	...
	/**
	 * Registers a packet interceptor with this connection. The interceptor will be invoked every
	 * time a packet is about to be sent by this connection. Interceptors may modify the packet to
	 * be sent. A packet filter determines which packets will be delivered to the interceptor.
	...
	/**
	 * Registers a packet listener with this connection. The listener will be notified of every
	 * packet that this connection sends. A packet filter determines which packets will be delivered
	 * to the listener. Note that the thread that writes packets will be used to invoke the
	 * listeners. Therefore, each packet listener should complete all operations quickly or use a
	...
	private void connectUsingConfiguration(ConnectionConfiguration config)
		...
		try {
			...
			socketClosed = false;
			initConnection();
			//LogUtils.i("connect the " + host + ":" + port + " success.");
	...
	/**
	 * Initializes the connection by creating a packet reader and writer and opening a XMPP stream
	 * to the server.
	...
	private void initConnection() throws XMPPException {
		...
		try {
			...
			// available)
			// open the heartbeat thread.#add by baoyuedong 2015-01-28
		...
		try {
			if (!usingCompression) {
				reader = new BufferedReader(new InputStreamReader(
						new VVInputstream(socket.getInputStream()), "UTF-8"));
				writer = new BufferedWriter(new OutputStreamWriter(
						new VVOutputstream(socket.getOutputStream()), "UTF-8"));
				//LogUtils.v("Xmpp#usingCompression no");
			} else {
				try {
					Class<?> zoClass = Class
					...
					Object out = constructor.newInstance(new VVOutputstream(
					...
					Object in = constructor.newInstance(new VVInputstream(
							socket.getInputStream()));
					method = ziClass.getMethod("setFlushMode", Integer.TYPE);
					method.invoke(in, 2);
					reader = new BufferedReader(new InputStreamReader(
							(InputStream) in, "UTF-8"));
					//LogUtils.v("Xmpp#usingCompression success");
				} catch (Exception e) {
					e.printStackTrace();
					reader = new BufferedReader(new InputStreamReader(
					...
					//LogUtils.v("Xmpp#usingCompression failed");
				}
			}
		} catch (IOException ioe) {
	 ...
	 **********************************************/
	/**
	 * Returns true if the connection to the server has successfully negotiated TLS. Once TLS has
	 * been negotiatied the connection has been secured.
	...
	/**
	 * Notification message saying that the server supports TLS so confirm the server that we want
	...
	/**
	 * Starts using stream compression that will compress network traffic. Traffic can be reduced up
	 * to 90%. Therefore, stream compression is ideal when using a slow speed network connection.
	 * However, the server and the client will need to use more CPU time in order to un/compress
	 * network data so under high load the server performance might be affected.
	 * <p>
	 * <p/>
	 * Stream compression has to have been previously offered by the server. Currently only the zlib
	 * method is supported by the client. Stream compression negotiation has to be done before
	 ...
	 * negotiation of stream compression can only happen after TLS was negotiated. If TLS
	...
	/**
	 * Notifies the XMPP connection that stream compression was denied so that the connection
	 * process can proceed.
	 */
	 ...
	 * <p>
	 * <p/>
	 * Listeners will be preserved from a previous connection if the reconnection occurs after an
	 * abrupt termination.
	 * @throws XMPPException if an error occurs while trying to establish the connection. Two
	 *             possible errors can occur which will be wrapped by an XMPPException --
