...
/**
 * A chat is a series of messages sent between two users. Each chat has a unique thread ID, which is
 * used to track which messages are part of a particular conversation. Some messages are sent
 * without a thread ID, and some clients don't send thread IDs at all. Therefore, if a message
 * without a thread ID arrives it is routed to the most recently created Chat with the message
 * sender.
 * @author Matt Tucker
 */
public class Chat {
	...
	Chat(ChatManager chatManager, String participant, String threadID) {
		...
		if (participant.contains("@")) {
			throw new IllegalArgumentException(
					"chat formater not correct,participant" + participant);
		}
	}
	/**
	 * Returns the thread id associated with this chat, which corresponds to the <tt>thread</tt>
	 * field of XMPP messages. This method may return <tt>null</tt> if there is no thread ID is
	...
	public void sendMessage(Message message) throws XMPPException {
		...
		if (participant.contains("@")) {
			throw new XMPPException(
					"participant must not own the suffix!,participant"
							+ participant);
		}
		message.setTo(VVXMPPUtils.makeJidCompleted(participant));
	 ...
	 * deliver it to all listeners registered with the Chat. This is used by the Connection class to
	 * deliver messages without a thread ID.
	...
	void deliver(Message message) {
		...
		if (message != null && message.getError() == null) {
			MessageAckPacket msgAck = new MessageAckPacket(
					message.getPacketID());
			chatManager.sendMessageAck(msgAck);
		}
	}
	@Override
	public int hashCode() {
		...
		int result = 1;
		result = prime * result
				+ ((chatManager == null) ? 0 : chatManager.hashCode());
		result = prime * result
	...
	public boolean equals(Object obj) {
		...
		Chat other = (Chat) obj;
		if (chatManager == null) {
			if (other.chatManager != null)
				return false;
		} else if (!chatManager.equals(other.chatManager))
			return false;
		if (participant == null) {
