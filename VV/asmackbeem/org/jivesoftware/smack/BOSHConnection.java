...
/**
 * Creates a connection to a XMPP server via HTTP binding. This is specified in the XEP-0206: XMPP
 * Over BOSH.
 * @see Connection
...
public class BOSHConnection extends Connection {
	 ...
	 * @param host the hostname or IP address of the connection manager (e.g. domain.lt for
	 *            http://domain.lt:7070/http-bind).
	 * @param port the port of the connection manager (e.g. 7070 for
	 *            http://domain.lt:7070/http-bind).
	 * @param filePath the file which is described by the URL (e.g. /http-bind for
	 *            http://domain.lt:7070/http-bind).
	 * @param xmppDomain the XMPP service name (e.g. domain.lt for the user alice@domain.lt)
	 */
	...
	/**
	 * Closes the connection by setting presence to unavailable and closing the HTTP client. The
	 * shutdown logic will be used during a planned disconnection or when dealing with an unexpected
	 * disconnection. Unlike {@link #disconnect()} the connection's BOSH packet reader and
	...
	/**
	 * A listener class which listen for a successfully established connection and connection errors
	 * and notifies the BOSHConnection.
	 * @author Guenther Niess
	 */
	private class BOSHConnectionListener implements BOSHClientConnListener {
		...
		/**
		 * Notify the BOSHConnection about connection state changes. Process the connection
		 * listeners and try to login if the connection was formerly authenticated and is now
		 * reconnected.
		 */
		@Override
