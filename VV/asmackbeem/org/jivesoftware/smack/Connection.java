...
/**
 * The abstract Connection class provides an interface for connections to a XMPP server and
 * implements shared methods which are used by the different types of connections (e.g.
 * XMPPConnection or BoshConnection). To create a connection to a XMPP server a simple usage of this
 ...
 * <p>
 * <p/>
 * If a connected Connection gets disconnected abruptly then it will try to reconnect again. To stop
 * the reconnection process, use {@link #disconnect()}. Once stopped you can use {@link #connect()}
...
public abstract class Connection {
	...
	/**
	 * Value that indicates whether debugging is enabled. When enabled, a debug window will apear
	 * for each new connection that will contain the following information:
	 * <ul>
	 ...
	 * property <tt>smack.debugEnabled</tt> to true. The system property can be set on the command
	 * line such as "java SomeApp -Dsmack.debugEnabled=true".
	 */
	public static boolean DEBUG_ENABLED = true && BeemApplication.sDebuggerEnabled;
	static {
	 ...
	 * <tt>null</tt> if not logged in yet. An XMPP address is in the form username@server/resource.
	 * @return the full XMPP address of the user logged in.
	 */
	public abstract String getUser();
	/**
	 * Returns the connection ID for this connection, which is the value set by the server when
	 * opening a XMPP stream. If the server does not set a connection ID, this value will be null.
	 * This value will be <tt>null</tt> if not connected to the server.
	 * @return the ID of this connection returned from the XMPP server or <tt>null</tt> if not
	...
	public void login(String username, String password) throws XMPPException {
		login(username, password, AppProperty.getInstance().XMPPSERVER_RESOUCE);
	}
	/**
	 * Logs in to the server using the strongest authentication mode supported by the server, then
	 * sets presence to available. If the server supports SASL authentication then the user will be
	...
	/**
	 * Set the store for the roster of this connection. If you set the roster storage of a
	 * connection you enable support for XEP-0237 (RosterVersioning)
	 ...
	 * <p>
	 * <p/>
	 * This method cleans up all resources used by the connection. Therefore, the roster, listeners
	 * and other stateful objects cannot be re-used by simply calling connect() on this connection
	 * again. This is unlike the behavior during unexpected disconnects (and subsequent
	 * connections). In that case, all state is preserved to allow for more seamless error recovery.
	 */
	 ...
	 * closing the stream. The Connection can still be used for connecting to the server again. A
	 * custom unavilable presence is useful for communicating offline presence information such as
	 * "On vacation". Typically, just the status text of the presence packet is set with online
	...
	/**
	 * Adds a new listener that will be notified when new Connections are created. Note that newly
	 * created connections will not be actually connected to the server.
	...
	/**
	 * Adds a connection listener to this connection that will be notified when the connection
	 * closes or fails. The connection needs to already be connected or otherwise an
	...
	/**
	 * Creates a new packet collector for this connection. A packet filter determines which packets
	 * will be accumulated by the collector. A PacketCollector is more suitable to use than a
	 * {@link PacketListener} when you need to wait for a specific result.
	...
	/**
	 * Registers a packet listener with this connection. A packet filter determines which packets
	 * will be delivered to the listener. If the same packet listener is added again with a
	 * different filter, only the new filter will be used.
