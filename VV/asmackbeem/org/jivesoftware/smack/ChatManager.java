 ...
 * in memory on its own so it is neccesary to keep a reference to the chat object itself. To be made
 * aware of new chats, register a listener by calling {@link #addChatListener(ChatManagerListener)}.
...
public class ChatManager {
	...
	private Chat createChat(String userJID, String threadID,
		...
		// vv custom# userJid进行去后缀处理
		userJID = VVXMPPUtils.makeJidParsed(userJID);
	...
	/**
	 * Try to get a matching chat for the given user JID. Try the full JID map first, the try to
	 * match on the base JID if no match is found.
	 * @param userJID
	 * @return
	 */
	private Chat getUserChat(String userJID) {
		// VV custom#jidChats保存的是去后缀的时光号
		userJID = VVXMPPUtils.makeJidParsed(userJID);
	...
	void sendMessageAck(MessageAckPacket msgAck) {
		connection.sendPacket(msgAck);
	}
