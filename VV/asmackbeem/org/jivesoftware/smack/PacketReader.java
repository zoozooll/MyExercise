...
/**
 * Listens for XML traffic from the XMPP server and parses it into packet objects. The packet reader
 * also invokes all packet listeners and collectors.
 * <p>
...
class PacketReader {
	...
	/**
	 * Starts the packet reader thread and returns once a connection to the server has been
	 * established. A connection will be attempted for a maximum of five seconds. An XMPPException
	 * will be thrown if the connection fails.
	 * @throws XMPPException if the server fails to send an opening stream back for more than five
	 *             seconds.
	 */
	...
	/**
	 * Sends out a notification that there was an error with the connection and closes the
	 * connection.
	...
	/**
	 * Resets the parser using the latest connection's reader. Reseting the parser is necessary when
	 * the plain connection has been secured or when a new opening stream element is going to be
	 * sent by the server.
	 */
	...
	/**
	 * Releases the connection ID lock so that the thread that was waiting can resume. The lock will
	 * be released when one of the following three conditions is met:
	 * <p>
	 * 1) An opening stream was sent from a non XMPP 1.0 compliant server 2) Stream features were
	 * received from an XMPP 1.0 compliant server that does not support TLS 3) TLS negotiation was
	 * successful
	 */
