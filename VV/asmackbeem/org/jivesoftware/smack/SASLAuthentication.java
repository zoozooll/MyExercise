...
/**
 * <p>
 * This class is responsible authenticating the user using SASL, binding the resource to the
 * connection and establishing a session with the server.
 * </p>
 * <p>
 * Once TLS has been negotiated (i.e. the connection has been secured) it is possible to register
 * with the server, authenticate using Non-SASL or authenticate using SASL. If the server supports
 * SASL then Smack will first try to authenticate using SASL. But if that fails then Non-SASL will
 * be tried.
 * </p>
 * <p>
 * The server may support many SASL mechanisms to use for authenticating. Out of the box Smack
 * provides several SASL mechanisms, but it is possible to register new SASL Mechanisms. Use
 ...
 * </p>
 * <p>
 * Once the user has been authenticated with SASL, it is necessary to bind a resource for the
 * connection. If no resource is passed in {@link #authenticate(String, String, String)} then the
 * server will assign a resource for the connection. In case a resource is passed then the server
 * will receive the desired resource but may assign a modified resource for the connection.
 * </p>
 * <p>
 * Once a resource has been binded and if the server supports sessions then Smack will establish a
 * session so that instant messaging and presence functionalities may be used.
 * </p>
...
public class SASLAuthentication implements UserAuthentication {
	...
	/**
	 * Boolean indication if SASL authentication has failed. When failed the server may end the
	 * connection.
	 */
	...
	/**
	 * Unregisters an existing SASL mechanism. Once the mechanism has been unregistered it won't be
	 * possible to authenticate users using the removed SASL mechanism. It also removes the
	...
	/**
	 * Registers a new SASL mechanism in the specified preference position. The client will try to
	 * authenticate using the most prefered SASL mechanism that is also supported by the server. The
	 * SASL mechanism must be registered via {@link #registerSASLMechanism(String, Class)}
	 * @param name common name of the SASL mechanism. E.g.: PLAIN, DIGEST-MD5 or KERBEROS_V4.
	 */
	...
	/**
	 * Registers a new SASL mechanism in the specified preference position. The client will try to
	 * authenticate using the most prefered SASL mechanism that is also supported by the server. Use
	 * the <tt>index</tt> parameter to set the level of preference of the new SASL mechanism. A
	 * value of 0 means that the mechanism is the most prefered one. The SASL mechanism must be
	...
	/**
	 * Un-supports an existing SASL mechanism. Once the mechanism has been unregistered it won't be
	 * possible to authenticate users using the removed SASL mechanism. Note that the mechanism is
	 * still registered, but will just not be used.
	 * @param name common name of the SASL mechanism. E.g.: PLAIN, DIGEST-MD5 or KERBEROS_V4.
	 */
	 ...
	 * <p>
	 * The server may assign a full JID with a username or resource different than the requested by
	 * this method.
	...
	/**
	 * Performs ANONYMOUS SASL authentication. If SASL authentication was successful then resource
	 * binding and session establishment will be performed. This method will return the full JID
	 ...
	 * available SASL mechanism once the TLS negotiation was successful. This information is stored
	 * and will be used when doing the authentication for logging in the user.
	 * @param mechanisms collection of strings with the available SASL mechanism reported by the
	 *            server.
	 */
	...
	/**
	 * The server is challenging the SASL authentication we just sent. Forward the challenge to the
	 * current SASLMechanism we are using. The SASLMechanism will send a response to the server. The
	 * length of the challenge-response sequence varies according to the SASLMechanism in use.
	...
	/**
	 * Notification message saying that SASL authentication was successful. The next step would be
	...
	/**
	 * Notification message saying that SASL authentication has failed. The server may have closed
	 * the connection depending on the number of possible retries.
	...
	/**
	 * Initializes the internal state in order to be able to be reused. The authentication is used
	 * by the connection at the first login and then reused after the connection is disconnected and
	 * then reconnected.
	 */
	protected void init() {
