...
/**
 * Configuration to use while establishing the connection to the server. It is possible to configure
 * the path to the trustore file that keeps the trusted CA root certificates and enable or disable
 * all or some of the checkings done while verifying server certificates.
 * <p>
...
public class ConnectionConfiguration implements Cloneable {
	/**
	 * Hostname of the XMPP server. Usually servers use the same service name as the name of the
	 * server. However, there are some servers like google where host would be talk.google.com and
	 * the serviceName would be gmail.com.
	 */
	...
	// 是否发送心跳包#2015-01-28 add by baoyuedong
	...
	/**
	 * Creates a new ConnectionConfiguration for the specified service name with specified proxy. A
	 * DNS SRV lookup will be performed to find out the actual host address and port to use for the
	 * connection.
	 * @param serviceName the name of the service provided by an XMPP server.
	...
	/**
	 * Creates a new ConnectionConfiguration using the specified host, port and service name. This
	 * is useful for manually overriding the DNS SRV lookup process that's used with the
	 * {@link #ConnectionConfiguration(String)} constructor. For example, say that an XMPP server is
	 * running at localhost in an internal network on port 5222 but is configured to think that it's
	 * "example.com" for testing purposes. This constructor is necessary to connect to the server in
	 * that case since a DNS SRV lookup for example.com would not point to the local testing server.
	...
	/**
	 * Returns the host to use when establishing the connection. The host and port to use might have
	 * been resolved by a DNS lookup as specified by the XMPP spec (and therefore may not match the
	 * {@link #getServiceName service name}.
	...
	/**
	 * Returns the port to use when establishing the connection. The host and port to use might have
	 * been resolved by a DNS lookup as specified by the XMPP spec.
	...
	/**
	 * Returns the TLS security mode used when making the connection. By default, the mode is
	 * {@link SecurityMode#enabled}.
	...
	/**
	 * Sets the TLS security mode used when making the connection. By default, the mode is
	 * {@link SecurityMode#enabled}.
	...
	/**
	 * Retuns the path to the trust store file. The trust store file contains the root certificates
	 * of several well known CAs. By default, will attempt to use the the file located in
	...
	/**
	 * Sets the path to the trust store file. The truststore file contains the root certificates of
	 * several well?known CAs. By default Smack is going to use the file located in
	 * $JREHOME/lib/security/cacerts.
	...
	/**
	 * Sets the password to use to access the trust store file. It is assumed that all certificates
	 * share the same password in the trust store.
	...
	/**
	 * Retuns the path to the keystore file. The key store file contains the certificates that may
	 * be used to authenticate the client to the server, in the event the server requests or
	 * requires it.
	...
	/**
	 * Sets the path to the keystore file. The key store file contains the certificates that may be
	 * used to authenticate the client to the server, in the event the server requests or requires
	 * it.
	...
	/**
	 * Returns true if the whole chain of certificates presented by the server are going to be
	 * checked. By default the certificate chain is not verified.
	 * @return true if the whole chaing of certificates presented by the server are going to be
	 *         checked.
	 */
	...
	/**
	 * Sets if the whole chain of certificates presented by the server are going to be checked. By
	 * default the certificate chain is not verified.
	 * @param verifyChainEnabled if the whole chaing of certificates presented by the server are
	 *            going to be checked.
	 */
	...
	/**
	 * Returns true if self-signed certificates are going to be accepted. By default this option is
	 * disabled.
	...
	/**
	 * Sets if self-signed certificates are going to be accepted. By default this option is
	 * disabled.
	...
	/**
	 * Sets if certificates presented by the server are going to be checked for their validity. By
	 * default certificates are not verified.
	 * @param expiredCertificatesCheckEnabled if certificates presented by the server are going to
	...
	/**
	 * Sets if certificates presented by the server are going to be checked for their domain. By
	 * default certificates are not verified.
	 * @param notMatchingDomainCheckEnabled if certificates presented by the server are going to be
	...
	/**
	 * Sets a custom SSLContext for creating SSL sockets. A custom Context causes all other SSL/TLS
	 * realted settings to be ignored.
	...
	/**
	 * Returns true if the connection is going to use stream compression. Stream compression will be
	 * requested after TLS was established (if TLS was enabled) and only if the server offered
	 * stream compression. With stream compression network traffic can be reduced up to 90%. By
	...
	/**
	 * Sets if the connection is going to use stream compression. Stream compression will be
	 * requested after TLS was established (if TLS was enabled) and only if the server offered
	 * stream compression. With stream compression network traffic can be reduced up to 90%. By
	 * default compression is disabled.
	 ...
	 * If SASL authenticatin fails then the client will try to use non-sasl authentication. By
	 * default SASL is enabled.
	 * @return true if the client is going to use SASL authentication when logging into the server.
	 */
	public boolean isSASLAuthenticationEnabled() {
		return saslAuthenticationEnabled;
	}
	/**
	 * Sets whether the client will use SASL authentication when logging into the server. If SASL
	 * authenticatin fails then the client will try to use non-sasl authentication. By default, SASL
	 * is enabled.
	...
	/**
	 * Returns true if the new connection about to be establish is going to be debugged. By default
	 * the value of {@link Connection#DEBUG_ENABLED} is used.
	...
	/**
	 * Sets if the new connection about to be establish is going to be debugged. By default the
	 * value of {@link Connection#DEBUG_ENABLED} is used.
	...
	/**
	 * Sets the socket factory used to create new xmppConnection sockets. This is useful when
	 * connecting through SOCKS5 proxies.
	...
	/**
	 * Sets if an initial available presence will be sent to the server. By default an available
	 * presence will be sent to the server indicating that this presence is not online and available
	 * to receive messages. If you want to log in without being 'noticed' then pass a <tt>false</tt>
	 * value.
	...
	/**
	 * Returns true if the roster will be loaded from the server when logging in. This is the common
	 * behaviour for clients but sometimes clients may want to differ this or just never do it if
	 * not interested in rosters.
	...
	/**
	 * Sets if the roster will be loaded from the server when logging in. This is the common
	 * behaviour for clients but sometimes clients may want to differ this or just never do it if
	 * not interested in rosters.
	...
	/**
	 * Returns a CallbackHandler to obtain information, such as the password or principal
	 * information during the SASL authentication. A CallbackHandler will be used <b>ONLY</b> if no
	 * password was specified during the login while using SASL authentication.
	 * @return a CallbackHandler to obtain information, such as the password or principal
	...
	/**
	 * Sets a CallbackHandler to obtain information, such as the password or principal information
	 * during the SASL authentication. A CallbackHandler will be used <b>ONLY</b> if no password was
	 * specified during the login while using SASL authentication.
	 * @param callbackHandler to obtain information, such as the password or principal information
	...
	/**
	 * Returns the socket factory used to create new xmppConnection sockets. This is useful when
	 * connecting through SOCKS5 proxies.
	...
	/**
	 * An enumeration for TLS security modes that are available when making a connection to the XMPP
	 * server.
	 */
	public static enum SecurityMode {
		/**
		 * Securirty via TLS encryption is required in order to connect. If the server does not
		 * offer TLS or if the TLS negotiaton fails, the connection to the server will fail.
		 */
		required,
		/**
		 ...
		 * If only TLS encryption is available from the server, the connection will fail.
		 */
		disabled
	}
	...
	/**
	 * Enable or disable Entity Capabilities (XEP-115) http://xmpp.org/extensions/xep-0115.html Only
	 * available on XMPPConnection default is enabled
	 * @param enabled
	 */
	...
	public void setSendHeartBeat(boolean sendHeartBeats) {
		this.sendHeartBeat = sendHeartBeats;
	}
}
