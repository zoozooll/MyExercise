 ...
 * here.
 * <p/>
 * When you construct an <code>AbstractReferenceMap</code>, you can specify what kind of references
 ...
 * <p/>
 * Different types of references can be specified for keys and values. The keys can be configured to
 * be weak but the values hard, in which case this class will behave like a <a
 ...
 * <p/>
 * This {@link Map} implementation does <i>not</i> allow null elements. Attempting to add a null key
 * or value to the map will raise a <code>NullPointerException</code>.
 * <p/>
 * All the available iterators can be reset back to the start by casting to
 * <code>ResettableIterator</code> and calling <code>reset()</code>.
 * <p/>
 * This implementation is not synchronized. You can use
 * {@link java.util.Collections#synchronizedMap} to provide synchronized access to a
...
public abstract class AbstractReferenceMap<K, V> extends
	 ...
	 * capacity.
	 * @param keyType the type of reference to use for keys; must be {@link #SOFT} or {@link #WEAK}
	 * @param valueType the type of reference to use for values; must be {@link #SOFT} or
	 *            {@link #WEAK}
	...
	/**
	 * Returns a set view of this map's entries. An iterator returned entry is valid until
	 * <code>next()</code> is called again. The <code>setValue()</code> method on the
	 * <code>toArray</code> entries has no effect.
	 ...
	 * <p/>
	 * Note that this method is not synchronized! Special care must be taken if, for instance, you
	...
	protected static class ReferenceEntry<K, V> extends HashEntry<K, V> {
		...
		/**
		 * Gets the key from the entry. This method dereferences weak and soft keys and thus may
		 * return null.
		 ...
		 * <p/>
		 * This implementation uses <code>isEqualKey</code> and <code>isEqualValue</code> on the
		 * main map for comparison.
		...
		/**
		 * Constructs a reference of the given type to the given referent. The reference is
		 * registered with the queue for later purging.
		 * @param type HARD, SOFT or WEAK
		 * @param referent the object to refer to
		 * @param hash the hash code of the <i>key</i> of the mapping; this number might be
		 *            different from referent.hashCode() if the referent represents a value and not
		 *            a key
		 */
