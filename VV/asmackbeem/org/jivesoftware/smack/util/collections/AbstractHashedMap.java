 ...
 * override.
 * <p/>
 * This class implements all the features necessary for a subclass hash-based map. Key-value entries
 * are stored in instances of the <code>HashEntry</code> class, which can be overridden and
 ...
 * here.
 * <p/>
 * NOTE: From Commons Collections 3.1 this class extends AbstractMap. This is to provide backwards
 * compatibility for ReferenceMap between v3.0 and v3.1. This extends clause will be removed in
 * v4.0.
...
public class AbstractHashedMap<K, V> extends AbstractMap<K, V> implements
	 ...
	 * uses the equals method. Subclasses can override this to match differently.
	 ...
	 * <p/>
	 * This method exists for subclasses that may need to perform a multi-step process accessing the
	 * entry. The public methods in this class don't use this method to gain a small performance
	 * boost.
	 * @param key the key
	 ...
	 * <p/>
	 * This implementation calls <code>setValue()</code> on the entry. Subclasses could override to
	 ...
	 * <p/>
	 * This implementation calls <code>createEntry()</code>, <code>addEntry()</code> and
	 * <code>checkCapacity()</code>. It also handles changes to <code>modCount</code> and
	 * <code>size</code>. Subclasses could override to fully control adds to the map.
	 ...
	 * <p/>
	 * This implementation calls <code>removeEntry()</code> and <code>destroyEntry()</code>. It also
	 * handles changes to <code>modCount</code> and <code>size</code>. Subclasses could override to
	...
	/**
	 * Calculates the new capacity of the map. This implementation normalizes the capacity to a
	 * power of two.
	...
	/**
	 * Calculates the new threshold of the map, where it will be resized. This implementation uses
	 * the load factor.
	...
	// -----------------------------------------------------------------------
	/**
	 * Gets the <code>next</code> field from a <code>HashEntry</code>. Used in subclasses that have
	...
	/**
	 * Gets the <code>key</code> field from a <code>HashEntry</code>. Used in subclasses that have
	 * no visibility of the field.
	 * @param entry the entry to query, must not be null
	 ...
	 * <p/>
	 * A MapIterator returns the keys in the map. It also provides convenient methods to get the key
	 * and value, and set the value. It avoids the need to create an entrySet/keySet/values object.
	...
	// -----------------------------------------------------------------------
	/**
	 * Gets the entrySet view of the map. Changes made to the view affect this map. To simply
	...
	/**
	 * Creates an entry set iterator. Subclasses can override this to return iterators with
	 ...
	 * <p/>
	 * If you subclass <code>AbstractHashedMap</code> but not <code>HashEntry</code> then you will
	 * not be able to access the protected fields. The <code>entryXxx()</code> methods on
	 ...
	 * <p/>
	 * Serialization is not one of the JDK's nicest topics. Normal serialization will initialise the
	 * superclass before the subclass. Sometimes however, this isn't what you want, as in this case
	 * the <code>put()</code> method on read can be affected by subclass state.
	 * <p/>
	 * The solution adopted here is to serialize the state data of this class in this protected
	 * method. This method must be called by the <code>writeObject()</code> of the first
	 * serializable subclass.
	 * <p/>
	 * Subclasses may override if they have a specific field that must be present on read before
	 * this implementation will work. Generally, the read determines what must be serialized here,
	 * if anything.
	...
	/**
	 * Reads the map data from the stream. This method must be overridden if a subclass must be
	 * setup before <code>put()</code> is used.
	 * <p/>
	 * Serialization is not one of the JDK's nicest topics. Normal serialization will initialise the
	 * superclass before the subclass. Sometimes however, this isn't what you want, as in this case
	 * the <code>put()</code> method on read can be affected by subclass state.
	 * <p/>
	 * The solution adopted here is to deserialize the state data of this class in this protected
	 * method. This method must be called by the <code>readObject()</code> of the first serializable
	 * subclass.
	 * <p/>
	 * Subclasses may override if the subclass has a specific field that must be present before
	 ...
	 * <p/>
	 * To implement <code>clone()</code>, a subclass must implement the <code>Cloneable</code>
