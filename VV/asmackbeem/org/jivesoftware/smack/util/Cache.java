...
/**
 * A specialized Map that is size-limited (using an LRU algorithm) and has an optional expiration
 * time for cache items. The Map is thread-safe.
 * <p>
 * The algorithm for cache is as follows: a HashMap is maintained for fast object lookup. Two linked
 * lists are maintained: one keeps objects in the order they are accessed from cache, the other
 * keeps objects in the order they were originally added to cache. When objects are added to cache,
 * they are first wrapped by a CacheObject which maintains the following pieces of information:
 * <ul>
...
public class Cache<K, V> implements Map<K, V> {
	 ...
	 * lifetime of objects.
	 * @param maxSize the maximum number of objects the cache will hold. -1 means the cache has no
	 *            max size.
	 * @param maxLifetime the maximum amount of time (in ms) objects can exist in cache before being
	...
	/*
	 * Remove operation with a flag so we can tell coherence if the remove was caused by cache
	 ...
	 * objects. This class is optimized for speed rather than strictly correct encapsulation.
	 */
	...
	/**
	 * Simple LinkedList implementation. The main feature is that list nodes are public, which
	 * allows very fast delete operations when one has a reference to the node that is to be
	 * deleted.
	 * <p>
	 */
	private static class LinkedList {
		...
		/**
		 * Adds an object to the beginning of the list by automatically creating a a new node and
		...
		/**
		 * Adds an object to the end of the list by automatically creating a a new node and adding
	...
	/**
	 * Doubly linked node in a LinkedList. Most LinkedList implementations keep the equivalent of
	 * this class private. We make it public so that references to each node in the list can be
	 * maintained externally. Exposing this class lets us make remove operations very fast. Remove
	 * is built into this class and only requires two reference reassignments. If remove existed in
	 * the main LinkedList class, a linear scan would have to be performed to find the correct node
	 * to delete. The linked list implementation was specifically written for the Jive cache system.
	 * While it can be used as a general purpose linked list, for most applications, it is more
	 * suitable to use the linked list that is part of the Java Collections package.
	 */
	private static class LinkedListNode {
		...
		/**
		 * This class is further customized for the Jive cache system. It maintains a timestamp of
		 * when a Cacheable object was first added to cache. Timestamps are stored as long values
		 * and represent the number of milliseconds passed since January 1, 1970 00:00:00.000 GMT.
		 * <p>
		 * The creation timestamp is used in the case that the cache has a maximum lifetime set. In
		 * that case, when [current time] - [creation time] > [max lifetime], the object will be
		 * deleted from cache.
		 */
		...
		/**
		 * Returns a String representation of the linked list node by calling the toString method of
		 * the node's object.
