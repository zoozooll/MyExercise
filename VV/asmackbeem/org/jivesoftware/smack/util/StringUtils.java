...
public class StringUtils {
	 ...
	 * "matt@jivesoftware.com/Smack", "matt" would be returned. If no username is present in the
	 * address, the empty string will be returned.
	...
	/**
	 * Returns the server portion of a XMPP address. For example, for the address
	 * "matt@jivesoftware.com/Smack", "jivesoftware.com" would be returned. If no server is present
	 * in the address, the empty string will be returned.
	 * @param XMPPAddress the XMPP address.
	...
	/**
	 * Returns the resource portion of a XMPP address. For example, for the address
	 * "matt@jivesoftware.com/Smack", "Smack" would be returned. If no resource is present in the
	 * address, the empty string will be returned.
	 * @param XMPPAddress the XMPP address.
	...
	/**
	 * Escapes the node portion of a JID according to "JID Escaping" (JEP-0106). Escaping replaces
	 * characters prohibited by node-prep with escape sequences, as follows:
	 * <p>
	 * <table border="1">
	 * <tr>
	 ...
	 * </table>
	 * <p>
	 * This process is useful when the node comes from an external source that doesn't conform to
	 * nodeprep. For example, a username in LDAP may be "Joe Smith". Because the &lt;space&gt;
	 * character isn't a valid part of a node, the username should be escaped to "Joe\20Smith"
	 * before being made into a JID (e.g. "joe\20smith@example.com" after case-folding, etc. has
	 * been applied).
	 * <p>
	 * All node escaping and un-escaping must be performed manually at the appropriate time; the JID
	 * class will not escape or un-escape automatically.
	 * @param node the node.
	...
	/**
	 * Array of numbers and letters of mixed case. Numbers appear in the list twice so that there is
	 * a more equal chance that a number will be picked. We can use the array to get a random number
	 * or letter by picking a random array index.
	 */
	private static char[] numbersAndLetters = ("0123456789abcdefghijklmnopqrstuvwxyz"
			+ "0123456789ABCDEFGHIJKLMNOPQRSTUVWXYZ").toCharArray();

	/**
	 * Returns a random String of numbers and letters (lower and upper case) of the specified
	 * length. The method uses the Random class that is built-in to Java which is suitable for low
	 * to medium grade security uses. This means that the output is only pseudo random, i.e., each
	 * number is mathematically generated so is not truly random.
	 * <p>
