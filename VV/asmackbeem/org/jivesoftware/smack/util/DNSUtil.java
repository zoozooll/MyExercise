...
public class DNSUtil {
	/**
	 * Create a cache to hold the 100 most recently accessed DNS lookups for a period of 10 minutes.
	 */
	 ...
	 * <p>
	 * As an example, a lookup for "example.com" may return "im.example.com:5269". Note on SRV
	 * record selection. We now check priority and weight, but we still don't do this correctly. The
	 * missing behavior is this: if we fail to reach a host based on its SRV record then we need to
	 * select another host from the other SRV records. In Smack 3.1.1 we're not going to be able to
	 * do the major system redesign to correct this.
