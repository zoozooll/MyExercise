...
/**
 * Utility class that helps to parse packets. Any parsing packets method that must be shared between
 * many clients must be placed in this utility class.
...
public class PacketParserUtils {
	...
	public static Packet parseMessage(XmlPullParser parser) throws Exception {
		...
		while (!done) {
			...
			if (eventType == XmlPullParser.START_TAG) {
				...
				} else if (elementName.equals("gid")) {
					if (message.getType() == Type.comment || message.getType() == Type.like) {
						message.getComment().gid = parseContent(parser);
					} else if (message.getType() == Type.mention) {
						message.getTimeflyNotify().gid = parseContent(parser);
					}
				} else if (elementName.equals("gid_create_time")) {
					String content = parseContent(parser);
					content = content.length() > 10 ? content.substring(0, 10)
							: content;
					if (message.getType() == Type.comment || message.getType() == Type.like) {
						message.getComment().gid_create_time = content;
					} else if (message.getType() == Type.mention) {
						message.getTimeflyNotify().gid_create_time = content;
					}
				} else if (elementName.equals("cid")) {
					message.getComment().cid = parseContent(parser);
				} else if (elementName.equals("to_cid")) {
					message.getComment().to_cid = parseContent(parser);
				} 
				else if (elementName.equals("jid")) {
					if (message.getType() == Type.comment || message.getType() == Type.like) {
						message.getComment().jid = VVXMPPUtils.makeJidParsed(parseContent(parser));
					}
				}
				else if (elementName.equals("comment_time")) {
					message.getComment().comment_time = parseContent(parser);
				} else if (elementName.equals("date")) {
					if (message.getType() == Type.mention) {
						message.getTimeflyNotify().date = parseContent(parser);
					}
				} else if (elementName.equals("mention_create_time")) {
					if (message.getType() == Type.mention) {
						message.getTimeflyNotify().mention_create_time = parseContent(parser);
					}
				} else if (elementName.equals("photogroup")) {
	...
	private static Presence selectPresence(XmlPullParser parser)
			throws Exception {
		...
		if (action != null && (action.equals("add") || action.equals("remove"))) {
			BlacklistPresence presence = new Presence.BlacklistPresence(
					Presence.Type.available);
			presence.setAction(parser.getAttributeValue("", "action"));
			return presence;
		} else {
	...
	private static boolean selectParsePresenceElem(Presence presence,
			String elemName, XmlPullParser parser) {
		Log.i("AAA", "elemName: " + elemName);
		if (presence instanceof BlacklistPresence) {
			if (elemName.equals("item")) {
				BlacklistPresence blPresence = (BlacklistPresence) presence;
				String user = parser.getAttributeValue("", "jid");
				blPresence.addItemUser(user);
				return true;
			}
		} else if (elemName.equals("vCard")) {
			Log.i("AAA", "elemName2: " + elemName + "--" + presence.getFrom());
			// presence.userInfoPacket = new UserInfoPacket(presence.getFrom());
			// presence.userInfoPacket.setUserInfo(new
			// Item(presence.getFrom()));
			// presence.userInfoPacket.getUserInfo().note = "hhdfsdfsd";
			try {
				presence.userInfoPacket = parseUserInfo(parser, presence);
			} catch (Exception e) {
				// TODO Auto-generated catch block
				e.printStackTrace();
			}
			return true;
		} else if (elemName.equals("times")) {
			Log.i("AAA", "shiguang : " + elemName);
			try {
				presence.userInfoPacket = parseTeamInfo(parser, presence);
			} catch (Exception e) {
				// TODO Auto-generated catch block
				e.printStackTrace();
			}
	...
	public static Presence parsePresence(XmlPullParser parser) throws Exception {
		Presence presence = selectPresence(parser);
		// Presence presence = new Presence(type);
		...
		while (!done) {
			...
			if (eventType == XmlPullParser.START_TAG) {
				...
				} else if (selectParsePresenceElem(presence, elementName,
						parser)) {
			...
			if (eventType == XmlPullParser.START_TAG) {
				if (parser.getName().equals("item")) {
					...
					if (jid == null) {
						...
						item = new RosterPacket.Item(jid, nickname);
						item.setLogintime(parser.getAttributeValue("",
								"logintime"));
						item.setHeadPhoto(parser.getAttributeValue("",
								"portrait_small"));
						item.setSignature(parser.getAttributeValue("",
								"signature"));
						item.setBday(parser.getAttributeValue("", "bday"));
						item.setSex(parser.getAttributeValue("", "sex"));
						// Set status.
						...
						// vv custom
						item.setAlias(parser.getAttributeValue("", "alias"));
						item.setLat(parser.getAttributeValue("", "lat"));
						item.setLon(parser.getAttributeValue("", "lon"));
					}
				}
			...
			if (eventType == XmlPullParser.START_TAG) {
				...
				if (parser.getNamespace().equals("jabber:iq:register")) {
					...
					for (int i = 0; i < parser.getAttributeCount(); i++) {
						fields.put(parser.getAttributeName(i),
	...
	private static Bind parseResourceBinding(XmlPullParser parser)
		...
		while (!done) {
			...
			if (eventType == XmlPullParser.START_TAG) {
				...
				} else if (parser.getName().equals("vvsid")) {
					bind.setVvsid(parser.nextText());
				}
			} else if (eventType == XmlPullParser.END_TAG) {
	...
	public static UserInfoPacket parseUserInfo(XmlPullParser parser,
			Presence presence) throws Exception {
		UserInfoPacket uip = new UserInfoPacket();
		boolean done = false;
		Set<String> userInfoKeys = new HashSet<String>(
				UserInfoKey.values().length);
		for (UserInfoKey key : UserInfoKey.values()) {
			userInfoKeys.add(key.toString());
		}
		while (!done) {
			int eventType = parser.next();
			if (eventType == XmlPullParser.START_TAG) {
				if (userInfoKeys.contains(parser.getName())) {
					uip.setField(UserInfoKey.valueOf(parser.getName()),
							parser.nextText());
				}
			} else if (eventType == XmlPullParser.END_TAG) {
	...
	private static UserInfoPacket parseTeamInfo(XmlPullParser parser,
			Presence presence) throws Exception {
		UserInfoPacket uip = new UserInfoPacket();
		boolean done = false;
		Log.i("kk", "PhoneContactPacket parse");
		Set<String> userInfoKeys = new HashSet<String>(
				UserInfoKey.values().length);
		for (UserInfoKey key : UserInfoKey.values()) {
			userInfoKeys.add(key.toString());
		...
		while (!done) {
			int eventType = parser.next();
			Log.i("kk", eventType + " parseTeamInfo parse "
			...
			if (eventType == XmlPullParser.START_TAG) {
				...
				if (parser.getName().equals("times")) {
					done = true;
				}
			}
		}
