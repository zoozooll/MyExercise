...
/**
 * A listener that is fired any time a roster is changed or the presence of a user in the roster is
 * changed.
...
public interface RosterListener {
	...
	/**
	 * Called when the presence of a roster entry is changed. Care should be taken when using the
	 * presence data delivered as part of this event. Specifically, when a user account is online
	 * with multiple resources, the UI should account for that. For example, say a user is online
	 * with their desktop computer and mobile phone. If the user logs out of the IM client on their
	 * mobile phone, the user should not be shown in the roster (contact list) as offline since
	 * they're still available as another resource.
	 * <p>
	 ...
	 * Note that this listener is triggered for presence (mode) changes only (e.g presence of types
	 * available and unavailable. Subscription-related presence packets will not cause this method
	 * to be called.
