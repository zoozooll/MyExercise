...
public class Message extends Packet {
	...
	private TimeflyNotify timeflyNotify;
	private String time;
	...
	/**
	 * Returns the type of the message. If no type has been set this method will return
	 * {@link org.jivesoftware.smack.packet.Message.Type#normal}.
	...
	/**
	 * Returns the default subject of the message, or null if the subject has not been set. The
	 * subject is a short description of message contents.
	 * <p>
	 * The default subject of a message is the subject that corresponds to the message's language.
	 * (see {@link #getLanguage()}) or if no language is set to the applications default language
	 * (see {@link Packet#getDefaultLanguage()}).
	 ...
	 * will be the same as {@link #getSubject()}. Null will be returned if the language does not
	...
	/**
	 * Returns a set of all subjects in this Message, including the default message subject
	 * accessible from {@link #getSubject()}.
	...
	/**
	 * Returns the default body of the message, or null if the body has not been set. The body is
	 * the main message contents.
	 * <p>
	 * The default body of a message is the body that corresponds to the message's language. (see
	 * {@link #getLanguage()}) or if no language is set to the applications default language (see
	 ...
	 * will be the same as {@link #getBody()}. Null will be returned if the language does not have a
	 * corresponding body.
	...
	/**
	 * Returns the thread id of the message, which is a unique identifier for a sequence of "chat"
	 * messages. If no thread id is set, <tt>null</tt> will be returned.
	...
	public String toXML() {
		...
		if (time != null) {
			buf.append(" time=\"").append(StringUtils.escapeForXML(time))
					.append("\"");
		}
		if (getPacketID() != null) {
		...
		// 增加commnet字段 # baoyuedong 2015-02-02 added
		if (comment != null) {
			buf.append(comment.toXML());
		}
	...
	public enum Type {
		...
		error,
		/**
		 * 时光机新增字段 # by baoyuedong 2015-02-02 add
		 */
		comment,
		/*
		 * 时光机新增字段:时光提醒 # by baoyuedong 2015-02-02 add
		 */
		mention,
		/*点赞提醒*/
		like,
		/*
		 * 时光机新增字段：html # by baoyuedong 2015-07-22 add
		 */
		html;
	...
	public static class Comment {
		...
		public String toXML() {
			...
			append(sb, "gid", gid);
			append(sb, "jid", jid);
			append(sb, "gid_create_time", gid_create_time);
			StringBuilder sbTwo = new StringBuilder();
			append(sbTwo, "photogroup", sb.toString());
			append(sbTwo, "to_cid", to_cid);
			append(sbTwo, "cid", cid);
			append(sbTwo, "comment_time", comment_time);
			return sbTwo.toString();
		}
		public void append(StringBuilder builder, String element, String content) {
			if (TextUtils.isEmpty(element) || TextUtils.isEmpty(content)) {
				return;
			}
			builder.append("<").append(element).append(">").append(content)
					.append("</").append(element).append(">");
		}
	}

	// 时光提醒
	public static class TimeflyNotify {
		public String gid;
		public String gid_create_time;
		public String date;
		public String mention_create_time;
		...
		public String toXML() {
			...
			append(sb, "gid", gid);
			append(sb, "gid_create_time", gid_create_time);
			append(sb, "mention_create_time", mention_create_time);
			append(sb, "date", date);
			return sb.toString();
		}
		public void append(StringBuilder builder, String element, String content) {
	...
	public TimeflyNotify getTimeflyNotify() {
		if (timeflyNotify == null)
			timeflyNotify = new TimeflyNotify();
		return timeflyNotify;
	}
	public void setTimeflyNotify(TimeflyNotify timeflyNotify) {
		this.timeflyNotify = timeflyNotify;
	}
}
