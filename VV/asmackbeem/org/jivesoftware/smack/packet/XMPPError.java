...
/**
 * Represents a XMPP error sub-packet. Typically, a server responds to a request that has problems
 * by sending the packet back and including an error packet. Each error has a code, type, error
 * condition as well as as an optional text explanation. Typical errors are:
 * <p>
 * <table border=1>
 * <hr>
 * <td><b>Code</b></td>
...
public class XMPPError {
	...
	/**
	 * Creates a new error with the specified condition infering the type and code. If the Condition
	 * is predefined, client code should be like: new
	 * XMPPError(XMPPError.Condition.remote_server_timeout); If the Condition is not predefined,
	 * invocations should be like new XMPPError(new XMPPError.Condition("my_own_error"));
	...
	/**
	 * Creates a new error with the specified condition and message infering the type and code. If
	 * the Condition is predefined, client code should be like: new
	 * XMPPError(XMPPError.Condition.remote_server_timeout, "Error Explanation"); If the Condition
	 * is not predefined, invocations should be like new XMPPError(new
	...
	/**
	 * Creates a new error with the specified code, type, condition and message. This constructor is
	 * used when the condition is not recognized automatically by XMPPError i.e. there is not a
	 * defined instance of ErrorCondition or it does not applies the default specification.
	 * @param code the error code.
	...
	/**
	 * Returns an Iterator for the error extensions attached to the xmppError. An application MAY
	 * provide application-specific error information by including a properly-namespaced child in
	 * the error element.
	...
	/**
	 * Returns the first patcket extension that matches the specified element name and namespace, or
	...
	public static class Condition {
		...
		public static final Condition request_timeout = new Condition(
				"request-timeout");
		public static final Condition error_login_authentication = new Condition(
