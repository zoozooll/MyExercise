...
/**
 * Provides a mechanism to collect packets into a result queue that pass a specified filter. The
 * collector lets you perform blocking and polling operations on the result queue. So, a
 * PacketCollector is more suitable to use than a {@link PacketListener} when you need to wait for a
 * specific result.
 * <p>
 * Each packet collector will queue up a configured number of packets for processing before older
 * packets are automatically dropped. The default number is retrieved by
...
public class PacketCollector {
	/**
	 * Max number of packets that any one collector can hold. After the max is reached, older
	 * packets will be automatically dropped from the queue as new packets are added.
	 */
	 ...
	 * collector has been cancelled, it cannot be re-enabled. Instead, a new packet collector must
	 * be created.
	 */
	public void cancel() {
	 ...
	 * <tt>null</tt> if no packets are currently in the result queue.
	 * @return the next packet result, or <tt>null</tt> if there are no more results.
	 */
	...
	/**
	 * Processes a packet to see if it meets the criteria for this packet collector. If so, the
	 * packet is added to the result queue.
