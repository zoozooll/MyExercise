...
public class AccountManager {
	...
	/**
	 * Flag that indicates whether the server supports In-Band Registration. In-Band Registration
	 * may be advertised as a stream feature. If no stream feature was advertised from the server
	...
	/**
	 * Sets whether the server supports In-Band Registration. In-Band Registration may be advertised
	 * as a stream feature. If no stream feature was advertised from the server then try sending an
	 * IQ packet to discover if In-Band Registration is available.
	...
	/**
	 * Returns true if the server supports creating new accounts. Many servers require that you not
	 * be currently authenticated when creating new accounts, so the safest behavior is to only
	 * create new accounts before having logged in to a server.
	 ...
	 * attributes must be set when creating new accounts. The standard set of possible attributes
	 * are as follows:
	 * <ul>
	 ...
	 * </ul>
	 * <p>
	 * Typically, servers require no attributes when creating new accounts, or just the user's email
	 * address.
	...
	/**
	 * Returns the value of a given account attribute or <tt>null</tt> if the account attribute
	 * wasn't found.
	 ...
	 * Smack will attempt to automatically set all required attributes with blank values, which may
	 * or may not be accepted by the server. Therefore, it's recommended to check the required
	 * account attributes and to let the end-user populate them with real values instead.
	...
	public Registration createAccount(Map<String, String> attributes)
			throws XMPPException {
		Registration reg = new Registration();
		reg.setType(IQ.Type.SET);
		reg.setTo(connection.getServiceName());
		...
		PacketCollector collector = connection.createPacketCollector(filter);
		connection.sendPacket(reg);
		Registration result = (Registration) collector
