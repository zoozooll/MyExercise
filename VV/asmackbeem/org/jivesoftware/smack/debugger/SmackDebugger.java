...
public interface SmackDebugger {
	...
	/**
	 * Returns a new special Reader that wraps the new connection Reader. The connection has been
	 * secured so the connection is using a new reader and writer. The debugger needs to wrap the
	 * new reader and writer to keep being notified of the connection traffic.
	 * @return a new special Reader that wraps the new connection Reader.
	 */
	public abstract Reader newConnectionReader(Reader reader);
	/**
	 * Returns a new special Writer that wraps the new connection Writer. The connection has been
	 * secured so the connection is using a new reader and writer. The debugger needs to wrap the
	 * new reader and writer to keep being notified of the connection traffic.
	...
	/**
	 * Returns the thread that will listen for all incoming packets and write them to the GUI. This
	 * is what we call "interpreted" packet data, since it's the packet data as Smack sees it and
	 * not as it's coming in as raw XML.
	 * @return the PacketListener that will listen for all incoming packets and write them to the
	 *         GUI
	 */
	 ...
	 * @return the PacketListener that will listen for all sent packets and write them to the GUI
	 */
