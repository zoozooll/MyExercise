...
/**
 * Writes packets to a XMPP server. Packets are sent using a dedicated thread. Packet interceptors
 * can be registered to dynamically modify packets before they're actually sent. Packet listeners
 * can be registered to listen for all outgoing packets.
...
class PacketWriter {
	...
	/**
	 * Timestamp when the last stanza was sent to the server. This information is used by the keep
	...
	/**
	 * Starts the packet writer thread and opens a connection to the server. The packet writer will
	 * continue writing packets until {@link #shutdown} or an error occurs.
	 */
	public void startup() {
		writerThread.start();
	}
	/**
	 * Starts the keep alive process. A white space (aka heartbeat) is going to be sent to the
	 * server every 30 seconds (by default) since the last stanza was sent to the server.
	 */
	...
	/**
	 * Shuts down the packet writer. Once this method has been called, no further packets will be
	...
	/**
	 * Sends to the server a new stream element. This operation may be requested several times so we
	 * need to encapsulate the logic in one place. This message will be sent while doing TLS, SASL
	...
	/**
	 * A TimerTask that keeps connections to the server alive by sending a space character on an
	 * interval.
	 */
