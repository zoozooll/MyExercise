 ...
 * <li>Getting and setting global library behavior, such as the period of time to wait for replies
 * to packets from the server. Note: setting these values via the API will override settings in the
 * configuration file.
 * </ul>
 * Configuration settings are stored in META-INF/smack-config.xml (typically inside the smack.jar
 * file).
...
public final class SmackConfiguration {
	...
	private static int keepAliveInterval = 30 * 1000 * 1;
	 ...
	 * <p>
	 * So far this means that: 1) a set of classes will be loaded in order to execute their static
	 * init block 2) retrieve and set the current Smack release
	 */
	static {
		try {
	...
	/**
	 * Returns the number of milliseconds to wait for a response from the server. The default value
	 * is 5000 ms.
	 ...
	 * The default value is 30000 ms. A value of -1 mean no keep-alive requests will be sent to the
	 * server.
	 * @return the milliseconds to wait between keep-alive requests, or -1 if no keep-alive should
	 *         be sent.
	 */
	...
	/**
	 * Sets the number of milleseconds delay between sending keep-alive requests to the server. The
	 * default value is 30000 ms. A value of -1 mean no keep-alive requests will be sent to the
	 * server.
	 * @param interval the milliseconds to wait between keep-alive requests, or -1 if no keep-alive
	 *            should be sent.
	 */
	...
	/**
	 * Returns the list of SASL mechanisms to be used. If a SASL mechanism is listed here it does
	 * not guarantee it will be used. The server may not support it, or it may not be implemented.
