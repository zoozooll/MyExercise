...
import org.jivesoftware.smack.packet.Presence.BlacklistPresence;
...
import com.beem.project.btf.service.NetRequest;
 ...
 * <p>
 * <p/>
 * Others users may attempt to subscribe to this user using a subscription request. Three modes are
 * supported for handling these requests:
 * <ul>
...
public class Roster {
	 ...
	 * from other users are made. The default subscription mode is
	 * {@link SubscriptionMode#accept_all}.
	 ...
	 * subscription requests from other users are made. The default subscription mode is
	 * {@link SubscriptionMode#accept_all}.
	 * <p>
	 * <p/>
	 * If using the manual mode, a PacketListener should be registered that listens for Presence
	 ...
	 */
	public void reload() {
		//LogUtils.i("login#load the roster.", 100);
		if (!AppProperty.getInstance().isAutoRosterLoaded) {
			//如果关闭自动获取通讯录，则将rosterInitialized置为true，假冒已经拉取了，防止getRoster中wait
	...
	/**
	 * Removes a listener from this roster. The listener will be fired anytime one or more changes
	 * to the roster are pushed from the server.
	 * @param rosterListener a roster listener.
	 */
	 ...
	 * <p>
	 * <p/>
	 * Note: you must add at least one entry to the group for the group to be kept after a
	...
	public void createEntry(String user, String name, String[] groups)
		...
		RosterEntry entry = new RosterEntry(item.getJid(), item.getNickName(),
				item.getItemType(), item.getItemStatus(), item.getLoginTime(),
				item.getHeadPhoto(), item.getSignature(), item.getBday(),
				item.getSex(), this, connection);
		// Make sure the entry is in the entry list.
		if (!entries.containsKey(item.getJid())) {
			entries.put(item.getJid(), entry);
		} else {
			// If the entry was in then list then update its state with the new
			// values
			entries.put(item.getJid(), entry);
		}
	...
	protected void insertRosterItem(RosterPacket.Item item,
		...
		if (RosterPacket.ItemType.remove.equals(item.getItemType())) {
			...
			// information
			// about them.
			String key = StringUtils.parseName(item.getJid()) + "@"
					+ StringUtils.parseServer(item.getJid());
			presenceMap.remove(key);
			// Keep note that an entry has been removed
			if (deletedEntries != null) {
				deletedEntries.add(item.getJid());
			}
		} else {
			// Make sure the entry is in the entry list.
			if (!entries.containsKey(item.getJid())) {
	 ...
	 * unfiled entries or from any roster group where it could belong and will no longer be part of
	 * the roster. Note that this is an asynchronous call -- Smack must wait for the server to send
	...
	public void removeEntry(RosterEntry entry) throws XMPPException {
		...
		item.setJid(VVXMPPUtils.makeJidCompleted(item.getJid()));
		...
		//LogUtils.i("remove contact" + entry.getUser());
		entries.remove(entry.getUser());
		for (RosterGroup rosterGroup : getGroups()) {
			Collection<RosterEntry> rentries = rosterGroup.getEntries();
			for (RosterEntry rentry : rentries) {
				if (rentry.getUser().equals(entry.getUser())) {
					rentries.remove(rentry);
					return;
				}
			}
		}
		for (RosterEntry rentry : unfiledEntries) {
			if (rentry.getUser().equals(entry.getUser())) {
				unfiledEntries.remove(rentry);
				break;
			}
		}
	}
	...
	/**
	 * Returns a count of the unfiled entries in the roster. An unfiled entry is an entry that
	...
	/**
	 * Returns an unmodifiable collection for the unfiled roster entries. An unfiled entry is an
	 * entry that doesn't belong to any groups.
	 ...
	 * @param user the XMPP address of the user (eg "jsmith@example.com"). The address could be in
	 *            any valid format (e.g. "domain/resource", "user@domain" or
	...
	/**
	 * Returns the roster group with the specified name, or <tt>null</tt> if the group doesn't
	 * exist.
	 * @param name the name of the group.
	...
	/**
	 * Returns the presence info for a particular user. If the user is offline, or if no presence
	 * data is available (such as when you are not subscribed to the user's presence updates),
	 * unavailable presence will be returned.
	 * <p>
	 * <p/>
	 * If the user has several presences (one for each resource), then the presence with highest
	 * priority will be returned. If multiple presences have the same priority, the one with the
	 * "most available" presence mode will be returned. In order, that's
	 ...
	 * <p>
	 * <p/>
	 * Note that presence information is received asynchronously. So, just after logging in to the
	 * server, presence values for users in the roster may be unavailable even if they are actually
	 * online. In other words, the value returned by this method should only be treated as a
	 * snapshot in time, and may not accurately reflect other user's presence instant by instant. If
	 * you need to track presence over time, such as when showing a visual representation of the
	 * roster, consider using a {@link RosterListener}.
	 * @param user an XMPP ID. The address could be in any valid format (e.g. "domain/resource",
	 *            "user@domain" or "user@domain/resource"). Any resource information that's part of
	 *            the ID will be discarded.
	...
	/**
	 * Returns the presence info for a particular user's resource, or unavailable presence if the
	 * user is offline or if no presence information is available, such as when you are not
	 ...
	 *         unavailable presence if the user is offline or if no presence information is
	 *         available.
	 */
	...
	/**
	 * Returns the key to use in the presenceMap for a fully qualified XMPP ID. The roster can
	 * contain any valid address format such us "domain/resource", "user@domain" or
	...
	public enum SubscriptionMode {
		/**
		 * Automatically accept all subscription and unsubscription requests. This is the default
		 * mode and is suitable for simple client. More complex client will likely wish to handle
		...
		reject_all,
		/**
		 * Subscription requests are ignored, which means they must be manually processed by
		 * registering a listener for presence packets and then looking for any presence requests
		 * that have the type Presence.Type.SUBSCRIBE or Presence.Type.UNSUBSCRIBE.
		 */
		manual
	}
	...
	private class PresencePacketListener implements PacketListener {
		...
		public void processPacket(Packet packet) {
			if (processCustomPresence(packet)) {
				return;
			}
			...
			} else if (presence.getType() == Presence.Type.add) {
			} else if (presence.getType() == Presence.Type.remove) {
				String user = StringUtils.parseName(from);
				// addDefriendBy(user);
			}
		}
		private boolean processCustomPresence(Packet packet) {
			Presence presence = (Presence) packet;
			if (presence instanceof BlacklistPresence) {
				BlacklistPresence blp = (BlacklistPresence) presence;
				String action = blp.getAction();
				ArrayList<String> itemUsers = blp.getItemUsers();
				if (action.equals("add")) {
					// addBlacklisted(itemUsers);
				} else if (action.equals("remove")) {
					// removeBlacklisted(itemUsers);
				}
				return true;
			}
			return false;
	...
	/**
	 * Listen for empty IQ results which indicate that the client has already a current roster
	 * version
	 * @author Till Klocke
	 */
			...
			if (persistentStorage != null) {
				for (RosterPacket.Item i : rosterPacket.getRosterItems()) {
					if (i.getItemType().equals(RosterPacket.ItemType.remove)) {
						persistentStorage.removeEntry(i.getJid());
					} else {
	...
	/**
	 * send request packet to server, the response time depends on network rate
	 */
	protected Packet request(Packet req) throws XMPPException {
		PacketCollector collector = connection
				.createPacketCollector(new PacketIDFilter(req.getPacketID()));
		connection.sendPacket(req);
		Packet response = collector.nextResult(NetRequest.ASYN_REQ_WAIT_TIME);
		collector.cancel();
		if (response == null) {
			try {
	...
	// ////////////////////////////////////////////////////////////////////
	// Notice:the following functions are not thread safe without mutex lock,
	// so they should be called in the main thread .
	// ////////////////////////////////////////////////////////////////////
	public List<Contact> getBlacklist() throws XMPPException {
		return ContactService.getInstance().getBlacklist();
	}
	// 删除黑名单
	public PacketResult removeBlacklist(String[] jids) throws XMPPException {
		return ContactService.getInstance().removeBlacklist(jids);
	}
	// 添加黑名单
	public PacketResult addBlacklist(String[] jids) throws XMPPException {
		return ContactService.getInstance().addBlacklist(jids);
	}
	// 获取联系人信息
	public Contact getContact(String jid, boolean... isReload) {
		return ContactService.getInstance().getContact(jid, isReload);
	}
	public boolean blackYet(String user) {
		return ContactService.getInstance().blackYet(user);
	}
	public boolean friendYet(String user) {
		return ContactService.getInstance().friendYet(user);
	}
	// 添加好友
	public PacketResult addFriend(String jid, String content, String operation)
			throws XMPPException {
		return ContactService.getInstance().addFriend(jid, content, operation);
	}
	// 删除好友
	public PacketResult removeFriend(String jid) throws XMPPException {
		return ContactService.getInstance().removeFriend(jid);
	}
	public List<Contact> getFriendlist(boolean... isReload) {
		return ContactService.getInstance().getFriendlist(isReload);
	}
	// 修改好友备注
	public PacketResult modifyFriendAlias(String jid, String alias)
			throws XMPPException {
		return ContactService.getInstance().modifyFriendAlias(jid, alias);
	}
	public PacketResult modifyContactInfo(Map<UserInfoKey, String> modifyMap)
			throws XMPPException {
		return ContactService.getInstance().modifyContactInfo(modifyMap);
	}
}
