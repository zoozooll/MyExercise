 ...
 * {@link FileTransferManager#createIncomingFileTransfer(FileTransferRequest)} method is invoked. It
 * is a file being sent to the local user from another user on the jabber network. There are two
 * stages of the file transfer to be concerned with and they can be handled in different ways
 * depending upon the method that is invoked on this class.
 * <p/>
 * The first way that a file is recieved is by calling the {@link #recieveFile()} method. This
 * method, negotiates the appropriate stream method and then returns the <b><i>InputStream</b></i>
 * to read the file data from.
 * <p/>
 * The second way that a file can be recieved through this class is by invoking the
 * {@link #recieveFile(File)} method. This method returns immediatly and takes as its parameter a
 * file on the local file system where the file recieved from the transfer will be put.
...
public class IncomingFileTransfer extends FileTransfer {
	...
	/**
	 * Negotiates the stream method to transfer the file over and then returns the negotiated
	 * stream.
