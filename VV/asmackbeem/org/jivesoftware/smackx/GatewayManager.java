...
/**
 * This class is the general entry point to gateway interaction (XEP-0100). This class discovers
 * available gateways on the users servers, and can give you also a list of gateways the you user is
 * registered with which are not on his server. All actual interaction with a gateway is handled in
 * the class {@see Gateway}.
 * @author Till Klocke
 */
public class GatewayManager {
	...
	/**
	 * Discovers {@link DiscoveryInfo} and {@link DiscoveryInfo.Identity} of a gateway and creates a
	 * {@link Gateway} object representing this gateway.
	 * @param itemJID
	 * @throws XMPPException
	 */
	...
	/**
	 * Returns an instance of GatewayManager for the given connection. If no instance for this
	 * connection exists a new one is created and stored in a Map.
	 * @param connection
	...
	/**
	 * Returns a list of gateways which are offered by the users server, wether the user is
	 * registered to them or not.
	...
	/**
	 * Returns a list of gateways the user has in his roster, but which are offered by remote
	 * servers. But note that this list isn't automatically refreshed. You have to refresh is
	 * manually if needed.
	 * @return a list of gateways
	 * @throws XMPPException
	 */
	...
	/**
	 * Returns a Gateway object for a given JID. Please note that it is not checked if the JID
	 * belongs to valid gateway. If this JID doesn't belong to valid gateway all operations on this
	 * Gateway object should fail with a XMPPException. But there is no guarantee for that.
	 * @param entityJID
