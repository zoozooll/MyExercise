...
/**
 * An AdHocCommandManager is responsible for keeping the list of available commands offered by a
 * service and for processing commands requests. Pass in a Connection instance to
 * {@link #getAddHocCommandsManager(org.jivesoftware.smack.Connection)} in order to get an instance
 * of this class.
...
public class AdHocCommandManager {
	...
	/**
	 * Map a Connection with it AdHocCommandManager. This map have a key-value pair for every active
	 * connection.
	 */
	private static Map<Connection, AdHocCommandManager> instances = new ConcurrentHashMap<Connection, AdHocCommandManager>();
	/**
	 * Register the listener for all the connection creations. When a new connection is created a
	 * new AdHocCommandManager is also created and related to that connection.
	 */
	static {
		Connection
	...
	/**
	 * Map a command node with its AdHocCommandInfo. Note: Key=command node, Value=command. Command
	 * node matches the node attribute sent by command requesters.
	 */
	private Map<String, AdHocCommandInfo> commands = Collections
			.synchronizedMap(new WeakHashMap<String, AdHocCommandInfo>());
	/**
	 * Map a command session ID with the instance LocalCommand. The LocalCommand is the an objects
	 * that has all the information of the current state of the command execution. Note: Key=session
	 * ID, Value=LocalCommand. Session ID matches the sessionid attribute sent by command
	 * responders.
	 */
	 ...
	 * command manager. The <tt>name</tt> is the human readable name of the command. The
	 * <tt>class</tt> is the class of the command, which must extend {@link LocalCommand} and have a
	 * default constructor.
	...
	/**
	 * Returns a command that represents an instance of a command in a remote host. It is used to
	 * execute remote commands. The concept is similar to RMI. Every invocation on this command is
	 * equivalent to an invocation in the remote command.
	...
	private void processAdHocCommand(AdHocCommandData requestData) {
		...
		if (sessionId == null) {
			...
			/*
			 * Since the requester could send two requests for the same executing command i.e. the
			 * same session id, all the execution of the action must be synchronized to avoid
			 * inconsistencies.
			 */
