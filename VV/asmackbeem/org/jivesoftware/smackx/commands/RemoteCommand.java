 ...
 * {@link AdHocCommand.Action#complete complete} actions results in executing that action in the
 * remote location. In response to that action the internal state of the this command instance will
 * change. For example, if the command is a single stage command, then invoking the execute action
 * will execute this action in the remote location. After that the local instance will have a state
 * of "completed" and a form or notes that applies.
...
public class RemoteCommand extends AdHocCommand {
	...
	/**
	 * The number of milliseconds to wait for a response from the server The default value is the
	 * default packet reply timeout (5000 ms).
	 */
	private long packetReplyTimeout;

	/**
	 * Creates a new RemoteCommand that uses an specific connection to execute a command identified
	 * by <code>node</code> in the host identified by <code>jid</code>
	...
	/**
	 * Executes the default action of the command with the information provided in the Form. This
	 * form must be the anwser form of the previous stage. If there is a problem executing the
	 * command it throws an XMPPException.
	...
	/**
	 * Executes the <code>action</codo> with the <code>form</code>. The action could be any of the
	 * available actions. The form must be the anwser of the previous stage. It can be <tt>null</tt>
	 ...
	 * {@link SmackConfiguration#getPacketReplyTimeout default} value should be adjusted for
	 * commands that can take a long time to execute.
