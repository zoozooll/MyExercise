...
/**
 * A DiscoverItems IQ packet, which is used by XMPP clients to request and receive items associated
 * with XMPP entities.
 * <p>
 * The items could also be queried in order to discover if they contain items inside. Some items may
 * be addressable by its JID and others may require to be addressed by a JID and a node name.
...
public class DiscoverItems extends IQ {
	...
	/**
	 * An item is associated with an XMPP Entity, usually thought of a children of the parent entity
	 * and normally are addressable as a JID.
	 * <p>
	 * An item associated with an entity may not be addressable as a JID. In order to handle such
	 * items, Service Discovery uses an optional 'node' attribute that supplements the 'jid'
	 * attribute.
	 */
	public static class Item {
		 ...
		 * <p>
		 * Node attributes SHOULD be used only when trying to provide or query information which is
		...
		/**
		 * Sets the node attribute that supplements the 'jid' attribute. A node is merely something
		 * that is associated with a JID and for which the JID can provide information.
		 * <p>
		 * Node attributes SHOULD be used only when trying to provide or query information which is
		 * not directly addressable.
