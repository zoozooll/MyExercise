...
/**
 * Manages private data, which is a mechanism to allow users to store arbitrary XML data on an XMPP
 * server. Each private data chunk is defined by a element name and XML namespace. Example private
 * data:
 * 
 * <pre>
 ...
 * </pre>
 * 
 * {@link PrivateDataProvider} instances are responsible for translating the XML into objects. If no
 * PrivateDataProvider is registered for a given element name and namespace, then a
 * {@link DefaultPrivateData} instance will be returned.
 * <p>
 * Warning: this is an non-standard protocol documented by <a
 * href="http://www.jabber.org/jeps/jep-0049.html">JEP-49</a>. Because this is a non-standard
...
public class PrivateDataManager {
	 ...
	 * For example, if a provider was registered to the element name "prefs" and the namespace
	 * "http://www.xmppclient.com/prefs", then the following packet would trigger the provider:
	 * 
	 * <pre>
	...
	/**
	 * The user to get and set private data for. In most cases, this value should be <tt>null</tt>,
	 * as the typical use of private data is to get and set your own private data and not others.
	 */
	private String user;

	/**
	 * Creates a new private data manager. The connection must have undergone a successful login
	 * before being used to construct an instance of this class.
	...
	/**
	 * Creates a new private data manager for a specific user (special case). Most servers only
	 * support getting and setting private data for the user that authenticated via the connection.
	 * However, some servers support the ability to get and set private data for other users (for
	 * example, if you are the administrator). The connection must have undergone a successful login
	 * before being used to construct an instance of this class.
	 * @param connection an XMPP connection which must have already undergone a successful login.
	...
	/**
	 * Sets a private data value. Each chunk of private data is uniquely identified by an element
	 * name and namespace pair. If private data has already been set with the element name and
	 * namespace, then the new private data will overwrite the old value.
