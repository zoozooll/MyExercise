...
/**
 * The PEPProvider parses incoming PEPEvent packets. (XEP-163 has a weird asymmetric deal: outbound
 * PEP are <iq> + <pubsub> and inbound are <message> + <event>. The provider only deals with
 * inbound, and so it only deals with <message>. Anyhoo... The way this works is that PEPxxx classes
 * are generic <pubsub> and <message> providers, and anyone who wants to publish/receive PEPs, such
 * as <tune>, <geoloc>, etc., simply need to extend PEPItem and register (here) a
...
public class PEPProvider implements PacketExtensionProvider {
	...
	/**
	 * Creates a new PEPProvider. ProviderManager requires that every PacketExtensionProvider has a
