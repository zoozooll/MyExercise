...
public class DelayInformationProvider implements PacketExtensionProvider {
	/*
	 * Date format used to parse dates in the XEP-0091 format but missing leading zeros for month
	 * and day.
	 */
	...
	/**
	 * Creates a new DeliveryInformationProvider. ProviderManager requires that every
	...
	public PacketExtension parseExtension(XmlPullParser parser)
		...
		/*
		 * if date could not be parsed but XML is valid, don't shutdown connection by throwing an
		 * exception instead set timestamp to current time
		 */
		if (stamp == null) {
			stamp = new Date();
		}
		...
		/*
		 * parser.nextText() returns empty string if there is no reason. DelayInformation API
		 * specifies that null should be returned in that case.
		 */
