...
/**
 * Represents a list of conversation transcripts that a user had in all his history. Each transcript
 * summary includes the sessionID which may be used for getting more detailed information about the
 * conversation. {@link org.jivesoftware.smackx.workgroup.packet.Transcript}
...
public class Transcripts extends IQ {
	...
	/**
	 * Returns the id of the user that was involved in the conversations. The userID could be a real
	 * JID if the connected user was not anonymous. Otherwise, the userID will be a String that was
	 * provided by the anonymous user as a way to idenitify the user across many user sessions.
	 ...
	 * transcript but some summary information like the sessionID and the time when the conversation
	 * started and finished. Once you have the sessionID it is possible to get the full conversation
	 * transcript.
	...
	public static class TranscriptSummary {
		...
		/**
		 * Returns a list of AgentDetails. For each Agent that was involved in the conversation the
		 * list will include an AgentDetail. An AgentDetail contains the JID of the agent as well as
		 * the time when the Agent joined and left the conversation.
