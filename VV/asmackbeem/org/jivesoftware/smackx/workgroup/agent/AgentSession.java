...
/**
 * This class embodies the agent's active presence within a given workgroup. The application should
 * have N instances of this class, where N is the number of workgroups to which the owning agent of
 * the application belongs. This class provides all functionality that a session within a given
 * workgroup is expected to have from an agent's perspective -- setting the status, tracking the
 * status of queues to which the agent belongs within the workgroup, and dequeuing customers.
 * @author Matt Tucker
 * @author Derek DeMoro
 */
public class AgentSession {
	...
	/**
	 * Constructs a new agent session instance. Note, the {@link #setOnline(boolean)} method must be
	 * called with an argument of <tt>true</tt> to mark the agent as available to accept chat
	 * requests.
	...
	/**
	 * Close the agent session. The underlying connection will remain opened but the packet
	 * listeners that were added by this agent session will be removed.
	 */
	public void close() {
	...
	/**
	 * Allows the addition of a new key-value pair to the agent's meta data, if the value is new
	 * data, the revised meta data will be rebroadcast in an agent's presence broadcast.
	...
	public void setMetaData(String key, String val) throws XMPPException {
		synchronized (this.metaData) {
			String oldVal = this.metaData.get(key);
	...
	public void removeMetaData(String key) throws XMPPException {
		synchronized (this.metaData) {
			String oldVal = metaData.remove(key);
			if (oldVal != null) {
				setStatus(presenceMode, maxChats);
			}
		}
	 ...
	 * @param key the meta data key
	 * @return the meta data value associated with the key or <tt>null</tt> if the meta-data doesn't
	 *         exist..
	 */
	...
	/**
	 * Sets whether the agent is online with the workgroup. If the user tries to go online with the
	 * workgroup but is not allowed to be an agent, an XMPPError with error code 401 will be thrown.
	...
	/**
	 * Sets the agent's current status with the workgroup. The presence mode affects how offers are
	 * routed to the agent. The possible presence modes with their meanings are as follows:
	 * <ul>
	 * <p/>
	 * <li>Presence.Mode.AVAILABLE -- (Default) the agent is available for more chats (equivalent to
	 * Presence.Mode.CHAT).
	 * <li>Presence.Mode.DO_NOT_DISTURB -- the agent is busy and should not be disturbed. However,
	 * special case, or extreme urgency chats may still be offered to the agent.
	 * <li>Presence.Mode.AWAY -- the agent is not available and should not have a chat routed to
	 * them (equivalent to Presence.Mode.EXTENDED_AWAY).
	 * </ul>
	 * <p/>
	 ...
	 * <p/>
	 * The agent is not guaranteed of having privileges to perform this action; an exception denying
	 * the request may be thrown.
	...
	/**
	 * Returns the Form to use for searching transcripts. It is unlikely that the server will change
	 * the form (without a restart) so it is safe to keep the returned form for future submissions.
	...
	/**
	 * Submits the completed form and returns the result of the transcript search. The result will
	 * include all the data returned from the server so be careful with the amount of data that the
	 * search may return.
	 ...
	 * information will include the real JID of the occupants, the nickname of the user in the room
	 * as well as the date when the user joined the room.
	 ...
	 * a user, an agent, a queue or a workgroup. In the case of a queue or a workgroup the workgroup
	 * service will decide the best agent to receive the invitation.
	 * <p>
	 * This method will return either when the service returned an ACK of the request or if an error
	 * occured while requesting the invitation. After sending the ACK the service will send the
	 * invitation to the target entity. When dealing with agents the common sequence of
	 * offer-response will be followed. However, when sending an invitation to a user a standard MUC
	 * invitation will be sent.
	 * <p>
	 * The agent or user that accepted the offer <b>MUST</b> join the room. Failing to do so will
	 * make the invitation to fail. The inviter will eventually receive a message error indicating
	 * that the invitee accepted the offer but failed to join the room. Different situations may
	 * lead to a failed invitation. Possible cases are: 1) all agents rejected the offer and ther
	 * are no agents available, 2) the agent that accepted the offer failed to join the room or 2)
	 * the user that received the MUC invitation never replied or joined the room. In any of these
	 * cases (or other failing cases) the inviter will get an error message with the failed
	 * notification.
	 ...
	 * @throws XMPPException if the sender of the invitation is not an agent or the service failed
	...
	/**
	 * Transfer an existing session support to another user or agent. The provided invitee's JID can
	 * be of a user, an agent, a queue or a workgroup. In the case of a queue or a workgroup the
	 * workgroup service will decide the best agent to receive the invitation.
	 * <p>
	 * This method will return either when the service returned an ACK of the request or if an error
	 * occured while requesting the transfer. After sending the ACK the service will send the
	 * invitation to the target entity. When dealing with agents the common sequence of
	 * offer-response will be followed. However, when sending an invitation to a user a standard MUC
	 * invitation will be sent.
	 * <p>
	 * Once the invitee joins the support room the workgroup service will kick the inviter from the
	 * room.
	 * <p>
	 * Different situations may lead to a failed transfers. Possible cases are: 1) all agents
	 * rejected the offer and there are no agents available, 2) the agent that accepted the offer
	 * failed to join the room or 2) the user that received the MUC invitation never replied or
	 * joined the room. In any of these cases (or other failing cases) the inviter will get an error
	 ...
	 * @param query an optional query object used to tell the server what metadata to retrieve. This
	 *            can be null.
	 * @throws XMPPException if an error occurs while sending the request to the server.
