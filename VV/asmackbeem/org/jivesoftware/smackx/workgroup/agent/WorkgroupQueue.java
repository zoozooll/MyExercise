...
/**
 * A queue in a workgroup, which is a pool of agents that are routed a specific type of chat
 * request.
 */
public class WorkgroupQueue {
	...
	/**
	 * Returns an Iterator for the users in the queue waiting to be routed to an agent (QueueUser
	 * instances).
	 ...
	 * If average wait time info isn't available, -1 will be returned.
	...
	/**
	 * Returns the date of the oldest request waiting in the queue. If there are no requests waiting
	 * to be routed, this method will return <tt>null</tt>.
	 ...
	 * <li>WorkgroupQueue.Status.ACTIVE -- the queue is active but NOT accepting new chat requests.
	 * <li>WorkgroupQueue.Status.CLOSED -- the queue is NOT active and NOT accepting new chat
	 * requests.
	 * </ul>
	 */
	public static class Status {
		...
		/**
		 * The queue is active but NOT accepting new chat requests. This state might occur when the
		 * workgroup has closed because regular support hours have closed, but there are still
		 * several requests left in the queue.
		 */
		...
		/**
		 * Converts a String into the corresponding status. Valid String values that can be
		 * converted to a status are: "open", "active", and "closed".
