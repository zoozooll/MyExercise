...
/**
 * Provides workgroup services for users. Users can join the workgroup queue, depart the queue, find
 * status information about their placement in the queue, and register to be notified when they are
 * routed to an agent.
 * <p>
 * <p/>
 * This class only provides a users perspective into a workgroup and is not intended for use by
 * agents.
 * @author Matt Tucker
 * @author Derek DeMoro
 */
public class Workgroup {
	 ...
	 * support@workgroup.example.com) and XMPP connection. The connection must have undergone a
	...
	public Workgroup(String workgroupJID, Connection connection) {
		...
		/**
		 * Internal handling of an invitation.Recieving an invitation removes the user from the
		 * queue.
		 */
	...
	/**
	 * Returns the users current position in the workgroup queue. A value of 0 means the user is
	 * next in line to be routed; therefore, if the queue position is being displayed to the end
	 * user it is usually a good idea to add 1 to the value this method returns before display. If
	 * the user is not currently waiting in the workgroup, or no queue position information is
	 * available, -1 will be returned.
	 * @return the user's current position in the workgroup queue, or -1 if the position isn't
	 *         available or if the user isn't in the queue.
	 */
	public int getQueuePosition() {
		return queuePosition;
	}
	/**
	 * Returns the estimated time (in seconds) that the user has to left wait in the workgroup queue
	 * before being routed. If the user is not currently waiting int he workgroup, or no queue time
	 * information is available, -1 will be returned.
	 * @return the estimated time remaining (in seconds) that the user has to wait inthe workgroupu
	 *         queue, or -1 if time information isn't available or if the user isn't int the queue.
	 */
	 ...
	 * </ul>
	 * <p/>
	 * A user cannot request to join the queue again if already in the queue. Therefore, this method
	 * will throw an IllegalStateException if the user is already in the queue.
	 * <p>
	 * <p/>
	 * Some servers may be configured to require certain meta-data in order to join the queue. In
	 * that case, the {@link #joinQueue(Form)} method should be used instead of this method so that
	 * meta-data may be passed in.
	 * <p>
	 * <p/>
	 * The server tracks the conversations that a user has with agents over time. By default, that
	 * tracking is done using the user's JID. However, this is not always possible. For example,
	 * when the user is logged in anonymously using a web client. In that case the user ID might be
	 * a randomly generated value put into a persistent cookie or a username obtained via the
	 * session. A userID can be explicitly passed in by using the {@link #joinQueue(Form, String)}
	 * method. When specified, that userID will be used instead of the user's JID to track
	 * conversations. The server will ignore a manually specified userID if the user's connection to
	 * the server is not anonymous.
	 * @throws XMPPException if an error occured joining the queue. An error may indicate that a
	 *             connection failure occured or that the server explicitly rejected the request to
	 *             join the queue.
	 */
	 ...
	 * when the user is logged in anonymously using a web client. In that case the user ID might be
	 * a randomly generated value put into a persistent cookie or a username obtained via the
	 * session. When specified, that userID will be used instead of the user's JID to track
	 * conversations. The server will ignore a manually specified userID if the user's connection to
	 * the server is not anonymous.
	...
	/**
	 * Adds a queue listener that will be notified of queue events for the user that created this
	 * Workgroup instance.
	...
	/**
	 * Adds an invitation listener that will be notified of groupchat invitations from the workgroup
	 * for the the user that created this Workgroup instance.
	...
	/*
	 * public static void main(String args[]) throws Exception { Connection con = new
	 * XMPPConnection("anteros"); con.connect(); con.loginAnonymously(); Workgroup workgroup = new
	 * Workgroup("demo@workgroup.anteros", con); WorkgroupProperties props =
	 * workgroup.getWorkgroupProperties("derek@anteros.com"); System.out.print(props);
	 * con.disconnect(); }
	 */
}
