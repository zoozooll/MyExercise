...
/**
 * A decorator for a {@link Form} to easily enable reading and updating of node configuration. All
 * operations read or update the underlying {@link DataForm}.
 * <p>
 * Unlike the {@link Form}.setAnswer(XXX)} methods, which throw an exception if the field does not
 * exist, all <b>ConfigureForm.setXXX</b> methods will create the field in the wrapped form if it
 * does not already exist.
 * @author Robin Collier
 */
public class ConfigureForm extends Form {
	/**
	 * Create a decorator from an existing {@link DataForm} that has been retrieved from parsing a
	...
	/**
	 * Create a decorator from an existing {@link Form} for node configuration. Typically, this can
	 * be used to create a decorator for an answer form by using the result of
	 * {@link #createAnswerForm()} as the input parameter.
	...
	/**
	 * Create a new form for configuring a node. This would typically only be used when creating and
	 * configuring a node at the same time via {@link PubSubManager#createNode(String, Form)}, since
	 * configuration of an existing node is typically accomplished by calling
	 * {@link LeafNode#getNodeConfiguration()} and using the resulting form to create a answer form.
	 * See {@link #ConfigureForm(Form)}.
	 * @param formType
	 */
	...
	/**
	 * Set the URL of an XSL transformation which can be applied to payloads in order to generate an
	 ...
	 * with the collection node. This is only relevant if {@link #getChildrenAssociationPolicy()} is
	 * set to {@link ChildrenAssociationPolicy#whitelist}.
	...
	/**
	 * Gets the URL of an XSL transformation which can be applied to the payload format in order to
	 * generate a valid Data Forms result that the client could display using a generic Data Forms
	 * rendering engine.
	...
	/**
	 * Sets the URL of an XSL transformation which can be applied to the payload format in order to
	 * generate a valid Data Forms result that the client could display using a generic Data Forms
	 * rendering engine.
