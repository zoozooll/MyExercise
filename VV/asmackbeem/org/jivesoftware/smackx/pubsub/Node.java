...
abstract public class Node {
	...
	/**
	 * Some XMPP servers may require a specific service to be addressed on the server. For example,
	 * OpenFire requires the server to be prefixed by <b>pubsub</b>
	 */
	...
	/**
	 * Returns a configuration form, from which you can create an answer form to be submitted via
	 * the {@link #sendConfigurationForm(Form)}.
	...
	/**
	 * The user subscribes to the node using the supplied jid. The bare jid portion of this one must
	 * match the jid for the connection. Please note that the {@link Subscription.State} should be
	 * checked on return since more actions may be required by the caller.
	 * {@link Subscription.State#pending} - The owner must approve the subscription request before
	 * messages will be received. {@link Subscription.State#unconfigured} - If the
	 * {@link Subscription#isConfigRequired()} is true, the caller must configure the subscription
	 * before messages will be received. If it is false the caller can configure it but is not
	 * required to do so.
	...
	/**
	 * The user subscribes to the node using the supplied jid and subscription options. The bare jid
	 * portion of this one must match the jid for the connection. Please note that the
	 * {@link Subscription.State} should be checked on return since more actions may be required by
	 * the caller. {@link Subscription.State#pending} - The owner must approve the subscription
	 * request before messages will be received. {@link Subscription.State#unconfigured} - If the
	 * {@link Subscription#isConfigRequired()} is true, the caller must configure the subscription
	 * before messages will be received. If it is false the caller can configure it but is not
	 * required to do so.
	 * @param jid The jid to subscribe as.
	...
	/**
	 * Remove the subscription related to the specified JID. This will only work if there is only 1
	 * subscription. If there are multiple subscriptions, use {@link #unsubscribe(String, String)}.
	...
	/**
	 * Returns a SubscribeForm for subscriptions, from which you can create an answer form to be
	 * submitted via the {@link #sendConfigurationForm(Form)}.
	...
	/**
	 * Register a listener for item publication events. This listener will get called whenever an
	 * item is published to this node.
	...
	/**
	 * Register a listener for configuration events. This listener will get called whenever the
	...
	/**
	 * This class translates low level item publication events into api level objects for user
	 * consumption.
	...
	/**
	 * This class translates low level item deletion events into api level objects for user
	 * consumption.
	 * @author Robin Collier
	 */
	...
	/**
	 * This class translates low level node configuration events into api level objects for user
	 * consumption.
	 * @author Robin Collier
	 */
	...
	/**
	 * Filter for {@link PacketListener} to filter out events not specific to the event type
	 * expected for this node.
	 * @author Robin Collier
	 */
