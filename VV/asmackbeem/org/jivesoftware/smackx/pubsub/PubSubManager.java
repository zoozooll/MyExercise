...
/**
 * This is the starting point for access to the pubsub service. It will provide access to general
 * information about the service, as well as create or retrieve pubsub {@link LeafNode} instances.
 * These instances provide the bulk of the functionality as defined in the pubsub specification <a
...
public class PubSubManager {
	 ...
	 * require a specific to address for packets.
	 * @param connection The XMPP connection
	 ...
	 * @param id The id of the node, which must be unique within the pubsub service
	 * @return The node that was created
	 * @exception XMPPException
	 */
	...
	/**
	 * Creates a node with specified configuration. Note: This is the only way to create a
	 * collection node.
	...
	/**
	 * Get all the nodes that currently exist as a child of the specified collection node. If the
	 * service does not support collection nodes then all nodes will be returned. To retrieve
	 * contents of the root collection node (if it exists), or there is no root collection node,
	 * pass null as the nodeId.
	 * @param nodeId - The id of the collection node for which the child nodes will be returned.
	...
	/**
	 * Gets the supported features of the servers pubsub implementation as a standard
	 * {@link DiscoverInfo} instance.
