...
/**
 * A <code>RoomListenerMultiplexor</code> multiplexes incoming packets on a <code>Connection</code>
 * using a single listener/filter pair. A single <code>RoomListenerMultiplexor</code> is created for
 * each {@link org.jivesoftware.smack.Connection} that has joined MUC rooms within its session.
...
class RoomListenerMultiplexor implements ConnectionListener {
	...
	/**
	 * All access should be through the static method {@link #getRoomMultiplexor(Connection)}.
	 */
	 ...
	 * connection gets closed. As soon as a room invitation is received the invitations listeners
	 * will be fired. When the connection gets closed the monitor will remove his listeners on the
	 * connection.
	 */
	public void init() {
	...
	/**
	 * The single <code>Connection</code>-level <code>PacketFilter</code> used by a
	 * {@link RoomListenerMultiplexor} for all muc chat rooms on an <code>Connection</code>. Each
	 * time a muc chat room is added to/removed from an <code>Connection</code> the address for that
	 * chat room is added to/removed from that <code>Connection</code>'s
	...
	/**
	 * The single <code>Connection</code>-level <code>PacketListener</code> used by a
	 * {@link RoomListenerMultiplexor} for all muc chat rooms on an <code>Connection</code>. Each
	 * time a muc chat room is added to/removed from an <code>Connection</code> the address and
	 * listener for that chat room are added to/removed from that <code>Connection</code>'s
