...
/**
 * A MultiUserChat is a conversation that takes place among many users in a virtual room. A room
 * could have many occupants with different affiliation and roles. Possible affiliatons are "owner",
 * "admin", "member", and "outcast". Possible roles are "moderator", "participant", and "visitor".
 * Each role and affiliation guarantees different privileges (e.g. Send messages to all occupants,
 * Kick participants and visitors, Grant voice, Edit member list, etc.).
...
public class MultiUserChat {
	 ...
	 * information is sent to or received from the server until you attempt to {@link #join(String)
	 * join} the chat room. On some server implementations, the room will not be created until the
	 * first person joins it.
	 * <p>
	 * Most XMPP servers use a sub-domain for the chat service (eg chat.example.com for the XMPP
	 * server example.com). You must ensure that the room address you're trying to connect to
	 * includes the proper chat sub-domain.
	 * @param connection the XMPP connection.
	 * @param room the name of the room in the form "roomName@service", where "service" is the
	 *            hostname at which the multi-user chat service is running. Make sure to provide a
	 *            valid JID.
	 */
	...
	/**
	 * Returns an Iterator on the rooms where the user has joined using a given connection. The
	 * Iterator will contain Strings where each String represents a room (e.g. room@muc.jabber.org).
	 ...
	 * the room. Once the completed configutation form is sent to the server, the server will unlock
	 * the room. {@link #sendConfigurationForm(Form)}
	 * @param nickname the nickname to use.
	 * @throws XMPPException if the room couldn't be created for some reason (e.g. room already
	 *             exists; user already joined to an existant room or 405 error if the user is not
	 *             allowed to create the room)
	 */
	...
	/**
	 * Joins the chat room using the specified nickname. If already joined using another nickname,
	 * this method will first leave the room and then re-join using the new nickname. The default
	 * timeout of Smack for a reply from the group chat server that the join succeeded will be used.
	 * After joining the room, the room will decide the amount of history to send.
	 * @param nickname the nickname to use.
	 * @throws XMPPException if an error occurs joining the room. In particular, a 401 error can
	 *             occur if no password was provided and one is required; or a 403 error can occur
	 *             if the user is banned; or a 404 error can occur if the room does not exist or is
	 *             locked; or a 407 error can occur if user is not on the member list; or a 409
	 *             error can occur if someone is already in the group chat with the same nickname.
	 */
	public void join(String nickname) throws XMPPException {
		join(nickname, null, null, SmackConfiguration.getPacketReplyTimeout());
	}
	/**
	 * Joins the chat room using the specified nickname and password. If already joined using
	 * another nickname, this method will first leave the room and then re-join using the new
	 * nickname. The default timeout of Smack for a reply from the group chat server that the join
	 * succeeded will be used. After joining the room, the room will decide the amount of history to
	 * send.
	 * <p>
	 * A password is required when joining password protected rooms. If the room does not require a
	 * password there is no need to provide one.
	 * @param nickname the nickname to use.
	 ...
	 * nickname.
	 * <p>
	 * To control the amount of history to receive while joining a room you will need to provide a
	 * configured DiscussionHistory object.
	 * <p>
	 * A password is required when joining password protected rooms. If the room does not require a
	 * password there is no need to provide one.
	 * <p>
	 * If the room does not already exist when the user seeks to enter it, the server will decide to
	 * create a new room or not.
	 * @param nickname the nickname to use.
	 * @param password the password to use.
	...
	/**
	 * Returns true if currently in the multi user chat (after calling the {@link #join(String)}
	 * method).
	...
	/**
	 * Returns the room's configuration form that the room's owner can use or <tt>null</tt> if no
	 * configuration is possible. The configuration form allows to set the room's language, enable
	 * logging, specify room's type, etc..
	...
	/**
	 * Sends the completed configuration form to the server. The room will be configured with the
	 * new settings defined in the form. If the form is empty then the server will create an instant
	 * room (will use default configuration).
	...
	/**
	 * Returns the room's registration form that an unaffiliated user, can use to become a member of
	 * the room or <tt>null</tt> if no registration is possible. Some rooms may restrict the
	 ...
	 * (e.g. because that privilege has been restricted), the room will return a "Not Allowed" error
	 * to the user (error code 405).
	 ...
	 * @throws XMPPException if an error occurs asking the registration form for the room or a 405
	 *             error if the user is not allowed to register with the room.
	 */
	...
	/**
	 * Sends the completed registration form to the server. After the user successfully submits the
	 * form, the room may queue the request for review by the room admins or may immediately add the
	 * user to the member list by changing the user's affiliation from "none" to "member.
	 * <p>
	 * If the desired room nickname is already reserved for that room, the room will return a
	 * "Conflict" error to the user (error code 409). If the room does not support registration, it
	 * will return a "Service Unavailable" error to the user (error code 503).
	 ...
	 *             409 error can occur if the desired room nickname is already reserved for that
	 *             room; or a 503 error can occur if the room does not support registration.
	 */
	...
	/**
	 * Sends a request to the server to destroy the room. The sender of the request should be the
	 * room's owner. If the sender of the destroy request is not the room's owner then the server
	 * will answer a "Forbidden" error (403).
	 * @param reason the reason for the room destruction.
	 * @param alternateJID the JID of an alternate location.
	 * @throws XMPPException if an error occurs while trying to destroy the room. An error can occur
	 *             which will be wrapped by an XMPPException -- XMPP error code 403. The error code
	 *             can be used to present more appropiate error messages to end-users.
	 */
	...
	/**
	 * Invites another user to the room in which one is an occupant. The invitation will be sent to
	 * the room which in turn will forward the invitation to the invitee.
	 * <p>
	...
	/**
	 * Invites another user to the room in which one is an occupant using a given Message. The
	 * invitation will be sent to the room which in turn will forward the invitation to the invitee.
	 * <p>
	 * If the room is password-protected, the invitee will receive a password to use to join the
	 * room. If the room is members-only, the the invitee may be added to the member list.
	...
	/**
	 * Removes a listener to invitation notifications. The listener will be fired anytime an
	...
	/**
	 * Adds a listener to invitation rejections notifications. The listener will be fired anytime an
	...
	/**
	 * Adds a listener to subject change notifications. The listener will be fired anytime the
	 * room's subject changes.
	...
	/**
	 * Removes a listener from subject change notifications. The listener will be fired anytime the
	 * room's subject changes.
	 * @param listener a subject updated listener.
	 */
	...
	/**
	 * Adds a new {@link PacketInterceptor} that will be invoked every time a new presence is going
	 * to be sent by this MultiUserChat to the server. Packet interceptors may add new extensions to
	 * the presence that is going to be sent to the MUC service.
	...
	/**
	 * Removes a {@link PacketInterceptor} that was being invoked every time a new presence was
	 * being sent by this MultiUserChat to the server. Packet interceptors may add new extensions to
	 * the presence that is going to be sent to the MUC service.
	...
	/**
	 * Returns the last known room's subject or <tt>null</tt> if the user hasn't joined the room or
	 * the room does not have a subject yet. In case the room has a subject, as soon as the user
	 * joins the room a message with the current room's subject will be received.
	 * <p>
	 * To be notified every time the room's subject change you should add a listener to this room.
	 * {@link #addSubjectUpdatedListener(SubjectUpdatedListener)}
	 * <p>
	 * To change the room's subject use {@link #changeSubject(String)}.
	 * @return the room's subject or <tt>null</tt> if the user hasn't joined the room or the room
	 ...
	 * cases it may be desirable for the user to discover the reserved nickname before attempting to
	 * enter the room.
	...
	/**
	 * Returns the nickname that was used to join the room, or <tt>null</tt> if not currently
	 * joined.
	...
	/**
	 * Changes the occupant's nickname to a new nickname within the room. Each room occupant will
	 * receive two presence packets. One of type "unavailable" for the old nickname and one
	...
	/**
	 * Changes the occupant's availability status within the room. The presence type will remain
	 * available but with a new status that describes the presence update and a new presence mode
	 * (e.g. Extended away).
	...
	/**
	 * Kicks a visitor or participant from the room. The kicked occupant will receive a presence of
	 * type "unavailable" including a status code 307 and optionally along with the reason (if
	 * provided) and the bare JID of the user who initiated the kick. After the occupant was kicked
	 * from the room, the rest of the occupants will receive a presence of type "unavailable". The
	 * presence will include a status code 307 which means that the occupant was kicked from the
	 * room.
	 * @param nickname the nickname of the participant or visitor to kick from the room (e.g.
	 *            "john").
	 * @param reason the reason why the participant or visitor is being kicked from the room.
	 * @throws XMPPException if an error occurs kicking the occupant. In particular, a 405 error can
	 *             occur if a moderator or a user with an affiliation of "owner" or "admin" was
	 *             intended to be kicked (i.e. Not Allowed error); or a 403 error can occur if the
	 *             occupant that intended to kick another occupant does not have kicking privileges
	 *             (i.e. Forbidden error); or a 400 error can occur if the provided nickname is not
	 *             present in the room.
	 */
	...
	/**
	 * Grants voice to visitors in the room. In a moderated room, a moderator may want to manage who
	 * does and does not have "voice" in the room. To have voice means that a room occupant is able
	 * to send messages to the room occupants.
	 * @param nicknames the nicknames of the visitors to grant voice in the room (e.g. "john").
	 * @throws XMPPException if an error occurs granting voice to a visitor. In particular, a 403
	 *             error can occur if the occupant that intended to grant voice is not a moderator
	 *             in this room (i.e. Forbidden error); or a 400 error can occur if the provided
	 ...
	 *             405 error can occur if a moderator or a user with an affiliation of "owner" or
	 *             "admin" was tried to revoke his voice (i.e. Not Allowed error); or a 400 error
	 *             can occur if the provided nickname is not present in the room.
	 */
	...
	/**
	 * Bans users from the room. An admin or owner of the room can ban users from a room. This means
	 * that the banned user will no longer be able to join the room unless the ban has been removed.
	 * If the banned user was present in the room then he/she will be removed from the room and
	 * notified that he/she was banned along with the reason (if provided) and the bare XMPP user ID
	 * of the user who initiated the ban.
	 * @param jids the bare XMPP user IDs of the users to ban.
	 * @throws XMPPException if an error occurs banning a user. In particular, a 405 error can occur
	 *             if a moderator or a user with an affiliation of "owner" or "admin" was tried to
	 *             be banned (i.e. Not Allowed error).
	 */
	 ...
	 * removed. If the banned user was present in the room then he/she will be removed from the room
	 * and notified that he/she was banned along with the reason (if provided) and the bare XMPP
	 * user ID of the user who initiated the ban.
	...
	/**
	 * Revokes users' membership. Only administrators are able to revoke membership. A user that
	 * becomes a room member will be able to enter a room of type Members-Only (i.e. a room that a
	...
	/**
	 * Revokes moderator privileges from other users. The occupant that loses moderator privileges
	...
	/**
	 * Grants ownership privileges to other users. Room owners may grant ownership privileges. Some
	...
	/**
	 * Revokes ownership privileges from other users. The occupant that loses ownership privileges
	...
	/**
	 * Revokes administrator privileges from users. The occupant that loses administrator privileges
	 * will become a member. Room owners may revoke administrator privileges from a member or
	 * unaffiliated user.
	 ...
	 * @param jid the bare XMPP user ID of the user to revoke administrator privileges (e.g.
	 *            "user@host.org").
	 * @throws XMPPException if an error occurs revoking administrator privileges from a user.
	 */
	 ...
	 * <p>
	 * Note: this value will only be accurate after joining the group chat, and may fluctuate over
	 * time. If you query this value directly after joining the group chat it may not be accurate,
	 * as it takes a certain amount of time for the server to send all presence packets to this
	 * client.
	 ...
	 * For example, "conference@chat.jivesoftware.com/SomeUser". Typically, a client would only
	 * display the nickname of the occupant. To get the nickname from the fully qualified name, use
	 * the {@link org.jivesoftware.smack.util.StringUtils#parseResource(String)} method. Note: this
	...
	/**
	 * Returns the presence info for a particular user, or <tt>null</tt> if the user is not in the
	 * room.
	 * <p>
	 * @param user the room occupant to search for his presence. The format of user must be:
	 *            roomName@service/nickname (e.g. darkcave@macbeth.shakespeare.lit/thirdwitch).
	 * @return the occupant's current presence, or <tt>null</tt> if the user is unavailable or if no
	...
	/**
	 * Returns the Occupant information for a particular occupant, or <tt>null</tt> if the user is
	 * not in the room. The Occupant object may include information such as full JID of the user as
	 * well as the role and affiliation of the user in the room.
	 * <p>
	 * @param user the room occupant to search for his presence. The format of user must be:
	 *            roomName@service/nickname (e.g. darkcave@macbeth.shakespeare.lit/thirdwitch).
	...
	/**
	 * Adds a packet listener that will be notified of any new Presence packets sent to the group
	 * chat. Using a listener is a suitable way to know when the list of occupants should be
	 * re-loaded due to any changes.
	 * @param listener a packet listener that will be notified of any presence packets sent to the
	 *            group chat.
	 */
	...
	/**
	 * Remoces a packet listener that was being notified of any new Presence packets sent to the
	 * group chat.
	 * @param listener a packet listener that was being notified of any presence packets sent to the
	 *            group chat.
	 */
	...
	/**
	 * Returns a new Chat for sending private messages to a given room occupant. The Chat's occupant
	 * address is the room's JID (i.e. roomName@service/nick). The server service will change the
	 * 'from' address to the sender's room JID and delivering the message to the intended
	...
	/**
	 * Polls for and returns the next message, or <tt>null</tt> if there isn't a message immediately
	 * available. This method provides significantly different functionalty than the
	 * {@link #nextMessage()} method since it's non-blocking. In other words, the method call will
	 * always return immediately, whereas the nextMessage method will return only when a message is
	 * available (or after a specific timeout).
	 * @return the next message if one is immediately available and <tt>null</tt> otherwise.
	 */
	public Message pollMessage() {
		return (Message) messageCollector.pollResult();
	}
	/**
	 * Returns the next available message in the chat. The method call will block (not return) until
	...
	/**
	 * Returns the next available message in the chat. The method call will block (not return) until
	 * a packet is available or the <tt>timeout</tt> has elapased. If the timeout elapses without a
	 * result, <tt>null</tt> will be returned.
	 * @param timeout the maximum amount of time to wait for the next message.
	 * @return the next message, or <tt>null</tt> if the timeout elapses without a message becoming
	 *         available.
	 */
	...
	/**
	 * Adds a packet listener that will be notified of any new messages in the group chat. Only
	 * "group chat" messages addressed to this group chat will be delivered to the listener. If you
	 * wish to listen for other packets that may be associated with this group chat, you should
	 * register a PacketListener directly with the Connection with the appropriate PacketListener.
	 ...
	 * Only "group chat" messages addressed to this multi user chat were being delivered to the
	 * listener.
	 * @param listener a packet listener.
	 */
	...
	/**
	 * Changes the subject within the room. As a default, only users with a role of "moderator" are
	 * allowed to change the subject in a room. Although some rooms may be configured to allow a
	 * mere participant or even a visitor to change the subject.
	 * @param subject the new room's subject to set.
	 * @throws XMPPException if someone without appropriate privileges attempts to change the room
	 *             subject will throw an error with code 403 (i.e. Forbidden)
	 */
	...
	/**
	 * Adds a listener that will be notified of changes in your status in the room such as the user
	...
	/**
	 * Removes a listener that was being notified of changes in your status in the room such as the
	...
	/**
	 * Adds a listener that will be notified of changes in occupants status in the room such as the
	 * user being kicked, banned, or granted admin permissions.
	...
	/**
	 * Removes a listener that was being notified of changes in occupants status in the room such as
	 * the user being kicked, banned, or granted admin permissions.
	 * @param listener a participant status listener.
	 */
	 ...
	 * <code>MultiUserChat</code> will be fired. On the other hand, if the occupant that changed his
	 * role is not yours then the <code>ParticipantStatusListeners</code> added to this
	 * <code>MultiUserChat</code> will be fired. The following table shows the events that will be
	 * fired depending on the previous and new role of the occupant.
	 * 
	 * <pre>
	 * <table border="1">
	 ...
	 * @param from the occupant whose role in the room has changed (e.g.
	...
	/**
	 * Fires notification events if the affiliation of a room occupant has changed. If the occupant
	 * that changed his affiliation is your occupant then the <code>UserStatusListeners</code> added
	 * to this <code>MultiUserChat</code> will be fired. On the other hand, if the occupant that
	 * changed his affiliation is not yours then the <code>ParticipantStatusListeners</code> added
	 * to this <code>MultiUserChat</code> will be fired. The following table shows the events that
	 * will be fired depending on the previous and new affiliation of the occupant.
	 * 
	 * <pre>
	 * <table border="1">
	 * <tr><td><b>Old</b></td><td><b>New</b></td><td><b>Events</b></td></tr>
	...
	/**
	 * An InvitationsMonitor monitors a given connection to detect room invitations. Every time the
	 * InvitationsMonitor detects a new invitation it will fire the invitation listeners.
	...
	private static class InvitationsMonitor implements ConnectionListener {
		...
		/**
		 * Creates a new InvitationsMonitor that will monitor invitations received on a given
		 * connection.
		...
		/**
		 * Adds a listener to invitation notifications. The listener will be fired anytime an
		 * invitation is received.
		 * <p>
		 * If this is the first monitor's listener then the monitor will be initialized in order to
		 * start listening to room invitations.
		 * @param listener an invitation listener.
		 */
		 ...
		 * invitation is received.
		 * <p>
		 * If there are no more listeners to notifiy for room invitations then the monitor will be
		 * stopped. As soon as a new listener is added to the monitor, the monitor will resume
		 ...
		 * listeners will be fired. When the connection gets closed the monitor will remove his
