...
public class ServiceDiscoveryManager {
	...
	/**
	 * Sets the name of the client that will be returned when asked for the client identity in a
	 * disco request. The name could be any value you need to identity this client.
	 ...
	 * the possible types: <a href="http://xmpp.org/registrar/disco-categories.html#client"
	 * >Jabber::Registrar</a>.
	 ...
	 * possible types: <a href="http://xmpp.org/registrar/disco-categories.html#client"
	 * >Jabber::Registrar</a>.
	...
	/**
	 * Enables caching of non caps entities to reduce traffic. If enabled discover infos of entities
	 * without xep-0115 are store in a String,DiscoverInfo map and discoverInfo(String) queries this
	 * map before sending a real discover info to the remote entity. Enabled by default.
	 */
	...
	/**
	 * Returns the NodeInformationProvider responsible for providing information (ie items) related
	 * to a given node or <tt>null</null> if none.<p>
	 * 
	...
	/**
	 * Sets the NodeInformationProvider responsible for providing information (ie items) related to
	 * a given node. Every time this client receives a disco request regarding the items of a given
	 * node, the provider associated to that node will be the responsible for providing the
	...
	/**
	 * Removes the NodeInformationProvider responsible for providing information (ie items) related
	 * to a given node. This means that no more information will be available for the specified
	 * node. In MUC, a node could be 'http://jabber.org/protocol/muc#rooms' which means that the
	 * NodeInformationProvider will provide information about the rooms where the user has joined.
	...
	/**
	 * Returns the discovered information of a given XMPP entity addressed by its JID if it's known
	 * by the entity caps manager.
	...
	/**
	 * Publishes new items to a parent entity. The item elements to publish MUST have at least a
	 * 'jid' attribute specifying the Entity ID of the item, and an action attribute which specifies
	 * the action being taken for that item. Possible action values are: "update" and "remove".
	 * @param entityID the address of the XMPP entity.
