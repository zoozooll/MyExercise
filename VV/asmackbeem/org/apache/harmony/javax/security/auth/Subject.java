...
public final class Subject implements Serializable {
	...
	private final class SecureSet<SST> extends AbstractSet<SST> implements
		...
		/*
		 * Is used to define a set type for serialization. A type can be principal, priv. or pub.
		 * credential set. The spec. doesn't clearly says that priv. and pub. credential sets can be
		 * serialized and what classes they are. It is only possible to figure out from writeObject
		 * method comments that priv. credential set is serializable and it is an instance of
		 * SecureSet class. So pub. credential was implemented by analogy Compatibility issue: the
		 * class follows its specified serial form. Also according to the serialization spec. adding
		 * new field is a compatible change. So is ok for principal set (because the default value
		 * for integer is zero). But priv. or pub. credential set it is not compatible because most
		 * probably other implementations resolve this issue in other way
		 */
		private int setType;
		...
		/*
		 * verifies specified element, checks set state, and security permission to modify set
		...
		public Iterator<SST> iterator() {
			if (permission == _PRIVATE_CREDENTIALS) {
				/*
				 * private credential set requires iterator with additional security check
				 * (PrivateCredentialPermission)
				 */
				return new SecureIterator(elements.iterator()) {
					/*
					 * checks permission to access next private credential moves to the next element
					 * even SecurityException was thrown
					 */
					@Override
					public SST next() {
