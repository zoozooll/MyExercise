...
public class LoginContext {
	...
	/*
	 * Integer constants which serve as a replacement for the corresponding LoginModuleControlFlag.*
	 * constants. These integers are used later as index in the arrays - see loginImpl() and
	 * logoutImpl() methods
	 */
	...
	/*
	 * Shows whether the subject was specified by user (true) or was created by this LoginContext
	 * itself (false).
	 */
	...
	/*
	 * Either a callback handler passed by the user or a wrapper for the user's specified handler -
	 * see init() below.
	 */
	private CallbackHandler callbackHandler;
	/*
	 * An array which keeps the instantiated and init()-ialized login modules and their states
	 */
	...
	private void init(String name, Subject subject,
		...
		//
		/*
		 * as some of the operations to be executed (i.e. get*ClassLoader, getProperty, class
		 * loading) are security-checked, then combine all of them into a single doPrivileged()
		 * call.
		 */
		try {
			AccessController
	...
	private void loginImpl() throws LoginException {
		...
		for (Module module : modules) {
			try {
				...
				if (module.klass == null) {
					/*
					 * an exception occurred during class lookup - overall attempt must fail a
					 * little trick: increase the REQUIRED's number - this will look like a failed
					 * REQUIRED module later, so overall attempt will fail
					 */
					++total[REQUIRED];
					break;
				}
		...
		boolean fail = true;
		/*
		 * Note: 'failed[xxx]!=0' is not enough to check. Use 'logged[xx] != total[xx]' instead.
		 * This is because some modules might not be counted as 'failed' if an exception occurred
		 * during preload()/Class.forName()-ing. But, such modules still get counted in the total[].
		 */
		...
		if (commited[REQUIRED] != total[REQUIRED]
			...
			if (total[REQUIRED] == 0 && total[REQUISITE] == 0) {
				/*
				 * neither REQUIRED nor REQUISITE was configured. must have at least one SUFFICIENT
				 * or OPTIONAL
				 */
