...
/**
 * Implementation of the BodyParser interface which uses the SAX API that is part of the JDK. Due to
 * the fact that we can cache and reuse SAXPArser instances, this has proven to be significantly
 * faster than the use of the javax.xml.stream API introduced in Java 6 while simultaneously
...
final class BodyParserSAX implements BodyParser {
	...
	/**
	 * Thread local to contain a SAX parser instance for each thread that attempts to use one. This
	 * allows us to gain an order of magnitude of performance as a result of not constructing
	 * parsers for each invocation while retaining thread safety.
	 */
	...
	private static final class Handler extends DefaultHandler {
		...
		/**
		 * {@inheritDoc} This implementation uses this event hook to keep track of the default
