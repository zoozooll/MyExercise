...
/**
 * BOSH Client session instance. Each communication session with a remote connection manager is
 * represented and handled by an instance of this class. This is the main entry point for
 * client-side communications. To create a new session, a client configuration must first be created
 ...
 * </pre>
 * 
 * Additional client configuration options are available. See the {@code BOSHClientConfig.Builder}
 * class for more information.
 * <p/>
 * Once a {@code BOSHClient} instance has been created, communication with the remote connection
 * manager can begin. No attempt will be made to establish a connection to the connection manager
 * until the first call is made to the {@code send(ComposableBody)} method. Note that it is possible
 * to send an empty body to cause an immediate connection attempt to the connection manager. Sending
 * an empty message would look like the following:
 * 
 * <pre>
 ...
 * <p/>
 * Once a session has been successfully started, the client instance can be used to send arbitrary
 * payload data. All aspects of the BOSH protocol involving setting and processing attributes in the
 * BOSH namespace will be handled by the client code transparently and behind the scenes. The user
 * of the client instance can therefore concentrate entirely on the content of the message payload,
 ...
 * {@code BOSHClientResponseListener} should be added to the client instance. All incoming messages
 * will be published to all response listeners as they arrive and are processed. As with the
 * transmission of payload data via the {@code send(ComposableBody)} method, there is no need to
 * worry about handling of the BOSH attributes, since this is handled behind the scenes.
 * <p/>
 * If the connection to the remote connection manager is terminated (either explicitly or due to a
 * terminal condition of some sort), all connection listeners will be notified. After the connection
 * has been closed, the client instance is considered dead and a new one must be created in order to
 * resume communications with the remote server.
 * <p/>
...
public final class BOSHClient {
	...
	/**
	 * The amount of time in milliseconds which will be reserved as a safety margin when scheduling
	 * empty requests against a maxpause value. This should give us enough time to build the message
	 ...
	 */
	/**
	 * Thread which is used to process responses from the connection manager. Becomes null when
	...
	/**
	 * The highest RID that we've already received a response for. This value is used to implement
	 * response acks.
	 */
	private Long responseAck = Long.valueOf(-1L);
	/**
	 * List of requests which have been made but not yet acknowledged. This list remains unpopulated
	 * if the CM is not acking requests.
	 */
	...
	abstract static class ExchangeInterceptor {
		 ...
		 * @return replacement exchange instance, or {@code null} to skip processing of this
		 *         exchange
		 */
	...
	// Constructors:
	/**
	 * Determine whether or not we should perform assertions. Assertions can be specified via system
	 * property explicitly, or defaulted to the JVM assertions status.
	 */
	static {
	...
	/**
	 * Send the provided message data to the remote connection manager. The provided message body
	 * does not need to have any BOSH-specific attribute information set. It only needs to contain
	 * the actual message payload that should be delivered to the remote server.
	 * <p/>
	 * The first call to this method will result in a connection attempt to the remote connection
	 * manager. Subsequent calls to this method will block until the underlying session state allows
	 * for the message to be transmitted. In certain scenarios - such as when the maximum number of
	 * outbound connections has been reached - calls to this method will block for short periods of
	 * time.
	...
	/**
	 * Attempt to pause the current session. When supported by the remote connection manager,
	 * pausing the session will result in the connection manager closing out all outstanding
	 * requests (including the pause request) and increases the inactivity timeout of the session.
	 * The exact value of the temporary timeout is dependent upon the connection manager. This
	 * method should be used if a client encounters an exceptional temporary situation during which
	 * it will be unable to send requests to the connection manager for a period of time greater
	 * than the maximum inactivity period. The session will revert back to it's normal, unpaused
	 * state when the client sends it's next message.
	 * @return {@code true} if the connection manager supports session pausing, {@code false} if the
	 *         connection manager does not support session pausing or if the session has not yet
	 *         been established
	 */
	...
	/**
	 * End the BOSH session by disconnecting from the remote BOSH connection manager, sending the
	 * provided content in the final connection termination message.
	...
	/**
	 * Forcibly close this client session instance. The preferred mechanism to close the connection
	 * is to send a disconnect message and wait for organic termination. Calling this method simply
	 * shuts down the local session without sending a termination message, releasing all resources
	...
	// Private methods:
	/**
	 * Initialize the session. This initializes the underlying HTTP transport implementation and
	...
	/**
	 * Evaluates the HTTP response code and response message and returns the terminal binding
	 * condition that it describes, if any.
	 * @param respCode HTTP response code
	 * @param respBody response body
	 * @return terminal binding condition, or {@code null} if not a terminal binding condition
	 *         message
	 */
	...
	/**
	 * Determines if the message specified is immediately sendable or if it needs to block until the
	...
	/**
	 * Blocks until either the message provided becomes immediately sendable or until the session is
	 * terminated.
	 * @param msg message to evaluate
	 */
	...
	/**
	 * Applies the local station ID information to the request message who's builder has been
	 * provided.
	...
	/**
	 * Applies existing session data to the outbound request, returning the modified request. This
	 * method assumes the lock is currently held.
	 * @param rid request ID to use
	...
	/**
	 * Sets the 'ack' attribute of the request to the value of the highest 'rid' of a request for
	 * which it has already received a response in the case where it has also received all responses
	 * associated with lower 'rid' values. The only exception is that, after its session creation
	 * request, the client SHOULD NOT include an 'ack' attribute in any request if it has received
	...
	/**
	 * While we are "connected", process received responses. This method is run in the processing
	 * thread.
	 */
	...
	/**
	 * Get the next message exchange to process, blocking until one becomes available if nothing is
	 * already waiting for processing.
	 * @return next available exchange to process, or {@code null} if no exchanges are immediately
	 *         available
	 */
	...
	/**
	 * Process the next, provided exchange. This is the main processing method of the receive
	 * thread.
	...
	/**
	 * Schedule an empty request to be sent if no other requests are sent in a reasonable amount of
	 * time.
	 */
	...
	/**
	 * Sends an empty request to maintain session requirements. If a request is sent within a
	 * reasonable time window, the empty request transmission will be cancelled.
	 */
	...
	/**
	 * Checks to see if the response indicates a terminal binding condition (as per XEP-0124 section
	...
	/**
	 * Determines whether or not the response indicates a recoverable binding condition (as per
	 * XEP-0124 section 17).
	...
	/**
	 * Process the request to determine if the empty request delay can be determined by looking to
	 * see if the request is a pause request. If it can, the request's delay is returned, otherwise
	...
	/**
	 * Check the response for request acknowledgements and take appropriate action. This method
	...
	/**
	 * Process the response in order to update the response acknowlegement data. This method assumes
	...
	/**
	 * Process the response in order to check for and respond to any potential ack reports. This
	 * method assumes the lock is currently held.
	 * @param resp response
	 * @return exchange to transmit if a resend is to be performed, or {@code null} if no resend is
	 *         necessary
	...
	/**
	 * Notifies all connection listeners that the session has been terminated due to the exceptional
	 * condition provided.
