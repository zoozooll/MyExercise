...
/**
 * The TokenParser class will parse individual tokens from a list of tokens that are a directive
 * value for a DigestMD5 authentication.The tokens are separated commas.
 */
class TokenParser extends Object {
	...
	/**
	 * This function parses the next token from the tokens string and returns it as a string. If
	 * there are no more tokens a null reference is returned.
				...
				case STATE_LOOKING_FOR_TOKEN:
					if (isWhiteSpace(currChar)) {
						...
						throw new SaslException(
								"Invalid token character at position "
										+ m_curPos);
					}
					break;
