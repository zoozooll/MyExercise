...
class DirectiveList extends Object {
	...
	/**
	 * This function takes a US-ASCII character string containing a list of comma separated
	 * directives, and parses the string into the individual directives and their values. A
	 * directive consists of a token specifying the directive name followed by an equal sign (=) and
	 * the directive value. The value is either a token or a quoted string
			...
			switch (m_state) {
				...
				case STATE_SCANNING_NAME:
					if (isValidTokenChar(currChar)) {
						...
						currentName = m_directives.substring(m_scanStart,
								m_curPos);
...
/**
	...
	/**
	 * This function returns TRUE if the character is linear white space (LWS). LWS = [CRLF] 1*( SP
	 * | HT )
	...
	/**
	 * This function creates a directive record and adds it to the list, the value will be added
	 * later after it is parsed.
	 * @param name Name
