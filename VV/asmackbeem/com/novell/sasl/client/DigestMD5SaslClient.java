...
public class DigestMD5SaslClient implements SaslClient {
	...
	/**
	 * Creates an DigestMD5SaslClient object using the parameters supplied. Assumes that the QOP,
	 * STRENGTH, and SERVER_AUTH properties are contained in props
	 * @param authorizationId The possibly null protocol-dependent identification to be used for
	 *            authorization. If null or empty, the server derives an authorization ID from the
	 *            client's authentication credentials. When the SASL authentication completes
	 ...
	 *            configure the authentication exchange of the selected mechanism. See the Sasl
	 *            class for a list of standard properties. Other, possibly mechanism-specific,
	 *            properties can be included. Properties not relevant to the selected mechanism are
	 *            ignored.
	 * @param cbh The possibly null callback handler to used by the SASL mechanisms to get further
	 *            information from the application/library to complete the authentication. For
	 *            example, a SASL mechanism might require the authentication ID, password and realm
	 *            from the caller. The authentication ID is requested by using a NameCallback. The
	 *            password is requested by using a PasswordCallback. The realm is requested by using
	 *            a RealmChoiceCallback if there is a list of realms to choose from, and by using a
	 *            RealmCallback if the realm must be entered.
	 * @return A possibly null SaslClient created using the parameters supplied. If null, this
	 *         factory cannot produce a SaslClient using the parameters supplied.
	 * @exception SaslException If a SaslClient instance cannot be created because of an error
	 */
	...
	/**
	 * Determines if this mechanism has an optional initial response. If true, caller should call
	 * evaluateChallenge() with an empty array to get the initial response.
	...
	/**
	 * Determines if the authentication exchange has completed. This method may be called at any
	 * time, but typically, it will not be called until the caller has received indication from the
	 * server (in a protocol- specific manner) that the exchange has completed.
	...
	/**
	 * Unwraps a byte array received from the server. This method can be called only after the
	 * authentication exchange has completed (i.e., when isComplete() returns true) and only if the
	 * authentication exchange has negotiated integrity and/or privacy as the quality of protection;
	 * otherwise, an IllegalStateException is thrown. incoming is the contents of the SASL buffer as
	 * defined in RFC 2222 without the leading four octet field that represents the length. offset
	...
	/**
	 * Wraps a byte array to be sent to the server. This method can be called only after the
	 * authentication exchange has completed (i.e., when isComplete() returns true) and only if the
	 * authentication exchange has negotiated integrity and/or privacy as the quality of protection;
	 * otherwise, an IllegalStateException is thrown. The result of this method will make up the
	 * contents of the SASL buffer as defined in RFC 2222 without the leading four octet field that
	 * represents the length. offset and len specify the portion of outgoing to use.
	 ...
	 * @exception IllegalStateException if the authentication exchange has not completed, or if the
	...
	/**
	 * Retrieves the negotiated property. This method can be called only after the authentication
	 * exchange has completed (i.e., when isComplete() returns true); otherwise, an
	 ...
	 * using. Invoking this method invalidates the SaslClient instance. This method is idempotent.
	...
	/**
	 * Evaluates the challenge data and generates a response. If a challenge is received from the
	 * server during the authentication process, this method is called to prepare an appropriate
	 * next response to submit to the server.
	 * @param challenge The non-null challenge sent from the server. The challenge array may have
	 *            zero length.
	 * @return The possibly null reponse to send to the server. It is null if the challenge
	 *         accompanied a "SUCCESS" status and the challenge only contains data for the client to
	 *         update its state and no response needs to be sent to the server. The response is a
	 *         zero-length byte array if the client is to send a response with no data.
	 * @exception SaslException If an error occurred while processing the challenge or generating a
	 *                response.
	 */
	@Override
		...
		switch (m_state) {
			case STATE_INITIAL:
				if (challenge.length == 0)
					...
					try {
						response = createDigestResponse(challenge).getBytes(
								"UTF-8");
	...
	/**
	 * This function takes a 16 byte binary md5-hash value and creates a 32 character (plus a
	 * terminating null character) hex-digit representation of binary data.
	 * @param hash 16 byte binary md5-hash value in bytes
	 * @return 32 character (plus a terminating null character) hex-digit representation of binary
	 *         data.
	 */
	...
	/**
	 * This function calculates the response-value of the response directive of the digest-response
	 * as documented in RFC 2831
	 * @param HA1 H(A1)
	...
	/**
	 * This function validates the server response. This step performs a modicum of mutual
	 * authentication by verifying that the server knows the user's password
