...
public abstract class PullToRefreshBase<T extends View> extends LinearLayout
	...
	/**
	 * @ClassName: PullType
	 * @Description: 拉载方式:PullUp-上拉加载,PullDown-下拉加载
	 * @author: yuedong bao
	 * @date: 2015-8-24 下午1:57:44
	 */
	...
	protected boolean mIsHandledTouchEvent = false;
	/** 移动点的保护范围值 */
	...
	private void refreshLoadingViewsSize() {
		...
		// 然后再松开手，就会触发刷新操作
		int headerHeight = mHeaderLayout.getContentSize();
		int footerHeight = mFooterLayout.getContentSize();
		if (headerHeight < 0) {
			headerHeight = 0;
		}
		if (footerHeight < 0) {
	...
	/**
	 * @Title: isHeaderLoadingLayoutVisible
	 * @Description:头部加载是否可见
	 * @param: @return
	 * @return: boolean
	 * @throws:
	 */
	public boolean isHeaderLoadingLayoutVisible() {
		int scrollY = Math.abs(getScrollYValue());
		return scrollY > 0;
	}
	/**
	 * @Title: isFooterLoadingLayoutVisible
	 * @Description: 尾部是否可见
	 * @param: @return
	 * @return: boolean
	 * @throws:
	 */
	public boolean isFooterLoadingLayoutVisible() {
		return getFooterLoadingLayout().isVisible();
	}
	@Override
	 ...
	 * 结束上拉加载更多
	 */
	protected void onPullUpRefreshComplete(boolean... noMoreData) {
		if (isPullLoading()) {
			if (noMoreData.length > 0 && noMoreData[0]) {
				mPullUpState = State.NO_MORE_DATA;
			} else {
	...
	/**
	 * @Title: onPullComplete
	 * @Description: 完成拉动动作
	 * @param:
	 * @return: void
	 * @throws:
	 */
	public void onPullRefreshComplete(boolean... noMoreData) {
		onPullDownRefreshComplete();
		onPullUpRefreshComplete(noMoreData);
	}
	/**
	 * 将刷新View添加到当前容器中
	 * @param context context
	 ...
	 * @param context context
	 */
	public abstract void addHeaderAndFooter(Context context);
	/**
	 * 拉动Header Layout时调用
	 * @param delta 移动的距离
	 */
			...
			postDelayed(new Runnable() {
				...
				public void run() {
					mRefreshListener.onPullDownToRefresh(
							PullToRefreshBase.this, PullType.PullDown);
				}
			}, getSmoothScrollDuration());
		}
	}
	...
	protected void startLoading() {
		...
		if (null != mRefreshListener) {
			// 因为滚动回原始位置的时间是SCROLL_DURATION，我们需要等回滚完后才执行加载回调
			postDelayed(new Runnable() {
				@Override
				public void run() {
