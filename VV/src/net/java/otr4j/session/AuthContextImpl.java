	...
	class MessageFactory {
		...
		private RevealSignatureMessage getRevealSignatureMessage()
			...
			try {
				...
				byte[] mhash = otrCryptoEngine.sha256Hmac(
						SerializationUtils.toByteArray(m), getM1());
		...
		private SignatureMessage getSignatureMessage() throws OtrException {
			...
			byte[] mhash;
			try {
				mhash = otrCryptoEngine.sha256Hmac(
						SerializationUtils.toByteArray(m), getM1p());
			} catch (IOException e) {
				throw new OtrException(e);
			}
			byte[] signature = otrCryptoEngine.sign(mhash,
	...
	private void handleSignatureMessage(SignatureMessage m) throws OtrException {
		...
		switch (this.getAuthenticationState()) {
			case AWAITING_SIG:
				...
				SignatureX remoteX;
				try {
					remoteX = SerializationUtils
							.toMysteriousX(remoteXDecrypted);
				} catch (IOException e) {
					throw new OtrException(e);
				}
				// Compute signature.
				PublicKey remoteLongTermPublicKey = remoteX.longTermPublicKey;
				SignatureM remoteM = new SignatureM(
						this.getRemoteDHPublicKey(), (DHPublicKey) this
				...
				byte[] signature;
				try {
					signature = otrCryptoEngine.sha256Hmac(
							SerializationUtils.toByteArray(remoteM),
							this.getM1p());
				} catch (IOException e) {
					throw new OtrException(e);
				}
	...
	private void handleRevealSignatureMessage(RevealSignatureMessage m)
		...
		switch (this.getAuthenticationState()) {
			case AWAITING_REVEALSIG:
				...
				this.setRemoteLongTermPublicKey(remoteLongTermPublicKey);
				getSession()
						.injectMessage(messageFactory.getSignatureMessage());
				break;
			default:
	...
	private void handleDHCommitMessage(DHCommitMessage m) throws OtrException {
		...
		switch (this.getAuthenticationState()) {
			...
			case AWAITING_DHKEY:
				...
				BigInteger ourHash = new BigInteger(1,
						this.getLocalDHPublicKeyHash());
				...
				if (theirHash.compareTo(ourHash) == -1) {
					...
					// Commit message.
					getSession().injectMessage(
							messageFactory.getDHCommitMessage());
					...
					this.setAuthenticationState(AuthContext.AWAITING_REVEALSIG);
					getSession()
							.injectMessage(messageFactory.getDHKeyMessage());
