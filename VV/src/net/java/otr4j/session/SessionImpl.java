	...
	private String handleDataMessage(DataMessage data) throws OtrException {
		...
		switch (this.getSessionStatus()) {
			case ENCRYPTED:
				...
				SessionKeys matchingKeys = this.getSessionKeysByID(
				...
				byte[] dmc = otrCryptoEngine.aesDecrypt(
				...
				if (tlvIndex > -1) {
					decryptedMsgContent = decryptedMsgContent.substring(0,
							tlvIndex);
					tlvIndex++;
	...
	public String transformSending(String msgText, List<TLV> tlvs)
		...
		switch (this.getSessionStatus()) {
			...
			case ENCRYPTED:
				...
				byte[] encryptedMsg = otrCryptoEngine.aesEncrypt(
						encryptionKeys.getSendingAESKey(), ctr, data);
				// Get most recent keys to get the next D-H public key.
				SessionKeys mostRecentKeys = this.getMostRecentSessionKeys();
				DHPublicKey nextDH = (DHPublicKey) mostRecentKeys
						.getLocalPair().getPublic();
				// Calculate T.
				MysteriousT t = new MysteriousT(2, 0, senderKeyID,
						receipientKeyID, nextDH, ctr, encryptedMsg);
				// Calculate T hash.
				...
				byte[] mac = otrCryptoEngine.sha1Hmac(serializedT,
						sendingMACKey, SerializationConstants.TYPE_LEN_MAC);
