				...
				case SerializationConstants.HEAD_ENCODED:
					...
					switch (messageType) {
						case AbstractEncodedMessage.MESSAGE_DATA:
							...
							return new DataMessage(protocolVersion, flags,
									senderKeyID, recipientKeyID, nextDH, ctr,
						...
						case AbstractEncodedMessage.MESSAGE_DHKEY:
							...
							return new DHKeyMessage(protocolVersion,
									dhPublicKey);
						case AbstractEncodedMessage.MESSAGE_REVEALSIG: {
						...
						case AbstractEncodedMessage.MESSAGE_SIGNATURE: {
							...
							return new SignatureMessage(protocolVersion,
									xEncryted, xEncryptedMac);
						}
						default:
							throw new IOException("Illegal message type.");
					}
				case SerializationConstants.HEAD_ERROR:
					return new ErrorMessage(AbstractMessage.MESSAGE_ERROR,
							content);
				...
				case SerializationConstants.HEAD_QUERY_Q:
					...
					if (SerializationConstants.HEAD_QUERY_Q == contentType) {
						...
						if (content.charAt(0) == 'v') {
							versionString = content.substring(1,
									content.indexOf('?'));
						}
					} else if (SerializationConstants.HEAD_QUERY_V == contentType) {
						versionString = content.substring(0,
								content.indexOf('?'));
					}
