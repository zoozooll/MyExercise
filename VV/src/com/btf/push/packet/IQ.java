...
/**
 * The base IQ (Info/Query) packet. IQ packets are used to get and set information on the server,
 * including authentication, roster operations, and creating accounts. Each IQ packet has a specific
 * type that indicates what type of action is being taken: "get", "set", "result", or "error".
 * <p>
 * IQ packets can contain a single child element that exists in a specific XML namespace. The
 * combination of the element name and namespace determines what type of IQ packet it is. Some
 * example IQ subpacket snippets:
 * <ul>
...
public abstract class IQ extends Packet {
	...
	/**
	 * Convenience method to create a new {@link Type#ERROR IQ.Type.ERROR} IQ based on a
	 * {@link Type#GET IQ.Type.GET} or {@link Type#SET IQ.Type.SET} IQ. The new packet will be
	 * initialized with:
	 * <ul>
	 * <li>The sender set to the recipient of the originating IQ.
	...
	public static class Type {
		...
		public static final Type CHECKPWD = new Type("check");
		public static final Type RESETPWD = new Type("retsetPass");
		public static final Type UPDATEPWD = new Type("modify");

		/**
		 * Converts a String into the corresponding types. Valid String values that can be converted
		 * to types are: "get", "set", "result", and "error".
		...
		public static Type fromString(String type) {
			...
			if (GET.toString().equals(type)) {
				...
				return RESULT;
			} else if (RESETPWD.toString().equals(type)) {
				return RESETPWD;
			} else if (CHECKPWD.toString().equals(type)) {
				return CHECKPWD;
			} else if (UPDATEPWD.toString().equals(type)) {
				return UPDATEPWD;
			} else {
				return null;
			}
