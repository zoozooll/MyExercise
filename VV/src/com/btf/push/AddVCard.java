 ...
 */
public class AddVCard extends IQ {
	/**
	...
	private void doLoad(Connection connection, String user)
		...
		PacketCollector collector = connection
				.createPacketCollector(new PacketIDFilter(getPacketID()));
		connection.sendPacket(this);
		AddVCard result = null;
		try {
			result = (AddVCard) collector.nextResult(SmackConfiguration
					.getPacketReplyTimeout());
			if (result == null) {
	...
	private void copyFieldsFrom(AddVCard result) {
		if (result == null)
			result = new AddVCard();
		Field[] fields = AddVCard.class.getDeclaredFields();
		for (Field field : fields) {
			if (field.getDeclaringClass() == AddVCard.class
	...
	private boolean hasSchoolFields() {
		return schoolname != null || entratime != null || major != null;
	}
	...
	public boolean equals(Object o) {
		...
		if (o == null || getClass() != o.getClass())
			return false;
		final AddVCard vCard = (AddVCard) o;
		...
		if (binval != null ? !binval.equals(vCard.binval)
				: vCard.binval != null) {
			return false;
		}
		...
		if (major != null ? !major.equals(vCard.major) : vCard.major != null) {
			return false;
		}
	...
	private class VCardWriter {
		...
		public void write() {
			appendTag("vCard", "xmlns", "info", hasContent(),
					new ContentBuilder() {
						@Override
		...
		private void appendCity() {
			if (hasCityFields()) {
				appendTag("ADR", true, new ContentBuilder() {
					@Override
					public void addTagContent() {
						appendTag("REGION", StringUtils.escapeForXML(fromCity));
						// appendTag("LON", StringUtils.escapeForXML(geolon));
					}
				});
			}
			...
			if (hasSchoolFields()) {
				appendTag("SCH", true, new ContentBuilder() {
					...
					public void addTagContent() {
						...
						appendTag("SPEC", StringUtils.escapeForXML(major));
					}
				});
			}
		}
