...
public class VCardProvider implements IQProvider {
	 ...
	 */
	public static AddVCard createVCardFromXML(String xml) throws Exception {
		// Log.i("VV", "createVCardFromXML");
		AddVCard vCard = new AddVCard();
	...
	private static class VCardReader {
		private final AddVCard vCard;
		private final Document document;

		VCardReader(AddVCard vCard, Document document) {
			this.vCard = vCard;
			this.document = document;
		}
		public void initializeFields() {
			// Log.i("VV", "SCHOOL "+getTagContents(getTagContents("SCHOOL")));
			...
			// System.out.println("SYSTIME "+getTagContents("SYSTIME"));
			setupSimpleFields();
			setupPhones();
			setupAddresses();
			setupGEO();
		...
		private void setupGEO() {
			NodeList allAddresses = document.getElementsByTagName("GEO");
			if (allAddresses == null)
				return;
			for (int i = 0; i < allAddresses.getLength(); i++) {
				Element addressNode = (Element) allAddresses.item(i);
				...
				for (int j = 0; j < childNodes.getLength(); j++) {
					...
					if (nodeName.equals("LAT")) {
						vCard.setGeolat(getTextContent(node));
					}
					if (nodeName.equals("LON")) {
						vCard.setGeolon(getTextContent(node));
					}
				}
			}
		}
		private void setupSCH() {
			NodeList allAddresses = document.getElementsByTagName("SCH");
			if (allAddresses == null)
				return;
			for (int i = 0; i < allAddresses.getLength(); i++) {
				Element addressNode = (Element) allAddresses.item(i);
				...
				for (int j = 0; j < childNodes.getLength(); j++) {
					...
					if (nodeName.equals("SCHOOL")) {
						vCard.setSchoolname(getTextContent(node));
					}
					if (nodeName.equals("TIME")) {
						vCard.setEntratime(getTextContent(node));
					}
					if (nodeName.equals("SPEC")) {
						vCard.setMajor(getTextContent(node));
					}
				}
			}
		}
		private void setupPHOTO() {
			NodeList allAddresses = document.getElementsByTagName("PHOTO");
			if (allAddresses == null)
				return;
			for (int i = 0; i < allAddresses.getLength(); i++) {
				Element addressNode = (Element) allAddresses.item(i);
				...
				for (int j = 0; j < childNodes.getLength(); j++) {
					...
					if (nodeName.equals("BINVAL")) {
						vCard.setBinval(getTextContent(node));
					}
				}
			}
		}
