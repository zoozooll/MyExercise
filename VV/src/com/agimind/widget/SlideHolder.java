...
public class SlideHolder extends FrameLayout implements BlockHolder {
	...
	public static final float SLIDE_DENOMINATOR = 6;
	public static final int SLIDE_X_SPACE_IN_DIP = 20;
	// add by baoyuedong 控制拖动距离为几分之屏幕，开始自由滑动
	private float slideDenominator = SLIDE_DENOMINATOR;

	public float getSlideDenominator() {
		return slideDenominator;
	}
	public void setSlideDenominator(float slideDenominator) {
		this.slideDenominator = slideDenominator;
	}
	...
	/**
	 * @param direction - direction in which SlideHolder opens. Can be: DIRECTION_LEFT,
	 *            DIRECTION_RIGHT
	 */
	...
	/**
	 * @param allow - if false, SlideHolder won't react to swiping gestures (but still will be able
	 *            to work by manually invoking mathods)
	 */
	...
	/**
	 * @param dispatch - if true, in open state SlideHolder will dispatch touch events to main
	 *            layout (in other words - it will be clickable)
	 */
	...
	/**
	 * @param opened - if true, SlideHolder will always be in opened state (which means that swiping
	 *            won't work)
	 */
	...
	protected void dispatchDraw(Canvas canvas) {
		try {
			...
			/*
			 * Possibility of crashes on some devices (especially on Samsung). Usually, when
			 * ListView is empty.
			 */
			Log.w(TAG, e);
		}
	...
	private boolean handleTouchEvent(MotionEvent ev) {
		...
		if (ev.getAction() == MotionEvent.ACTION_MOVE) {
			...
			float diffY = y - mHistoricalY;
			if (Math.abs(diffY) / Math.abs(diff) <= 1) {
				if ((mDirection * diff > DipToPx(SLIDE_X_SPACE_IN_DIP, mContext) && mMode == MODE_READY)
						|| (Math.abs(mDirection * diff) > DipToPx(
								SLIDE_X_SPACE_IN_DIP, mContext) && mMode == MODE_FINISHED)) {
					mHistoricalX = (int) x;
					mHistoricalY = (int) y;
					// Log.d(TAG, "handleTouchEvent, diff = " + diff +
					// ", mDirection = " +
					// mDirection);
					initSlideMode();
	...
	private void finishSlide() {
		if (mDirection * mEndOffset > 0) {
			// 侧滑菜单打开方向
			Log.d(TAG, "finishSlide open, mDirection = " + mDirection
					+ ", mEndOffset = " + mEndOffset);
			Log.d(TAG, "finishSlide open, mOffset = " + mOffset
					+ ", slideDenominator = " + slideDenominator);
			...
			// 侧滑菜单关闭方向
			Log.d(TAG, "finishSlide close, mOffset = " + mOffset
					+ ", mStartOffset = " + mStartOffset);
			Log.d(TAG, "finishSlide close, mEndOffset = " + mEndOffset
	...
	public static int DipToPx(int dipIn, Context context) {
		...
		// Log.d(TAG, "DipToPx, densityScale = " + densityScale);
