 ...
 * .com/platform/frameworks/base/+/jb-release/core/java/android/os/AsyncTask.java so that threading
 * behavior on all OS versions is the same and we can tweak behavior by using executeOnExecutor() if
 * needed. There are 3 changes in this copy of AsyncTask: -pre-HC a single thread executor is used
 * for serial operation (Executors.newSingleThreadExecutor) and is the default -the default
 ...
 * and does not constitute a generic threading framework. AsyncTasks should ideally be used for
 * short operations (a few seconds at the most.) If you need to keep threads running for long
...
public abstract class AsyncTask<Params, Progress, Result> {
	 ...
	 */
	public static final Executor SERIAL_EXECUTOR = Build.VERSION.SDK_INT > 10 ? new SerialExecutor()
	 ...
	 * execution of {@link #onProgressUpdate} on the UI thread. {@link #onProgressUpdate} will note
