 ...
 */
package com.beem.project.btf.otr;
...
import com.beem.project.btf.service.ChatAdapter;
...
public class BeemOtrManager implements OtrEngineHost {
	...
	// We will have a global policy for Beem as long as we won't need to modify
	// the policy per chat.
	private static final OtrPolicy mGlobalPolicy = new OtrPolicyImpl(
	...
	/**
	 * We must call addChat before stating a new otr session because we will need the chat instance
	...
	private class BeemOtrListener implements OtrEngineListener {
		...
		public void sessionStatusChanged(final SessionID sessionID) {
			...
			if (mOtrKeyManager.loadRemotePublicKey(sessionID) == null) {
				mOtrKeyManager.savePublicKey(sessionID,
						mOtrEngine.getRemotePublicKey(sessionID));
			}
			SessionStatus status = mOtrEngine.getSessionStatus(sessionID);
			if (status.equals(SessionStatus.ENCRYPTED)
					&& mOtrKeyManager.isVerified(sessionID)) {
