...
/**
 * Wraps a ListAdapter to give it expandable list view functionality. The main thing it does is add
 * a listener to the getToggleButton which expands the getExpandableView for each list item.
 * @author tjerk
 * @date 6/9/12 4:41 PM
 */
public abstract class AbstractSlideExpandableListAdapter extends
	...
	/**
	 * Reference to the last expanded list item. Since lists are recycled this might be null if
	 * though there is an expanded list item
	 */
	private View lastOpen = null;
	/**
	 * The position of the last expanded list item. If -1 there is no list item expanded. Otherwise
	 * it points to the position of the last expanded list item
	 */
	...
	/**
	 * A list of positions of all list items that are expanded. Normally only one is expanded. But a
	 * mode to expand multiple will be added soon. If an item onj position x is open, its bit is set
	 */
	private BitSet openItems = new BitSet();
	/**
	 * We remember, for each collapsable view its height. So we dont need to recalculate. The height
	 * is calculated just before the view is drawn.
	 */
	...
	/**
	 * This method is used to get the Button view that should expand or collapse the Expandable
	 * View. <br/>
	 ...
	 * </pre>
	 * 
	 * A listener will be attached to the button which will either expand or collapse the expandable
	 * view
	...
	/**
	 * This method is used to get the view that will be hidden initially and expands or collapse
	 * when the ExpandToggleButton is pressed @see getExpandToggleButton <br/>
	 * Normally it will be implemented as:
	 * 
	 * <pre>
	 ...
	 * @param parent the list view item
	 * @ensure return!=null
	 * @return a child of parent which is a view (or often ViewGroup) that can be collapsed and
	 *         expanded
	 */
	public abstract View getExpandableView(View parent);
	/**
	 * Gets the duration of the collapse animation in ms. Default is 330ms. Override this method to
	 * change the default.
	...
	private void enableFor(final View button, final View target,
		...
		if (position != getCount() - 1) {
			setConnectionState(target, position);
		}
		button.setOnClickListener(new View.OnClickListener() {
			...
			public void onClick(final View view) {
				...
				if (a != null && a.hasStarted() && !a.hasEnded()) {
					...
					// add by baoyuedong 2014-10-22:设置Selected：true-展开 false-收起
					button.findViewById(R.id.up_toggle).setSelected(false);
					if (lastOpen != null) {
						((View) lastOpen.getParent()).findViewById(
								R.id.up_toggle).setSelected(false);
					}
					// check if we need to collapse a different view
					if (type == ExpandCollapseAnimation.EXPAND) {
						Log.i("yangle", "~~展开~~");
						button.findViewById(R.id.up_toggle).setSelected(true);
						if (lastOpenPosition != -1
								&& lastOpenPosition != position) {
							Log.i("yangle", "~~");
							if (lastOpen != null) {
								// 将以前的view关掉
								if (lastOpenPosition != getCount() - 1) {
									setConnectLineAnim(
											ExpandCollapseAnimation.EXPAND,
											lastOpen);
								}
					...
					if (position != getCount() - 1) {
						setConnectLineAnim(Math.abs(type - 1), target);
					}
				}
			}
		});
	}
	private void setConnectLineAnim(int type, View target) {
		View connectline = ((View) target.getParent())
				.findViewById(R.id.connectline);
		animateView(connectline, type);
	}
	private void setConnectionState(View target, int position) {
		View connectline = ((View) target.getParent())
				.findViewById(R.id.connectline);
		final LinearLayout.LayoutParams params = (LinearLayout.LayoutParams) connectline
				.getLayoutParams();
		if (openItems.get(position)) {
			connectline.setVisibility(View.GONE);
			params.bottomMargin = -connectline.getMeasuredHeight();
		} else {
	...
	private ExpandToggleButtonLis expandtogglebuttonlis;

	public void setExpandtogglebuttonlis(
			ExpandToggleButtonLis expandtogglebuttonlis) {
		this.expandtogglebuttonlis = expandtogglebuttonlis;
	}

	public interface ExpandToggleButtonLis {
		void change(int state, Object... params);
	}
